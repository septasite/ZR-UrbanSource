...
BOOL CPadDoc::OnOpenDocument(LPCTSTR lpszPathName)
{
	...
	if( strExt != _T(".CHARSET") && strExt != _T(".CHARCLASS") &&
		...
		strExt != _T(".GENITEM") && strExt != _T(".BIN") && strExt != _T(".INI")&& strExt != _T(".ALS"))
	{
	...
	int nVersion, nBufSize;

	CFile file;
	file.Open( lpszPathName, CFile::modeRead );	// 파일 열기
	//file.Read( &nVersion, sizeof(int) );							// 버젼 쓰기
	//file.Read( &nLen, sizeof(int) );
	nBufSize = file.SeekToEnd();
	file.SeekToBegin();
	char * szBuffer = new char[nBufSize];
	nBufSize = file.Read( szBuffer, nBufSize );		// 파일 쓰기

	CRijndael oRijndael;

		memcpy( &nVersion, szBuffer, sizeof(int) );
//		if( nVersion > 0 && CRijndael::VERSION >= nVersion )
		if( CRijndael::VERSION == nVersion )
		{
			oRijndael.Initialize( CRijndael::sm_Version[nVersion-1].c_str(), CRijndael::sm_KeyLength[nVersion-1], 
									CRijndael::sm_chain0, CRijndael::sm_KeyLength[nVersion-1] );

			nBufSize -= sizeof(int);
			BYTE * pTemp = (BYTE*)((int)szBuffer + sizeof(int));
			memcpy( szBuffer, pTemp, nBufSize );

			oRijndael.DecryptEx( (char*)szBuffer, nBufSize );
		}
	...
	// 에디트뷰에 출력
	reinterpret_cast<CEditView*>(m_viewList.GetHead())->SetWindowText(szBuffer);//strTemp.GetString());
