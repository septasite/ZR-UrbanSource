...
BOOL CCrystalTextBuffer::LoadFromFile(LPCTSTR pszFileName, int nCrlfStyle /*= CRLF_STYLE_AUTOMATIC*/)
{
...

	{
		...
		if (dwFileAttributes == (DWORD) -1)			return FALSE;
		...
		for ( int i=0; i<6; ++i )
		{
			int nFind = strFileName.Find ( strExt[i], 0 );
			if ( nFind != LB_ERR )
			{
				bDeCode = TRUE;
				break;
		...
		CStringFile StrFile;
		if ( !StrFile.Open ( pszFileName, bDeCode ) )	return FALSE;

		CString strLine;
		while ( StrFile.GetNextLine(strLine) )
		{
			InsertLine(A2T(strLine.GetBuffer()));
...
BOOL CCrystalTextBuffer::SaveToFile ( LPCTSTR pszFileName, int nCrlfStyle /*= CRLF_STYLE_AUTOMATIC*/, BOOL bClearModifiedFlag /*= TRUE*/ )
{
	...
	__try
	{
		...
		for (int nLine = 0; nLine < nLineCount; nLine ++)
		{
			...
			if (nLength > 0)
			{
				char *szEncode = (char*)_alloca ( nLength+1 );
				szEncode[nLength] = '\0';
				strcpy ( szEncode, (m_aLines[nLine].m_pcLine) );

				compbyte::encode ( (BYTE*)szEncode, nLength );

				if (! ::WriteFile(hTempFile, szEncode, nLength, &dwWrittenBytes, NULL))
					__leave;
			...
			{
				char *szEncode = (char*)_alloca ( nCRLFLength+1 );
				szEncode[nCRLFLength] = '\0';
				strcpy ( szEncode, pszCRLF );

				compbyte::encode ( (BYTE*)szEncode, nCRLFLength );

				if (! ::WriteFile(hTempFile, szEncode, nCRLFLength, &dwWrittenBytes, NULL))
					__leave;
