...
void CCrystalTextView::MoveWordLeft(BOOL bSelect)
{
	...
	while (nPos > 0 && _ismbcspace(pszChars[nPos - 1]))
		nPos --;
	...
	if (nPos > 0)
	{
		...
		else
		{
			while (nPos > 0 && ! isalnum(pszChars[nPos - 1])
						&& pszChars[nPos - 1] != _T('_') && ! _ismbcspace(pszChars[nPos - 1]))
				nPos --;
		}
	}
...
void CCrystalTextView::MoveWordRight(BOOL bSelect)
{
	...
	else
	{
		while (nPos < nLength && ! isalnum(pszChars[nPos])
						&& pszChars[nPos] != _T('_') && ! _ismbcspace(pszChars[nPos]))
			nPos ++;
	}

	while (nPos < nLength && _ismbcspace(pszChars[nPos]))
		nPos ++;
...
void CCrystalTextView::MoveHome(BOOL bSelect)
{
	...
	LPCTSTR pszChars = GetLineChars(m_ptCursorPos.y);
	int nHomePos = 0;
	while (nHomePos < nLength && _ismbcspace(pszChars[nHomePos]))
		nHomePos ++;
