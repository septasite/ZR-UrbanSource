...
IMPLEMENT_DYNAMIC(CRanManagerTab, CPropertySheet)

CRanManagerTab::CRanManagerTab(CRect& rect, LOGFONT logfont, CWnd* pParentWnd)
	 ...
	 m_pFont ( NULL )
{
	...
	m_PageMain.SetToolTab ( this );
	AddPage ( &m_PageMain );
	m_PageUser.SetToolTab ( this );
	AddPage ( &m_PageUser );
	m_PageChar.SetToolTab ( this );
	AddPage ( &m_PageChar );
	m_PageGuild.SetToolTab ( this );
	AddPage ( &m_PageGuild );
	m_PageVehicle.SetToolTab ( this );
	AddPage ( &m_PageVehicle );
	m_PagePet.SetToolTab ( this );
	AddPage ( &m_PagePet );
...
BOOL CRanManagerTab::OnInitDialog() 
{
	HWND hTabWnd;
	CRect rectTabCtrl;
...
BEGIN_MESSAGE_MAP(CRanManagerTab, CPropertySheet)
	//{{AFX_MSG_MAP(CRanManagerTab)
	ON_WM_CTLCOLOR_REFLECT()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()


void CRanManagerTab::ActivePage(int nPage)
{
	switch (nPage)
	{
	case MAINPAGE_MAIN:	SetActivePage ( MAINPAGE_MAIN ); break;
	case MAINPAGE_USER:	SetActivePage ( MAINPAGE_USER ); break;
	case MAINPAGE_CHAR:	SetActivePage ( MAINPAGE_CHAR ); break;
	case MAINPAGE_GUILD:	SetActivePage ( MAINPAGE_GUILD ); break;
	case MAINPAGE_VEHICLE:	SetActivePage ( MAINPAGE_VEHICLE ); break;
	case MAINPAGE_PET:	SetActivePage ( MAINPAGE_PET ); break;
	};
}

HBRUSH CRanManagerTab::CtlColor(CDC* pDc, UINT uCtlColor) 
{
...
BOOL CRanManagerTab::PreTranslateMessage(MSG* pMsg)
{
	switch ( pMsg->message )
	{
	case WM_KEYDOWN:
...
void CRanManagerTab::ChangeDialogFont(CWnd* pWnd, CFont* pFont, int nFlag)
{
	CRect windowRect;
