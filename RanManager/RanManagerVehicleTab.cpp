...
IMPLEMENT_DYNAMIC(CRanManagerVehicleTab, CPropertySheet)

CRanManagerVehicleTab::CRanManagerVehicleTab(CRect& rect, LOGFONT logfont, CWnd* pParentWnd)
	 : CPropertySheet("jdev", pParentWnd),
	 m_bNoTabs(FALSE),
	 m_pWndParent(NULL),
	 m_iDistanceFromTop(0),

	m_VehicleDefault( logfont ),
	m_VehicleEdit( logfont ),
	m_VehicleInven( logfont ),
	 ...
	 m_pFont ( NULL )
{
	...
	m_VehicleDefault.SetToolTab ( this );
	AddPage ( &m_VehicleDefault );
	m_VehicleEdit.SetToolTab ( this );
	AddPage ( &m_VehicleEdit );
	m_VehicleInven.SetToolTab ( this );
	AddPage ( &m_VehicleInven );

	m_pFont = new CFont();
	m_pFont->CreateFontIndirect(&logfont);
}

CRanManagerVehicleTab::~CRanManagerVehicleTab()
{
	SAFE_DELETE( m_pFont );
}
...
BOOL CRanManagerVehicleTab::OnInitDialog() 
{
	HWND hTabWnd;
	CRect rectTabCtrl;
...
BEGIN_MESSAGE_MAP(CRanManagerVehicleTab, CPropertySheet)
	//{{AFX_MSG_MAP(CRanManagerVehicleTab)
	ON_WM_CTLCOLOR_REFLECT()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()


void CRanManagerVehicleTab::ActivePage(int nPage ,int nVehicle )
{
	switch (nPage)
	{
	case VEHICLEPAGE_MAIN:	
		{
			SetActivePage ( VEHICLEPAGE_MAIN );
			m_VehicleDefault.SetVehicle(nVehicle);
		}break;

	case VEHICLEPAGE_EDIT:	
		{
			SetActivePage ( VEHICLEPAGE_EDIT );
			m_VehicleEdit.SetVehicle(nVehicle);
		}break;

	case VEHICLEPAGE_INVEN:	
		{
			SetActivePage ( VEHICLEPAGE_INVEN );
			m_VehicleInven.SetVehicle(nVehicle);
		}break;
	};
}

HBRUSH CRanManagerVehicleTab::CtlColor(CDC* pDc, UINT uCtlColor) 
{
...
BOOL CRanManagerVehicleTab::PreTranslateMessage(MSG* pMsg)
{
	switch ( pMsg->message )
	{
	case WM_KEYDOWN:
...
void CRanManagerVehicleTab::ChangeDialogFont(CWnd* pWnd, CFont* pFont, int nFlag)
{
	CRect windowRect;
...
void CRanManagerVehicleTab::ClearData()
{
}
