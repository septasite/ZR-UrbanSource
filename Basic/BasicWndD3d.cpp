...
#if defined(MY_PARAM) || defined(PH_PARAM) //|| defined(KR_PARAM) /*|| defined(MYE_PARAM)*/ //|| defined ( VN_PARAM ) || defined(TH_PARAM) || defined ( HK_PARAM ) || defined(TW_PARAM) // Nprotect 정의
...
HRESULT CBasicWnd::CreateObjects()
{
	...
	hr = NLOADINGTHREAD::StartThreadLOAD ( &m_pd3dDevice, m_hWnd, ((CBasicApp*)AfxGetApp())->m_szAppPath, CString("loading_000.dds"), true );
...
HRESULT CBasicWnd::RenderText()
{
	...
	/*if ( DxGlobalStage::GetInstance().EM_STAGE_LOBY )
		{
	...
	if ( dxincommand::bDISP_FPS )
	{
			...
			fNextLine -= 20.0f;
			pD3dFont->DrawText( 440, 10, fontColor, szMsg );

			StringCchCopy( szMsg, MAX_PATH, m_strFrameStats );
			fNextLine -= 20.0f;
			pD3dFont->DrawText( 440, 20, fontColor, szMsg );
	}
