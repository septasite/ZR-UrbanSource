...
CCTFRankPlayerSG::CCTFRankPlayerSG () :
	m_pBase( NULL )
	, m_pScrollBar(NULL)
	, m_nStart( 0 )
	, m_nTotal( 0 )
...
void CCTFRankPlayerSG::CreateSubControl ()
{
...
void CCTFRankPlayerSG::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	if ( !IsVisible () ) return ;

	CUIGroup::Update ( x, y, LB, MB, RB, nScroll, fElapsedTime, bFirstControl );

	m_nTotal = (int)GLGaeaClient::GetInstance().m_CaptureTheFieldRankingPlayerSGVec.size();
...
void CCTFRankPlayerSG::RefreshRanking()
{
	m_nTotal = (int)GLGaeaClient::GetInstance().m_CaptureTheFieldRankingPlayerSGVec.size();
...
void CCTFRankPlayerSG::RenderView()
{
	for ( int i=0; i < CTF_PLAYER_SLOT_MAX; ++ i )
	{
		if ( m_pSlot[i] )
	...
	CTF_RANK_PLAYER_CLIENT_VEC& vecRank = GLGaeaClient::GetInstance().m_CaptureTheFieldRankingPlayerSGVec;
...
void CCTFRankPlayerSG::TranslateUIMessage ( UIGUID ControlID, DWORD dwMsg )
{
	CUIGroup::TranslateUIMessage ( ControlID, dwMsg );
}

void CCTFRankPlayerSG::SetVisibleSingle ( BOOL bVisible )
{
