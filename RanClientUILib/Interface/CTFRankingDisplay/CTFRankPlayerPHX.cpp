...
CCTFRankPlayerPHX::CCTFRankPlayerPHX () :
	m_pBase( NULL )
	, m_pScrollBar(NULL)
	, m_nStart( 0 )
	, m_nTotal( 0 )
...
void CCTFRankPlayerPHX::CreateSubControl ()
{
...
void CCTFRankPlayerPHX::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	if ( !IsVisible () ) return ;

	CUIGroup::Update ( x, y, LB, MB, RB, nScroll, fElapsedTime, bFirstControl );

	m_nTotal = (int)GLGaeaClient::GetInstance().m_CaptureTheFieldRankingPlayerPHXVec.size();
...
void CCTFRankPlayerPHX::RefreshRanking()
{
	m_nTotal = (int)GLGaeaClient::GetInstance().m_CaptureTheFieldRankingPlayerPHXVec.size();
...
void CCTFRankPlayerPHX::RenderView()
{
	for ( int i=0; i < CTF_PLAYER_SLOT_MAX; ++ i )
	{
		if ( m_pSlot[i] )
	...
	CTF_RANK_PLAYER_CLIENT_VEC& vecRank = GLGaeaClient::GetInstance().m_CaptureTheFieldRankingPlayerPHXVec;
...
void CCTFRankPlayerPHX::TranslateUIMessage ( UIGUID ControlID, DWORD dwMsg )
{
	CUIGroup::TranslateUIMessage ( ControlID, dwMsg );
}

void CCTFRankPlayerPHX::SetVisibleSingle ( BOOL bVisible )
{
