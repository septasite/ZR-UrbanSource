...
void CCTFRankPlayer::CreateSubControl ()
{
...
void CCTFRankPlayer::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	if ( !IsVisible () ) return ;

	CUIGroup::Update ( x, y, LB, MB, RB, nScroll, fElapsedTime, bFirstControl );

	m_nTotal = (int)GLGaeaClient::GetInstance().m_CaptureTheFieldRankingPlayerAllVec.size();
...
void CCTFRankPlayer::RefreshRanking()
{
	m_nTotal = (int)GLGaeaClient::GetInstance().m_CaptureTheFieldRankingPlayerAllVec.size();
...
void CCTFRankPlayer::RenderView()
{
	for ( int i=0; i < CTF_PLAYER_SLOT_MAX; ++ i )
	{
		if ( m_pSlot[i] )
	...
	CTF_RANK_PLAYER_CLIENT_VEC& vecRank = GLGaeaClient::GetInstance().m_CaptureTheFieldRankingPlayerAllVec;
...
void CCTFRankPlayer::TranslateUIMessage ( UIGUID ControlID, DWORD dwMsg )
{
	CUIGroup::TranslateUIMessage ( ControlID, dwMsg );
}

void CCTFRankPlayer::SetVisibleSingle ( BOOL bVisible )
{
