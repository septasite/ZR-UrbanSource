...
{
	m_pFont8 = DxFontMan::GetInstance().LoadDxFont ( _DEFAULT_FONT, 8, TRUE, D3DFONT_SHADOW | D3DFONT_ASCII );
...
// #endif
	...
	CBasicButton* pLockButton = new CBasicButton;
...
void CBasicPotionTray::TranslateUIMessage ( UIGUID ControlID, DWORD dwMsg )
{
	switch ( ControlID )
	{
	case QUICK_POTION_TRAY_LOCK_BUTTON_LOCK_H:
		{
					...
					if ( sItemCustom.sNativeID != NATIVEID_NULL () )	//	설정
					{
						if(bPotionLock)
						{
							CInnerInterface::GetInstance().PrintMsgText ( 
										NS_UITEXTCOLOR::LIGHTSKYBLUE, ID2GAMEINTEXT("LOCK_POTION_MESSAGE_PRESS") );
							return;
						}
