...
//#endif
	...
	pRegisterButton->CreateSub ( this, "BASIC_TEXT_BUTTON18", UI_FLAG_XSIZE, LOGIN_REGISTER );
	pRegisterButton->CreateBaseButton ( "LOGIN_PAGE_REGISTER", nBUTTONSIZE, CBasicButton::CLICK_FLIP, (char*)ID2GAMEWORD ( "LOGIN_PAGE_OKCANCEL", 2 ) );
	RegisterControl ( pRegisterButton );

	
	CBasicTextButton* pForgetPassButton = new CBasicTextButton;
	pForgetPassButton->CreateSub ( this, "BASIC_TEXT_BUTTON18", UI_FLAG_XSIZE, LOGIN_RESETPASS );
	pForgetPassButton->CreateBaseButton ( "LOGIN_PAGE_RESETPASS", nBUTTONSIZE, CBasicButton::CLICK_FLIP, (char*)ID2GAMEWORD ( "LOGIN_PAGE_OKCANCEL", 3 ) );
	RegisterControl ( pForgetPassButton );
...
//		Japan 로그인 방식 변경
				...
				default:
					pNetClient->ChinaSndLogin( strID, strPW, strRP, nServerChannel ); //Md5ed - Seifer13		
					break;
				};
	...
	case LOGIN_RESETPASS:
		{
			if ( CHECK_KEYFOCUSED ( dwMsg ) || CHECK_MOUSEIN_LBUPLIKE ( dwMsg ) )
			{
				...
				if( pNetClient->IsOnline() == true )
				{
					COuterInterface::GetInstance().ToRPSPage ( GetWndID () );
				}
