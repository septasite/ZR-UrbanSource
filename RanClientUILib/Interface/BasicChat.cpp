...
{
	CD3DFontPar* pFont9 = DxFontMan::GetInstance().LoadDxFont ( _DEFAULT_FONT, 9, FALSE, _DEFAULT_FONT_FLAG );
//
...
void CBasicChat::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	...
	case CHAT_REGION:	m_pBasicChatLeftBar->DO_CHAT_STATE( 6 ); break; //add region chat by AnFire16 ( 7/31/2017)
	}
	...
	case CHAT_SHOW_FLAG:
		{
			...
			if ( UIMSG_CHAT_REGION   & dwMsg ) m_pBasicChatRightBody->AddChatFlagState ( CHAT_REGION ); //add region chat by AnFire16 ( 7/31/2017)
...
/*				AddMessageEx ( UIMSG_MOUSEIN_BLOCK_CHARACTER_MOVE );
				...
				if ( UIMSG_MOUSEIN_REGION_CHAT & dwMsg )
				{
					BEGIN_REGION_CHAT();
					return ;
				}
...
//add region chat by AnFire16 ( 7/31/2017)
void	CBasicChat::BEGIN_REGION_CHAT ( const CString& strMessage )
{
	m_pBasicChatRightBody->SetIgnoreState ( true );
	m_pBasicChatRightBody->BEGIN_REGION_CHAT ( strMessage );
	m_pBasicChatRightBody->SetIgnoreState ( false );
}
