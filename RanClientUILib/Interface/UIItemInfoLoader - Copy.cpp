		...
		/*if ( sNpcNativeID != sItemCustom.sNativeID ) 
			{
				...
				if ( sNpcNativeID.IsValidNativeID() )
				{
					...
					if ( pItem->sBasicOp.emItemType == ITEM_BOX  )
					{
						...
						//AddTextNoSplit ( "Alt + Left Click to Show Item Preview in your character. Items inside here aren't randomized", NS_UITEXTCOLOR::WHITE );
					}
					...
					else //if ( pItem->sBasicOp.emItemType != ITEM_RANDOMITEM && pItem->sBasicOp.emItemType != ITEM_BOX )
					{
						ResetItemRender ();	
						ResetItemBoxRender ();
						//ResetItemBoxRandomRender();
