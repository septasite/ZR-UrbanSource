...
const CString CBasicChatRightBody::REGION_SYMBOL( _T("^") ); //add region chat by AnFire16 ( 7/31/2017)
...
void CBasicChatRightBody::CreateSubControl ( CD3DFontPar*	pFont )
{
	...
	{	//add region chat by AnFire16 ( 7/31/2017)
		m_pTextBox_REGION = CreateTextBoxEx ( pFont, CHAT_TEXTBOX_REGION );
		nTotalLine = m_pTextBox_REGION->GetTotalLine ();
		nVisibleLine = m_pTextBox_REGION->GetVisibleLine ();

		m_pScrollBar_REGION = CreateScrollBarEx ( nTotalLine, nVisibleLine );
	}
...
CBasicScrollBarEx*	CBasicChatRightBody::CreateScrollBarEx ( const int& nTotalLine, const int& nVisibleLine )
{
	...
	pScrollBar->CreateSub ( this, "BASIC_NEW_SCROLLBAR", UI_FLAG_RIGHT | UI_FLAG_YSIZE );
	pScrollBar->CreateBaseScrollBarBlack ( "CHAT_NEW_SCROLLBAR" );
...
//	if ( bFirstControl )
	{
		...
		{	
			...
			else if ( m_pTextBox_REGION->IsVisible () )		UPDATE_CHAT_PAGE_TEXTBOX ( m_pTextBox_REGION, m_pScrollBar_REGION ); //add region chat by AnFire16 ( 7/31/2017)
		...
		case CHAT_PRIVATE:
			{
				...
				if( nEditLength == 1 ) // 두번 출력되는 것을 막기 위해 임시로 넣은 코드이다. ㅜㅜ
				{
					...
					AddStringToNORMAL ( strCombine, GLCONST_CHAR::dwCHAT_PRIVATE_COLOR, NULL  );
				}
			}
			break;
		...
		case CHAT_PARTY:
			{
				...
				AddStringToNORMAL ( strCombine, GLCONST_CHAR::dwCHAT_PARTY_COLOR, NULL  );
			}
			break;
		...
		case CHAT_TOALL:
			{
				...
				AddStringToNORMAL ( strCombine, GLCONST_CHAR::dwCHAT_TOALL_COLOR, NULL  );
			}
			break;
		...
		case CHAT_GUILD:
			{
				...
				AddStringToNORMAL ( strCombine, GLCONST_CHAR::dwCHAT_GUILD_COLOR, NULL  );
			}
			break;
		...
		case CHAT_REGION: 
			{
				...
				strCombine.Format ( "[%s]%s",ID2GAMEWORD("CHAT_OPTION",8), ID2GAMEINTEXT("CHAT_REGION_MODE") );
				AddStringToNORMAL ( strCombine, NS_UITEXTCOLOR::DODGERBLUE, NULL  );
			}
			break;
		...
		case CHAT_ALLIANCE:
			{
				...
				AddStringToNORMAL ( strCombine, GLCONST_CHAR::dwCHAT_ALLIANCE_COLOR, NULL  );
			}
			break;
...
void	CBasicChatRightBody::ChangeChatType ()
{
	...
	if ( UIKeyCheck::GetInstance()->Check ( DIK_TAB, DXKEY_DOWN ) )
	{
		...
		case CHAT_ALLIANCE:	BEGIN_REGION_CHAT();	break; //add region chat by AnFire16 ( 7/31/2017)
		case CHAT_REGION:	BEGIN_NORMAL_CHAT();	break;
		}			
	}	
}
...
{	
	...
	if( nEditLength )
	{
		...
		else if( FIRST_CHAR == REGION_SYMBOL )		m_nCHATTYPE = CHAT_REGION; //add region chat by AnFire16 ( 7/31/2017)
	}
}
...
//	m_pSystemMessageDisplay->SetVisibleSingle ( TRUE );
	...
	//add region chat by AnFire16 ( 7/31/2017)
	m_pTextBox_REGION->SetVisibleSingle ( FALSE );
	m_pScrollBar_REGION->SetVisibleSingle ( FALSE );
	...
	//m_pChatRegionButton->SetFlip ( FALSE );
...
void CBasicChatRightBody::AddChatMacro(CString strMsg)
{
	...
	else if( ChatType == REGION_SYMBOL )	wType = CHAT_REGION; //add region chat by AnFire16 ( 7/31/2017)
...
void CBasicChatRightBody::SetVisibleChatButton( BOOL bVisible )
{
	...
	//m_pChatRegionButton->SetVisibleSingle( bVisible );
