#include "StdAfx.h"
#include "TowerCapturedDisplay.h"
...
CTowerCapturedDisplay::CTowerCapturedDisplay ()
{
}
...
void CTowerCapturedDisplay::CreateSubControl ()
{
	CString strKeyword[MAXTYPE] = 
	{
		"TOWER_CAPTURED_THERMAL_TOWER_SG",
	...
	{
		m_pQI_TYPE[i] = new CTowerCapturedType;
		m_pQI_TYPE[i]->CreateSub ( this, strKeyword[i].GetString(), UI_FLAG_DEFAULT, TOWER_CAPTURED_THERMAL_TOWER_SG + i );
...
bool	CTowerCapturedDisplay::START ( UIGUID cID )
{
	if ( cID < TOWER_CAPTURED_THERMAL_TOWER_SG || TOWER_CAPTURED_THERMAL_TOWER_PHX < cID ) return false;
	
	int nIndex = cID - TOWER_CAPTURED_THERMAL_TOWER_SG;
...
bool	CTowerCapturedDisplay::RESET ( UIGUID cID )
{
	if ( cID < TOWER_CAPTURED_THERMAL_TOWER_SG || TOWER_CAPTURED_THERMAL_TOWER_PHX < cID ) return false;
...
bool	CTowerCapturedDisplay::STOP ( UIGUID cID )
{
	if ( cID < TOWER_CAPTURED_THERMAL_TOWER_SG || TOWER_CAPTURED_THERMAL_TOWER_PHX < cID ) return false;
...
bool CTowerCapturedDisplay::START_FADEINOUT( UIGUID cID )
{
	if ( cID < TOWER_CAPTURED_THERMAL_TOWER_SG || TOWER_CAPTURED_THERMAL_TOWER_PHX < cID ) return false;
...
void CTowerCapturedDisplay::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	if ( !IsVisible () ) return ;
	...
	for ( int i = 0; i < MAXTYPE; ++i )
	{
		...
		{
			STOP ( TOWER_CAPTURED_THERMAL_TOWER_SG + i );
		}
...
void CTowerCapturedDisplay::ALLSTOP ()
{
	for ( int i = 0; i < MAXTYPE; ++i )
	{
		m_pQI_TYPE[i]->STOP ();
...
bool CTowerCapturedDisplay::KEEP_START ( UIGUID cID )
{
	if ( cID < TOWER_CAPTURED_THERMAL_TOWER_SG ||  TOWER_CAPTURED_THERMAL_TOWER_PHX < cID ) return false;
...
void CTowerCapturedDisplay::KEEP_STOP ()
{
	for ( int i = 0; i < MAXTYPE; ++i )
	{
		m_QI_TYPE_KEEP[i]->SetVisibleSingle ( FALSE );
