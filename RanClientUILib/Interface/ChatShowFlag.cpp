...
CUIChatShowFlag::CUIChatShowFlag ()
	...
	, m_pPartyButton(NULL)
	, m_pClubButton(NULL)
	, m_pRegionButton(NULL) //add region chat by AnFire16 ( 7/31/2017)
	...
	, m_pClubTextBox(NULL)
	, m_pRegionTextBox(NULL) //add region chat by AnFire16 ( 7/31/2017)
...
//		pBasicLineBox->ResetAlignFlagLine ();
	...
	m_pRegionButton = CreateButton ( "CHAT_REGION_CHECK", "CHAT_REGION_CHECK_F", CHAT_REGION_CHECK );
	m_pRegionButton->SetFlip ( TRUE );
	m_pRegionTextBox = CreateTextBox ( "CHAT_REGION_CHECK_TEXT", ID2GAMEWORD("CHAT_ONOFF",6), CHAT_REGION_TEXTBOX );
...
void CUIChatShowFlag::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	...
	//	고치기전까지의 사용으로 임시적용
	{
		...
		m_pRegionButton->SetVisibleSingle ( TRUE ); //add region chat by AnFire16 ( 7/31/2017)
	...
	if ( m_pRegionButton->IsFlip() )	AddMessageEx ( UIMSG_CHAT_REGION ); //add region chat by AnFire16 ( 7/31/2017)
...
void CUIChatShowFlag::TranslateUIMessage ( UIGUID cID, DWORD dwMsg )
{
	...
	case CHAT_CLUB_CHECK:
		...
		//add region chat by AnFire16 ( 7/31/2017)
	case CHAT_REGION_TEXTBOX:
	case CHAT_REGION_CHECK:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				if ( UIMSG_LB_DOWN & dwMsg )
				{
					BOOL bFlip = !m_pRegionButton->IsFlip();
					m_pRegionButton->SetFlip ( bFlip );

					D3DCOLOR dwColor = NS_UITEXTCOLOR::DARKGRAY;
					if ( bFlip ) dwColor = NS_UITEXTCOLOR::WHITESMOKE;					

					m_pRegionTextBox->SetUseTextColor ( 0, TRUE );
					m_pRegionTextBox->SetTextColor ( 0, dwColor );
				}
			}
		}
		break;
