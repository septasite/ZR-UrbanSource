...
#include "CSwRankingDisplay.h"		//add school wars
...
#include "BoxItemInfo.h" //Box Info by NjD
...
//Add random char string by AnFire16 (Made specially for royal rumble)
...
void CInnerInterface::SetTargetInfo ( STARGETID sTargetID, int wSchool )
{
	CString strName;
	...
	else if ( pCOPY->GetCrow () == CROW_PC )
	{
		...
		//juver battle royale
		bool bRedZone = false;
		//bool bDomiWars = false;
		bool bRoyalRumble = false;
	
		PLANDMANCLIENT pLand = GLGaeaClient::GetInstance().GetActiveMap();
		if ( pLand )
		{
			bGuidBattleMap = ( pLand->m_bClubBattle || pLand->m_bClubDeathMatch || pLand->m_bSchoolWars );

			bRedZone = ( pLand->IsRedZone() );
			bRoyalRumble = ( pLand->IsRoyalRumble() );
		}
			...
			!GLGaeaClient::GetInstance().GetCharacter()->IS_PLAYHOSTILE ( pCOPY->GetCharID() ) )
		{
			...
			if ( !(bBRIGHTEVENT || bSCHOOL_FREEPK || bRedZone || bRoyalRumble ) )
			{
				sHP.TO_FULL ();
			}
		}
		...
		int fPercent3 = (int) (float(sHP.dwNow) / float(sHP.dwMax)) * 100;
	...
	{
		if ( m_pTargetInfoDisplay ) m_pTargetInfoDisplay->SetTargetInfo ( fPercent, strName, dwColor, sHP.dwMax );
	}
	...
	//		포지션 설정
	{
		...
		if ( pCrow && ( pCrow->m_pCrowData->m_sAction.m_dwActFlag&EMCROWACT_CTFTOWER ) ) 	ShowGroupBottom ( TARGETINFO_CTF_DISPLAY );
...
//		strGuildMaster = "GuildMaster";
	...
	//		??? ??
	{
		...
		//	?? ?? ???
		if ( m_pNameDisplayMan )	m_pNameDisplayMan->VisibleTargetInfoCtrl ( sTargetID.emCrow, sTargetID.dwID, TRUE );
	}
}
...
//		strGuildMaster = "GuildMaster";
	...
	if ( pCOPY->GetCrow() == CROW_PC )
	{
		...
		PGLCROWCLIENT pCrow = GLGaeaClient::GetInstance().GetActiveMap()->GetCrow ( sTargetID.dwID );
		if ( !pCrow ) return;
	int fPercent =   (int)(float(sHP.dwNow) / float(sHP.dwMax)) * 100;		
	if ( m_pCrowTargetInfoPlayer ) m_pCrowTargetInfoPlayer->SetTargetInfo ( sHP.dwNow, sHP.dwMax, fPercent3,pCHAR->GetSchool(), strCombine, pCHAR->GetLevel(),strPartyMember, strPartyM, strPartyNew, strPartyJoined, strParty, strGuild, strName, sTargetID );
...
void CInnerInterface::SetMarketWindowOpen ( SNATIVEID sNativeID )
{
	if ( m_pDialogueWindow )
	{
		...
		m_pDialogueWindow->SetVisible ( 0 ); //Open Page #0 when click trade items -- Ocram Networkso
...
void CInnerInterface::RESET_INFO ()
{
	...
	m_sItemInfoTemp = SITEMCUSTOM();
	//if( m_pInfoDisplay ) m_pInfoDisplay->ResetItemRender ();
}/*
...
void CInnerInterface::ADDITEM_RENDER ( SNATIVEID sICONINDEX, const char* szTexture )
{
	if ( m_bITEM_INFO_LINK_DISPLAY_MODE )
	{
		m_pInfoDisplayLink->SetItemRender ( sICONINDEX, szTexture );
	}
	else if ( m_bITEM_INFO_EX_DISPLAY_MODE )
	{
...
void CInnerInterface::RESETITEM_RENDER ()
{
	if ( m_bITEM_INFO_LINK_DISPLAY_MODE )
	{
		m_pInfoDisplayLink->ResetItemRender ();
	}
	else if ( m_bITEM_INFO_EX_DISPLAY_MODE )
	{
...
void CInnerInterface::RESETNAME_RENDER ()
{
	if ( m_bITEM_INFO_LINK_DISPLAY_MODE )
	{
		//m_pInfoDisplayEx->ClearText ();
...
void CInnerInterface::RESET_INFO_BOXINFO ()
{
	NS_ITEMINFO_BOXINFO::RESET ();
	m_strGeneralInfoBack.Empty ();
	if( m_pBoxInfo ) m_pBoxInfo->ClearText ();
}
void CInnerInterface::SHOW_BOXINFO ( SITEMCUSTOM &sItemCustom, BOOL bShopOpen, BOOL bInMarket, BOOL bInPrivateMarket, WORD wPosX, WORD wPosY, SNATIVEID sNpcNativeID /* = SNATIVEID */ )
{	

	if ( IsMouseInControl () ) return ;

	if ( IsVisibleGroup ( BOXINFO_DISPLAY ) ) return ;
	if ( m_etInfoType != ET_ITEM_INFO )
	{
		RESET_INFO_BOXINFO ();
		m_etInfoType = ET_ITEM_INFO;
	...
	NS_ITEMINFO_BOXINFO::LOAD ( sItemCustom, bShopOpen, bInMarket, bInPrivateMarket, FALSE, wPosX, wPosY, sNpcNativeID );
	m_sItemInfoTemp = sItemCustom;
	ShowGroupFocus ( BOXINFO_DISPLAY );
...
void CInnerInterface::SHOW_SKILL_INFO ( SNATIVEID sNativeID, const BOOL bNextLevel )
{
	...
	for( unsigned s=0;s<SKILL::SAPPLY::MAX_DESCRIPTOR;++s)
	{
		...
		if( desc.emSPEC == EMSPECA_LINKSKILL)
		{
			SNATIVEID sNativeID = desc.sSPEC[ 0 ].dwNativeID;
			m_bITEM_INFO_EX_DISPLAY_MODE = true;
			NS_SKILLINFO::LOAD ( sNativeID, false, true );
			m_bITEM_INFO_EX_DISPLAY_MODE = false;
			ShowGroupTop ( INFO_DISPLAY_EX );
...
void CInnerInterface::OpenItemMixWindowButton()
{
	if( !m_pItemMixWindow || !m_pItemMixInvenWindow )
		return;
...
void CInnerInterface::CloseItemMixWindow()
{
	...
	//m_pItemMixWindow->ResetItemMixWindow(); //Reset ItemMix JuanArturooo
...
bool CInnerInterface::SET_PK_COMBO_ID ( int nID )
{
	UIGUID cID = NO_ID;
	switch ( nID )
	{
	case PK_FIRSTBLOOD:			cID = CPKComboDisplay::FIRSTBLOOD;		break;
	case PK_DOUBLEKILL:			cID = CPKComboDisplay::DOUBLEKILL;		break;
	case PK_TRIPLEKILL:			cID = CPKComboDisplay::TRIPLEKILL;		break;
	case PK_ULTRAKILL:			cID = CPKComboDisplay::ULTRAKILL;		break;
	case PK_RAMPAGE:			cID = CPKComboDisplay::RAMPAGE;			break;
	case PK_GODLIKE:			cID = CPKComboDisplay::GODLIKE;			break;
	}
...
void	CInnerInterface::RESET_KEEP_PK_COMBO ()
{
	if( m_pPKComboDisplay )
	{
		m_pPKComboDisplay->KEEP_STOP ();
	}
	ShowGroupTop( PK_COMBO_DISPLAY );
}
...
HRESULT CInnerInterface::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
...

	{
		...
		m_pKeySettingWindow->CreateBaseWindowBlackNew ( "KEY_SETTING_WINDOW", (char*)ID2GAMEWORD("KEYSETTING_WINDOW_NAME_STATIC") );
...

	{
		...
		m_pChatMacroWindow->CreateBaseWindowBlackNew ( "CHATMACRO_WINDOW", (char*)ID2GAMEWORD("CHATMACRO_WINDOW_NAME_STATIC") );
	...
	//add charinfoview
	{
		...
		m_pCharacterViewPropertiesWindow->Create ( VIEW_PROPERTIES_WINDOW, "BASIC_WINDOW", UI_FLAG_XSIZE | UI_FLAG_YSIZE );
		m_pCharacterViewPropertiesWindow->CreateBaseWindowBlackNew ( "2014_VIEWCHARACTER_WINDOW", (char*)ID2GAMEWORD("VIEW_CHARACTER_NAME_STATIC") );
	...
	{
		m_pkillTimeDisplay = new CkillTimeDisplay;	
		m_pkillTimeDisplay->Create ( KILL_TIME_DISPLAY, "KILL_TIME_DISPLAY", UI_FLAG_RIGHT | UI_FLAG_TOP );
		m_pkillTimeDisplay->CreateSubControl ();
		RegisterControl ( m_pkillTimeDisplay );
		ShowGroupTop ( KILL_TIME_DISPLAY );
	}
	...
	{
		CCompundCompete* pItemCompund = new CCompundCompete;
		pItemCompund->Create ( ITEM_COMPOUND, "ITEM_COMPOUND_BUTTON", UI_FLAG_RIGHT | UI_FLAG_BOTTOM  );
		pItemCompund->CreateSubControl ();
		RegisterControl ( pItemCompund );
...
//#endif
		...
		//add school wars
		CSwRankingDisplay* pSwRankingDisplay = new CSwRankingDisplay;
		pSwRankingDisplay->Create ( SW_RANKING_DISPLAY, "SW_RANKING_DISPLAY", UI_FLAG_RIGHT );
		pSwRankingDisplay->CreateSubControl ();
		RegisterControl ( pSwRankingDisplay );
		ShowGroupBottom ( SW_RANKING_DISPLAY );	
		HideGroup( SW_RANKING_DISPLAY );

		//add rr
		CRRRankingDisplay* pRRRankingDisplay = new CRRRankingDisplay;
		pRRRankingDisplay->Create ( RRP_RANKING_DISPLAY, "RRP_RANKING_DISPLAY", UI_FLAG_RIGHT );
		pRRRankingDisplay->CreateSubControl ();
		RegisterControl ( pRRRankingDisplay );
		ShowGroupBottom ( RRP_RANKING_DISPLAY );	
		HideGroup( RRP_RANKING_DISPLAY );

		/*//add rr
		CCtfTopTenRank* pCtfTopTenRank = new CCtfTopTenRank;
		pCtfTopTenRank->Create ( TOPTEN_CTF_RANK_WINDOW, "TOPTEN_CTF_RANK_WINDOW", UI_FLAG_RIGHT );
		pCtfTopTenRank->CreateSubControl ();
		RegisterControl ( pCtfTopTenRank );
		ShowGroupBottom ( TOPTEN_CTF_RANK_WINDOW );	
		HideGroup( TOPTEN_CTF_RANK_WINDOW );*/
		...
		/*m_pInBoxIcon = new CInBoxIcon;
		m_pInBoxIcon->Create ( POSTNOTIFY_BUTTON, "POSTNOTIFY_BUTTON", UI_FLAG_RIGHT | UI_FLAG_BOTTOM );
		m_pInBoxIcon->CreateSubControl ();
		RegisterControl ( m_pInBoxIcon );		
		ShowGroupBottom ( POSTNOTIFY_BUTTON );*/

		m_pAtteIcon = new CAtteIcon;
		m_pAtteIcon->Create ( STUDENTRECORD_NOTIFY_BUTTON, "STUDENTRECORD_NOTIFY_BUTTON", UI_FLAG_RIGHT | UI_FLAG_BOTTOM );
		m_pAtteIcon->CreateSubControl ();
		RegisterControl ( m_pAtteIcon );		
		ShowGroupBottom ( STUDENTRECORD_NOTIFY_BUTTON );

		m_pSWarIcon = new CSWarIcon;
		m_pSWarIcon->Create ( COMPETITION_NOTIFY_BUTTON, "COMPETITION_NOTIFY_BUTTON", UI_FLAG_RIGHT | UI_FLAG_BOTTOM );
		m_pSWarIcon->CreateSubControl ();
...
// #ifndef CH_PARAM // 중국 인터페이스 변경
		...
		RegisterControl ( pButton , true );
		ShowGroupFocus ( QUICK_SKILL_TRAY_OPEN_BUTTON );
...
//#endif
...

	{
		...
		m_pCharacterWindow->CreateBaseWindowBlackNew ( "CHARACTER_WINDOW_MAIN", (char*)ID2GAMEWORD("CHARACTER_NAME_STATIC") );
...

	{
		...
		pOptionWindow->CreateBaseWindowBlackNew ( "HWOPTION_WINDOW", (char*)ID2GAMEWORD("HWOPTION_WINDOW_NAME_STATIC") );
...

	{
		...
		pSkillWindow->CreateBaseWindowBlackNew ( "RNSKILL_WINDOW", (char*)ID2GAMEWORD("SKILL_NAME_STATIC") );
...

	{
		...
		m_pPartyWindow->CreateBaseWindowBlackNew ( "PARTY_WINDOW", (char*)ID2GAMEWORD("PARTY_NAME_STATIC") );
	...
	{	//	인벤토리
		...
		m_pInventoryWindow->CreateBaseWindowBlackNew ( "INVENTORY_WINDOW_RENEW", (char*)ID2GAMEWORD("INVENTORY_NAME_STATIC") );
	...
	{	//	거래
		...
		m_pTradeWindow->CreateBaseWindowBlackNew ( "TRADE_WINDOW", (char*)ID2GAMEWORD("TRADE_NAME_STATIC") );
		...
		m_pTradeInventoryWindow->CreateBaseWindowBlackNew ( "TRADEINVENTORY_WINDOW", (char*)ID2GAMEWORD("TRADEINVENTORY_NAME_STATIC") );
	...
	{	//	창고
		...
		m_pStorageWindow->CreateBaseWindowBlackNew ( "STORAGE_WINDOW", (char*)ID2GAMEWORD("STORAGE_NAME_STATIC") );
	...
	{	//	창고
		...
		m_pClubStorageWindow->CreateBaseWindowBlackNew ( "STORAGE_WINDOW", (char*)ID2GAMEWORD("CLUB_STORAGE_NAME_STATIC") );
	...
	{	//	미니 파티
		...
		pMiniPartyWindow->CreateBaseWindowBlackNew ( "MINIPARTY_WINDOW", (char*)ID2GAMEWORD("MINIPARTY_NAME_STATIC") );
...
//		m_pPtoPWindow->Create ( PTOPMENU_WINDOW, "PTOPMENU_WINDOW" );		
...

	{
		...
		m_pDialogueWindow->CreateBaseWindowBlackNew ( "DIALOGUE_WINDOW", (char*)ID2GAMEWORD("DIALOGUE_WINDOW_STATIC") );
	...
	{	//BOX INFO
		m_pBoxInfo = new CBoxItemInfo;
		m_pBoxInfo->Create ( BOXINFO_DISPLAY, "BOXINFO_DISPLAY", UI_FLAG_XSIZE | UI_FLAG_YSIZE );
		m_pBoxInfo->CreateSubControl ();
		m_pBoxInfo->SetVisibleSingle ( FALSE );
		RegisterControl ( m_pBoxInfo );
		ShowGroupTop ( BOXINFO_DISPLAY );
	}
...
//		m_pPrivateMarketSellWindow->LoadDefaultCondition ();
...

	{
		...
		if( CCommonWeb::Get()->GetCreate() )
		{
			...
			pHelpWindow->CreateBaseWindowBlackNew ( "HELP_WINDOW", (char*)ID2GAMEWORD("HELP_WINDOW_STATIC") );
	...
	{		
		...
		m_pQuestWindow->CreateBaseWindowBlackNew ( "QUEST_WINDOW_NEW", (char*)ID2GAMEWORD("QUEST_WINDOW_STATIC") );
...

	{
		...
		m_pFriendWindow->CreateBaseWindowBlackNew ( "FRIEND_WINDOW", (char*)ID2GAMEWORD("FRIEND_WINDOW_NAME_STATIC") );
		...
		m_pClubWindow->CreateBaseWindowBlackNew ( "CLUB_WINDOW", (char*)ID2GAMEWORD("CLUB_WINDOW_NAME_STATIC") );
...

	{
		...
		pReceiveNote->CreateBaseWindowBlackNew( "RECEIVE_NOTE_WINDOW", (char*)ID2GAMEWORD("FRIEND_TAB_BUTTON", 5) );
...

	{
		...
		pWirteNote->CreateBaseWindowBlackNew( "WRITE_NOTE_WINDOW", (char*)ID2GAMEWORD("FRIEND_TAB_BUTTON", 4) );
...
#if defined(KRT_PARAM)
	{
		...
		pSMSSendWindow->CreateBaseWindowBlackNew( "SMS_SEND_WINDOW", (char*)ID2GAMEWORD("FRIEND_BUTTON", 12) );
...

	{
		...
		pHairColorCard->CreateBaseWindowBlackNew( "HAIRCOLORCARD_WINDOW", NULL );
		...
		pPetColorCard->CreateBaseWindowBlackNew( "HAIRCOLORCARD_WINDOW", NULL );
		...
		pBikeColorCard->CreateBaseWindowBlackNew( "HAIRCOLORCARD_WINDOW", NULL );
		...
		//ShowGroupFocus( PETCOLORCARD_WINDOW ); //add bike color
	}
	
	{
		...
		pHairStyleCard->CreateBaseWindowBlackNew( "HAIRSTYLECARD_WINDOW", NULL );
		...
		pGenderChangeWindow->CreateBaseWindowBlackNew( "GENDER_CHANGE_WINDOW", NULL );
		...
		pPetStyleCard->CreateBaseWindowBlackNew( "HAIRSTYLECARD_WINDOW", NULL );
...

	{
		...
		m_pLargeMapWindow->CreateBaseWindowBlackNew ( "LARGEMAP_WINDOW", (char*)ID2GAMEWORD("LARGEMAP_WINDOW_STATIC") );
	...
	{	// ITEMREBUILD_MARK
		...
		m_pItemRebuildWindow->CreateBaseWindowBlackNew( "ITEM_REBUILD_WINDOW", NULL );
...

	{
		...
		m_pRebuildInventoryWindow->CreateBaseWindowBlackNew( "REBUILDINVENTORY_WINDOW", NULL );
	...
	{	// ITEMREBUILD_MARK
		...
		m_pItemGarbageWindow->CreateBaseWindowBlackNew( "ITEM_GARBAGE_WINDOW", NULL );
...

	{
		...
		m_pGarbageInventoryWindow->CreateBaseWindowBlackNew( "GARBAGEINVENTORY_WINDOW", NULL );
...

	{
		...
		m_pPetRebirthDialogue->CreateBaseWindowBlackNew ( "PET_REBIRTH_DIALOGUE", (char*)ID2GAMEWORD("PET_REBIRTH_NAME_STATIC") );
	...
	/*{
		...
		m_pGambleBox->CreateBaseWindowBlackNew ( "GAMBLE_DIALOGUE", (char*)ID2GAMEWORD("GAMBLE_NAME_STATIC") );
	...
	
	{
		...
		m_pGambleSelectBox->CreateBaseWindowBlackNew ( "GAMBLE_SELECT_DIALOGUE", (char*)ID2GAMEWORD("GAMBLE_NAME_STATIC") );
...

	{
		...
		m_pGambleAgainBox->CreateBaseWindowBlackNew ( "GAMBLE_AGAIN_DIALOGUE", (char*)ID2GAMEWORD("GAMBLE_NAME_STATIC") );
...

	{
		...
		m_pBusWindow->CreateBaseWindowBlackNew ( "BUS_WINDOW", (char*)ID2GAMEWORD("BUS_WINDOW_NAME_STATIC") );
		//m_pBusWindow->CreateBaseWindowBlackNew ( "BUS_WINDOW", (char*)ID2GAMEWORD("BUS_WINDOW_NAME_STATIC") );
	...
	{	//	유료아이템 수령
		...
		m_pItemShopWindow->CreateBaseWindowBlack ( "ITEMSHOP_WINDOW", (char*)ID2GAMEWORD("ITEMSHOP_NAME_STATIC") );
		m_pItemShopWindow->CreateSubControl ();
		m_pItemShopWindow->SetAlignFlag ( UI_FLAG_CENTER_Y | UI_FLAG_RIGHT );
	...
	{	//	베트남 탐닉 방지 인벤토리
		...
		m_pVNGainSysInventory->CreateBaseWindowBlackNew ( "VNGAINSYS_WINDOW", (char*)ID2GAMEWORD("VNGAINSYSTEM_NAME_STATIC") );
...
//		ShowGroupFocus ( VNGAINSYS_GAUGE_WINDOW );
	...
	{	
		...
		m_pPrivateMarketWindow->CreateBaseWindowBlackNew ( "PRIVATE_MARKET_WINDOW", (char*)ID2GAMEWORD("PRIVATE_MARKET_NAME_STATIC") );
...

	{
		...
		m_pRebirthDialogue->Create ( REBIRTH_DIALOGUE, "BASIC_WINDOW",  UI_FLAG_XSIZE | UI_FLAG_YSIZE | UI_FLAG_CENTER_X | UI_FLAG_CENTER_Y);
		m_pRebirthDialogue->CreateBaseWindowBlackNew ( "REBIRTH_DIALOGUE", (char*) "" );
...

	{
		...
		m_pCountMsgWindow->Create( COUNT_MSG_WINDOW, "COUNT_MSG_WINDOW", UI_FLAG_CENTER_X | UI_FLAG_TOP );
		m_pCountMsgWindow->CreateSubControl();
		RegisterControl( m_pCountMsgWindow );
	}

	{
		m_pCountMsgCDWindow = new CCountMsgCD;
		m_pCountMsgCDWindow->Create( COUNT_MSG_CD_WINDOW, "COUNT_MSG_CD_WINDOW", UI_FLAG_CENTER_X | UI_FLAG_TOP );
		m_pCountMsgCDWindow->CreateSubControl();
		RegisterControl( m_pCountMsgCDWindow );
	}
...

	{
		...
		m_pItemPreviewWindow->CreateBaseWindowBlackNew( "ITEMPREVIEW_WINDOW", NULL );
	...
	{	//	펫
		...
		m_pPetWindow->CreateBaseWindowBlackNew ( "PET_WINDOW", (char*)ID2GAMEWORD("PET_NAME_STATIC") );
		...
		pPetSkillWindow->CreateBaseWindowBlackNew ( "PET_SKILL_WINDOW", (char*)ID2GAMEWORD("PET_NAME_STATIC", 1) );
	...
	{	//  소환수
		...
		m_pSummonWindow->CreateBaseWindowBlackNew( "SUMMON_WINDOW", (char*)ID2GAMEWORD("SUMMON_WINDOW_STATIC",0 ) );
	...
	{	// 아이템 검색
		...
		m_pShopItemSearchWindow->CreateBaseWindowBlackNew ( "ITEM_SHOP_SEARCH_WINDOW", (char*)ID2GAMEWORD("ITEM_SHOP_SEARCH_STATIC",0 ) );
		...
		m_pItemSearchResultWindow->CreateBaseWindowBlackNew ( "ITEM_SEARCH_RESULT_WINDOW", (char*)ID2GAMEWORD("ITEM_SHOP_SEARCH_STATIC",0 ) );
...
//#endif 
	...
	////RankingWindow
	{
		m_pSwPRankingDisplay = new CSwPRankingDisplay;
		m_pSwPRankingDisplay->Create( SCHOOL_WARS_WINDOW, "BASIC_WINDOW", UI_FLAG_XSIZE | UI_FLAG_YSIZE );
		m_pSwPRankingDisplay->CreateBaseWindowLightGray( "SCHOOLWARS_WINDOW", "TEST" );
		m_pSwPRankingDisplay->SetTitleName( ID2GAMEWORD("SCHOOLWARS_TITLE", 0 ) );
		m_pSwPRankingDisplay->CreateSubControl ();
		m_pSwPRankingDisplay->ResetAlignFlag ();
		RegisterControl ( m_pSwPRankingDisplay, true );
	...
	////RankingWindow
	{
		m_pRrPRankingDisplay = new CRRPRankingDisplay;
		m_pRrPRankingDisplay->Create( ROYAL_RUMBLE_WINDOW, "BASIC_WINDOW", UI_FLAG_XSIZE | UI_FLAG_YSIZE );
		m_pRrPRankingDisplay->CreateBaseWindowBlackNew( "ROYAL_RUMBLE_WINDOW", "TEST" );
		m_pRrPRankingDisplay->SetTitleName( ID2GAMEWORD("ROYALRUMBLE_TITLE", 0 ) );
		m_pRrPRankingDisplay->CreateSubControl ();
		m_pRrPRankingDisplay->ResetAlignFlag ();
		RegisterControl ( m_pRrPRankingDisplay, true );
	...
	{
		m_pCtfTopTenRank = new CCtfTopTenRank;
		m_pCtfTopTenRank->Create( TOPTEN_CTF_RANK_WINDOW, "BASIC_WINDOW", UI_FLAG_XSIZE | UI_FLAG_YSIZE );
		m_pCtfTopTenRank->CreateBaseWindowBlackNew( "ROYAL_RUMBLE_WINDOW", "TEST" );
		m_pCtfTopTenRank->SetTitleName( ID2GAMEWORD("ROYALRUMBLE_TITLE", 0 ) );
		m_pCtfTopTenRank->CreateSubControl ();
		m_pCtfTopTenRank->ResetAlignFlag ();
		RegisterControl ( m_pCtfTopTenRank, true );

		ShowGroupFocus ( TOPTEN_CTF_RANK_WINDOW );
		HideGroup ( TOPTEN_CTF_RANK_WINDOW );
	}
	...
	
	{
		...
		m_pAttendanceBookWindow->CreateBaseWindowBlackNew ( "ATTENDANCE_BOOK_WINDOW", (char*)ID2GAMEWORD("ATTENDANCE_BOOK", 0) );
...

	{
		...
		pFaceChangeWindow->CreateBaseWindowBlackNew( "FACESTYLECARD_WINDOW", NULL );
...

	{
		...
		m_pClubBattleModalWindow->CreateBaseWindowBlackNew( "CLUB_BATTLE_MODAL_WINDOW", NULL );
...

	{
		...
		m_pTaxiWindow->CreateBaseWindowBlackNew ( "TAXI_WINDOW", (char*)ID2GAMEWORD("TAXI_WINDOW_NAME_STATIC") );
	...
	{	// ITEMREBUILD_MARK
		...
		m_pItemMixWindow->CreateBaseWindowBlackNew( "ITEM_MIX_WINDOW", NULL );
...

	{
		...
		m_pPvpWindowDisplay->Create( BATTLEFIELDUI_WINDOW, "BASIC_WINDOW", UI_FLAG_XSIZE | UI_FLAG_YSIZE );
		m_pPvpWindowDisplay->CreateBaseWindowLightGray( "PVP_WINDOW", NULL );
		...
		ShowGroupFocus( BATTLEFIELDUI_WINDOW );
		HideGroup( BATTLEFIELDUI_WINDOW );
	}

	{
		...
		m_pItemMixInvenWindow->CreateBaseWindowBlackNew( "ITEM_MIX_INVEN_WINDOW", NULL );
	...
	{	//add sw buff
		...
		m_pCtfInfoDisplay->Create ( INFO_CTF_DISPLAY, "INFO_CTF_DISPLAY", UI_FLAG_CENTER_X | UI_FLAG_YSIZE );
...
void CInnerInterface::UpdateClubBattleTime( float fClubBattleTime, bool bTW )
{
	if( bTW && m_pCtfRankingDisplay && m_pCtfInfoDisplay)
	{
		m_pCtfRankingDisplay->UpdateClubTime( fClubBattleTime );
		m_pCtfInfoDisplay->UpdateClubTime( fClubBattleTime );
	}
	else if( m_pMiniMap )
		m_pMiniMap->UpdateClubTime( fClubBattleTime );
}

void CInnerInterface::UpdateSchoolWarBattleTime( float fSchoolWarsTime )
{
	if( m_pMiniMap )
		m_pMiniMap->UpdateSchoolWarTime( fSchoolWarsTime );
}
...
void CInnerInterface::SLOT_EVENT_ON( bool bVisible, int nRamdomNumber )
{
	if( m_pCountMsgWindow )
	{
		m_pCountMsgWindow->SetCount( bVisible, nRamdomNumber);
		ShowGroupTop( COUNT_MSG_WINDOW );
	}
}

void CInnerInterface::SLOT_EVENT_ON_CD( bool bVisible )
{
	if( m_pCountMsgCDWindow )
	{
		m_pCountMsgCDWindow->SetCount( bVisible );
		ShowGroupTop( COUNT_MSG_CD_WINDOW );
	}
}
...
void CInnerInterface::ADDTEXT_MULTICOLORSPLIT ( const CString& strText, const D3DCOLOR& dwColor, const CString& strText2 ,const D3DCOLOR& dwColor2 )
{
	if ( m_bITEM_INFO_LINK_DISPLAY_MODE )
	{
		...
		int nIndex = CInnerInterface::GetInstance().ADDTEXT ( strText, dwColor );			
		CInnerInterface::GetInstance().ADDSTRING ( nIndex, strText2, dwColor2 );
		m_pInfoDisplayLink->RePosControl(0,0);
	}
	else if ( m_bITEM_INFO_EX_DISPLAY_MODE )
	{
...
void CInnerInterface::ADDTEXT_MULTICOLORSPLIT2 ( const CString& strText, const D3DCOLOR& dwColor , const CString& strText2 , const D3DCOLOR& dwColor2 , const CString& strText3, const D3DCOLOR& dwColor3 , const CString& strText4 , const D3DCOLOR& dwColor4 )
{
	if ( m_bITEM_INFO_LINK_DISPLAY_MODE )
	{
		...
		int nIndex = CInnerInterface::GetInstance().ADDTEXT ( strText, dwColor );			
		CInnerInterface::GetInstance().ADDSTRING ( nIndex, strText2, dwColor2 );
		CInnerInterface::GetInstance().ADDSTRING ( nIndex, strText3, dwColor3 );
		CInnerInterface::GetInstance().ADDSTRING ( nIndex, strText4, dwColor4 );
		m_pInfoDisplayLink->RePosControl(0,0);
	}
	else if ( m_bITEM_INFO_EX_DISPLAY_MODE )
	{
...
void CInnerInterface::ADDTEXT_MULTICOLORSPLIT3 ( const CString& strText, const D3DCOLOR& dwColor , const CString& strText2 , const D3DCOLOR& dwColor2 , const CString& strText3, const D3DCOLOR& dwColor3 )
{
	if ( m_bITEM_INFO_LINK_DISPLAY_MODE )
	{
		m_pInfoDisplayLink->RePosControl(0,0);
...
void CInnerInterface::ADDTEXT_MULTICOLORSPLIT6 ( const CString& strText, const D3DCOLOR& dwColor , const CString& strText2 , const D3DCOLOR& dwColor2 , const CString& strText3, const D3DCOLOR& dwColor3 , const CString& strText4, const D3DCOLOR& dwColor4 , const CString& strText5, const D3DCOLOR& dwColor5 , const CString& strText6, const D3DCOLOR& dwColor6)
{
	if ( m_bITEM_INFO_LINK_DISPLAY_MODE )
	{
		...
		CInnerInterface::GetInstance().ADDSTRING ( nIndex, strText3, dwColor3 );
		CInnerInterface::GetInstance().ADDSTRING ( nIndex, strText4, dwColor4 );
		CInnerInterface::GetInstance().ADDSTRING ( nIndex, strText5, dwColor5 );
		CInnerInterface::GetInstance().ADDSTRING ( nIndex, strText6, dwColor6 );
		m_pInfoDisplayLink->RePosControl(0,0);
	}
	else if ( m_bITEM_INFO_EX_DISPLAY_MODE )
	{
...
void CInnerInterface::ADDTEXT_MULTICOLORSPLIT5 ( const CString& strText, const D3DCOLOR& dwColor , const CString& strText2 , const D3DCOLOR& dwColor2 , const CString& strText3, const D3DCOLOR& dwColor3 , const CString& strText4 , const D3DCOLOR& dwColor4 , const CString& strText5 , const D3DCOLOR& dwColor5 )
{
	if ( m_bITEM_INFO_LINK_DISPLAY_MODE )
	{
		m_pInfoDisplayLink->RePosControl(0,0);
...
void CInnerInterface::ADDTEXT_MULTICOLORSPLIT11 ( const CString& strText, const D3DCOLOR& dwColor , const CString& strText2 , const D3DCOLOR& dwColor2 , const CString& strText3, const D3DCOLOR& dwColor3 , const CString& strText4 , const D3DCOLOR& dwColor4 , const CString& strText5 , const D3DCOLOR& dwColor5 , const CString& strText6 , const D3DCOLOR& dwColor6 , const CString& strText7 , const D3DCOLOR& dwColor7 , const CString& strText8 , const D3DCOLOR& dwColor8 , const CString& strText9 , const D3DCOLOR& dwColor9 , const CString& strText10 , const D3DCOLOR& dwColor10 , const CString& strText11 , const D3DCOLOR& dwColor11 )
{
	if ( m_bITEM_INFO_LINK_DISPLAY_MODE )
	{
		...
		CInnerInterface::GetInstance().ADDSTRING ( nIndex, strText5, dwColor5 );
		CInnerInterface::GetInstance().ADDSTRING ( nIndex, strText6, dwColor6 );
		CInnerInterface::GetInstance().ADDSTRING ( nIndex, strText7, dwColor7 );
		CInnerInterface::GetInstance().ADDSTRING ( nIndex, strText8, dwColor8 );
		CInnerInterface::GetInstance().ADDSTRING ( nIndex, strText9, dwColor9 );
		CInnerInterface::GetInstance().ADDSTRING ( nIndex, strText10, dwColor10 );
		CInnerInterface::GetInstance().ADDSTRING ( nIndex, strText11, dwColor11 );
		m_pInfoDisplayLink->RePosControl(0,0);
	}
	else if ( m_bITEM_INFO_EX_DISPLAY_MODE )
	{
