...
CRRPRankingDisplay::CRRPRankingDisplay()
	//For Player Ranking
	:	m_pRrPRankText ( NULL )
	,	m_pRrPNameText ( NULL )
	,	m_pRrPKillText ( NULL )
...
void CRRPRankingDisplay::CreateSubControl ()
{
...
void CRRPRankingDisplay::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	CUIWindowEx::Update ( x, y, LB, MB, RB, nScroll, fElapsedTime, bFirstControl );
}

void CRRPRankingDisplay::RefreshRrPRanking()
{
	...
	{
		strRank.Format( "%2d" ,vecRrPRank[i].wCharRanking);
		m_pRrPRankText->AddText( strRank, NS_UITEXTCOLOR::LIGHTGREY );

		strName.Format( "%s" ,vecRrPRank[i].szCharName);
		m_pRrPNameText->AddText( strName, NS_UITEXTCOLOR::PALEGREEN );

		strKill.Format( "%d" ,vecRrPRank[i].wKillNum);
		m_pRrPKillText->AddText( strKill, NS_UITEXTCOLOR::GOLD );

		strDeath.Format( "%d" ,vecRrPRank[i].wDeathNum);
		m_pRrPDeathText->AddText( strDeath, NS_UITEXTCOLOR::RED );
	}
}