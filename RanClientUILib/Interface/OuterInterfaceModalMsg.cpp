...
void COuterInterface::ModalMsgProcess ( UIGUID nCallerID, DWORD dwMsg )
{
	...
	case OUTER_MODAL_REGISTERSUCCEED:
		{
			if ( dwMsg & UIMSG_MODAL_OK )
			{
				DoModalOuter ( ID2GAMEEXTEXT ( "REGISTERSTAGE_3" ), MODAL_INFOMATION, OK,OUTER_MODAL_REGISTERTOLOGIN );
			}
		}
		break;
	case OUTER_MODAL_REGISTERTOLOGIN:
		{
			if ( dwMsg & UIMSG_MODAL_OK )
			{
				CNetClient* pNetClient = DxGlobalStage::GetInstance().GetNetClient ();
				...
				COuterInterface::GetInstance().ToSelectServerPage ( REGISTER_PAGE );
			}
		}break;
	...
	case OUTER_MODAL_REGISTERERROR:
		{
			if ( dwMsg & UIMSG_MODAL_OK )
			{
				if( RANPARAM::VALIDIDPARAM () )
				{
					...
					if( uiGUID == REGISTER_PAGE )
						COuterInterface::GetInstance().GetRegisterPage()->ResetAll();
					
				}
			}
		}break;

	case OUTER_MODAL_REGISTERFAIL:
		{
			if( RANPARAM::VALIDIDPARAM () )
			{
				UIGUID uiGUID = GetModalCallWindowID();
