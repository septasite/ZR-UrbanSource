...
//#include "CTFRankingDisplay/CTFTowerStatus.h"
...
{	
	...
	if ( CHECK_MOUSE_IN ( dwMsg ) )
	{
		...
		case SW_RANKING_DISPLAY:
		case PK_COMBO_DISPLAY:
		case INFO_CTF_DISPLAY:
		//case KILL_TIME_DISPLAY:
			break;
		...
		//case CTF_TOWER_STATUS:
		//	{
		//		if ( UIMSG_MOUSEIN_CTF_SCROLLBAR & dwMsg )
		//		{
		//			SetCharMoveBlock ();
		//		}
		//	}
	...
	case MINIPARTY_OPEN:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{	
				CInnerInterface::GetInstance().SHOW_COMMON_LINEINFO( "Party Window", NS_UITEXTCOLOR::WHITE  );
			}
			if ( bMOUSEIN_LBUP )
			{
				HideGroup ( ControlID );
...
void	CInnerInterface::DisplayChatMessage ( int nType, const char *szName, const char *szMsg, SITEMLINK sLINK  )
{
	if ( m_pChat )
	{
		...
		case CHAT_TYPE_GLOBAL:		// 관리용 글로벌 메시지
			{
				strCombine.Format ( "[%s]( %s ):%s",ID2GAMEWORD("CHAT_OPTION",8), ID2GAMEWORD("ADMIN_NAME"), strTemp );	
		...
		case CHAT_TYPE_GLOBAL_STAFF:		// 관리용 글로벌 메시지
			{
				...
				strCombine.Format ( "[%s]( %s ):%s",ID2GAMEWORD("CHAT_OPTION",8), ID2GAMEWORD("ADMIN_NAME_STAFF"), strTemp );	
				m_pChat->AddStringToChatEx ( strCombine, CHAT_SYSTEM, &sLINK );
			}
			break;

		case CHAT_TYPE_GLOBAL_STAFF2:		// 관리용 글로벌 메시지
			{
				
				strCombine.Format ( "[%s]( %s ):%s",ID2GAMEWORD("CHAT_OPTION",8), ID2GAMEWORD("ADMIN_NAME_STAFF2"), strTemp );	
				m_pChat->AddStringToChatEx ( strCombine, CHAT_SYSTEM, &sLINK );
			}
			break;
		...
		case CHAT_TYPE_REGION:		//add region chat by AnFire16 ( 7/31/2017)
			{
				strCombine.Format ( "[%s]( %s ):%s",ID2GAMEWORD("CHAT_OPTION",9), szName, strTemp );	
				m_pChat->AddStringToChatEx ( strCombine, CHAT_REGION, &sLINK );
			}
			break;
		...
		case CHAT_TYPE_CTRL_GLOBAL:
			{
				strCombine.Format ( "[%s]( %s ):%s",ID2GAMEWORD("CHAT_OPTION",8), ID2GAMEWORD("NOTIFY_NAME"), strTemp );	
				m_pChat->AddStringToChatEx ( strCombine, CHAT_SYSTEM, &sLINK );
			}
			break;
		...
		case CHAT_TYPE_GLOBAL:
			{
				if ( m_pAdminMessageDisplay )
				{
					...
					m_pAdminMessageDisplay->AddText ( strCombine, NS_UITEXTCOLOR::IDNAMECOLOR );
		...
		case CHAT_TYPE_GLOBAL_STAFF2:
			{
				if ( m_pAdminMessageDisplay )
				{
					if ( !IsVisibleGroup ( ADMIN_MESSAGE_DISPLAY ) )
						strCombine.Format ( "[ %s ] : %s", ID2GAMEWORD("ADMIN_NAME_STAFF2"), strTemp );	
					else
						strCombine.Format ( "%s", strTemp );	
