...
CFriendWindowNormalPage::CFriendWindowNormalPage ()
	...
	, m_pDEL(NULL)
	, m_pMOVE(NULL)
	, m_pPARTY(NULL)	//added by : tobets | 08-03-2012 | add party button in friend window
	//, m_pNote(NULL)
	, m_pEditBox(NULL)
	, m_pBLOCK(NULL)
{
...
void CFriendWindowNormalPage::CreateSubControl ()
{
	...
	m_pADD = CreateButtonBlue ( "FRIEND_LIST_ADDBUTTON", "FRIEND_LIST_BUTTON_TEXT_POS", pFont9, nAlignCenterBoth, FRIEND_LIST_ADDBUTTON, "Add" );
	m_pADD->SetShortcutKey ( DIK_RETURN );
	m_pDEL = CreateButtonBlue ( "FRIEND_LIST_DELBUTTON", "FRIEND_LIST_BUTTON_TEXT_POS", pFont9, nAlignCenterBoth, FRIEND_LIST_DELBUTTON, "Delete" );
	m_pMOVE = CreateButtonBlue ( "FRIEND_LIST_MOVEBUTTON", "FRIEND_LIST_BUTTON_TEXT_POS", pFont9, nAlignCenterBoth, FRIEND_LIST_MOVEBUTTON, "Move" );
	//added by : tobets | 08-03-2012 | add party button in friend window
	m_pPARTY = CreateButtonBlue ( "FRIEND_LIST_PARTYBUTTON", "FRIEND_LIST_BUTTON_TEXT_POS", pFont9, nAlignCenterBoth, FRIEND_LIST_PARTYBUTTON, "Party" );
//#if defined(KRT_PARAM)
//	m_pBLOCK = CreateTextButton24 ( "FRIEND_LIST_BLOCKBUTTON", FRIEND_LIST_BLOCKBUTTON, (char*)ID2GAMEWORD("FRIEND_BUTTON",15) );
//#else
	m_pBLOCK = CreateButtonBlue ( "FRIEND_LIST_BLOCKBUTTON", "FRIEND_LIST_BUTTON_TEXT_POS", pFont9, nAlignCenterBoth, FRIEND_LIST_BLOCKBUTTON, "Block" );
//#endif
...
CBasicTextButton* CFriendWindowNormalPage::CreateTextButton24 ( const char* szButton, UIGUID ControlID, const char* szText )
{
...
CBasicButtonText* CFriendWindowNormalPage::CreateButtonBlue ( char* szButton, char* szTextBox, CD3DFontPar* pFont, int nAlign, UIGUID ControlID, CString strText )
{
	...
	case FRIEND_LIST_TEXTBOX:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				m_pListText->SetOverColor ( nIndex, NS_UITEXTCOLOR::IDNAMECOLOR );
	...
	case FRIEND_LIST_PARTYBUTTON:
		{
			if ( CHECK_MOUSEIN_LBUPLIKE ( dwMsg ) )
			{
				...
				CString strName;
				if( GetFriend ( nCharID, strName ) ) PARTY_FRIEND ( strName );
			}
		}
		break;
...
void CFriendWindowNormalPage::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	...
	if ( 0 <= m_nSelectIndex && m_nSelectIndex < m_pListText->GetCount () )
	{
		...
		m_pListText->SetTextColor ( m_nSelectIndex, NS_UITEXTCOLOR::LIGHTSKYBLUE );
	}
...
//added by : tobets | 20-03-2012 | added party for friend window
BOOL CFriendWindowNormalPage::IsSelectedIndex()
{
	return ( 0 <= m_nSelectIndex && m_nSelectIndex < m_pListText->GetCount() );
}

/*const DWORD CFriendWindowNormalPage::GetFriendID()
{
	if ( !IsSelectedIndex() )
