...
const int CCreateCharacterNew::nLIMITCHAR = 14;
...
void CCreateCharacterNew::CreateSubControl()
{
	...
	CD3DFontPar* pFont20 = DxFontMan::GetInstance().LoadDxFont ( _DEFAULT_FONT, 20 * fontSize, TRUE, D3DFONT_SHADOW | D3DFONT_ASCII );
	CD3DFontPar* pFont12 = DxFontMan::GetInstance().LoadDxFont ( _DEFAULT_FONT, 12 * fontSize, TRUE, D3DFONT_SHADOW | D3DFONT_ASCII );
	CD3DFontPar* pFont9 = DxFontMan::GetInstance().LoadDxFont ( _DEFAULT_FONT, 9 * fontSize, TRUE, D3DFONT_SHADOW | D3DFONT_ASCII );
	CD3DFontPar* pFont10 = DxFontMan::GetInstance().LoadDxFont ( _DEFAULT_FONT, 10 * fontSize, TRUE, D3DFONT_SHADOW | D3DFONT_ASCII );
	CD3DFontPar* pFont16 = DxFontMan::GetInstance().LoadDxFont ( _DEFAULT_FONT, 16 * fontSize, TRUE, D3DFONT_SHADOW | D3DFONT_ASCII );
	CD3DFontPar* pFont8 = DxFontMan::GetInstance().LoadDxFont ( _DEFAULT_FONT, 8 * fontSize, TRUE, D3DFONT_SHADOW | D3DFONT_ASCII );
	//พื้นหลัง
	...
	//พื้นหลังในส่วนของหัวข้อ เลือกโรงเรียน
	{
	...
	//พื้นหลังในส่วนของหัวข้อ เลือกเพศ
	{
	...
	//พื้นหลังในส่วนของหัวข้อ เลือกอาชีพ
	{
	...
	//พื้นหลังในส่วนของหัวข้อ เลือกสไตล์
	{
	...
	//ปุ่มกดเลือกโรงเรียน SG ยังไม่ได้กด
	{		
	...
	//ปุ่มกดเลือกโรงเรียน SG ตอนที่กด
	{
	...
	//ปุ่มกดเลือกโรงเรียน MP ยังไม่ได้กด
	{	
	...
	//ปุ่มกดเลือกโรงเรียน MP ตอนที่กด
	{
	...
	//ปุ่มกดเลือกโรงเรียน PH ยังไม่ได้กด
	{
	...
	//ปุ่มกดเลือกโรงเรียน PH ตอนที่กด
	{
	...
	case SELECT_CLASS_MEN_IMAGE0:
		{
			...
			{
				m_pSelectClassImg_men[0]->SetVisibleSingle(FALSE);
			}
		}
		break;
	...
	case SELECT_CLASS_WOMEN_IMAGE0:
		{
			...
			{
				m_pSelectClassImg_women[0]->SetVisibleSingle(FALSE);
			}
		}
		break;
...
void CCreateCharacterNew::PREV_STEP()
{
	...
	// ตำแหน่งเดิมของ ชื่อ ร.ร
	Pos2 = m_pSchoolNameStatic->GetGlobalPos();
	// ตำแหน่งเดิมของ ข้อมูล ร.ร
...
void CCreateCharacterNew::SELECT_SEX_PROCESS( int nSelect )
{
	...
	// พื้นหลัง ร.ร. เปลี่ยนตำแหน่ง
	/*UIRECT Pos,newPos,newPos2;
	//ตำแหน่งเดิมของ พื้นหลัง ร.ร.
	Pos = pBasicLineBoxSchool->GetGlobalPos();
	//ตำแหน่งเดิมของ พื้นหลัง เพศ
	newPos = pBasicLineBoxSex->GetGlobalPos();

	// ตำแห่นง ร.ร. ใหม่ แกนเอ็กซ์ - ด้วยความกว้าง ของ พื้นหลัง เพศ
	newPos2.left = newPos.left - newPos.sizeX;
	// ตำแหน่ง วาย ใหม่ เท่ากลับ ตำแหน่งวาย ของพื้นหลัง เพศ
	newPos2.top = newPos.top;
	// ตั้งต่า เดิม ของ กว้าง และ ยาว 
	newPos2.sizeX = Pos.sizeX;
	newPos2.sizeY = Pos.sizeY;
	
	// เซต ตำแหน่งใหม่ ให่ พื้นหลัง ร.ร.
	pBasicLineBoxSchool->SetGlobalPos(newPos2);

	UIRECT Pos1,Pos2,Pos3,newPos3,newPos4,newPos5;
	// ตำแหน่ง เดิมของ ไอคอน ร.ร
...
int CCreateCharacterNew::GetCharIndex()
{
	if ( !m_nSex )
	{
		...
		case ASSASSIN_CLASS:	return GLCI_ASSASSIN_M;
		}
	}
	else
	{
		...
		case ASSASSIN_CLASS:	return GLCI_ASSASSIN_W;
		}
