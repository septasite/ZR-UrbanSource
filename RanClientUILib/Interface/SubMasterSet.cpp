...
{
	CD3DFontPar* m_pFont = DxFontMan::GetInstance().LoadDxFont ( _DEFAULT_FONT, 8, D3DFONT_SHADOW );
...

	{
		...
		pTextBox = CreateStaticControl ( "SUBMASTER_SW_STATIC", m_pFont, NS_UITEXTCOLOR::DEFAULT, TEXT_ALIGN_LEFT );
		pTextBox->SetOneLineText( ID2GAMEWORD( "SUBMASTER_OPTION", 7 ) );
	}

	{
		...
		//m_pSWButton = CreateFlipButton( "SUBMASTER_SW_BUTTON", "SUBMASTER_SW_BUTTON_F", SUBMASTER_SW_BUTTON );
	}

	{
		// 확인, 취소
		CBasicTextButton* pButton = NULL;
		pButton = CreateTextButton24( "SUBMASTER_OK", SUBMASTER_OK, (char*)ID2GAMEWORD( "PRIVATE_MARKET_MAKE_BUTTON", 0 ) ); // 개인상점의 확인,취소를 그대로 사용
		pButton->SetShortcutKey( DIK_RETURN, DIK_NUMPADENTER );
		pButton = CreateTextButton24( "SUBMASTER_CANCEL", SUBMASTER_CANCEL, (char*)ID2GAMEWORD( "PRIVATE_MARKET_MAKE_BUTTON", 1 ) );
...
CBasicTextButton*  CSubMasterSet::CreateTextButton24 ( const char* szButton, UIGUID ControlID, const char* szText )
{
...
//void CSubMasterSet::LoadSchoolWar()
//{
//	bool bSW = m_pCLUB->IsMemberFlgSW( m_dwCharID );
//	m_pSWButton->SetFlip( bSW );
//	SetCheck_Flags( bSW, m_dwClubFlag, EMCLUB_SUBMATER_SW );
//}
...
void CSubMasterSet::TranslateUIMessage ( UIGUID ControlID, DWORD dwMsg )
{
	...
	/*case SUBMASTER_SW_BUTTON:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				if ( dwMsg & UIMSG_LB_UP )
				{
					bool bSW = GetCheck_Flags( m_dwClubFlag, EMCLUB_SUBMATER_SW );
					SetCheck_Flags( !bSW, m_dwClubFlag, EMCLUB_SUBMATER_SW );
					m_pSWButton->SetFlip( !bSW );
				}
			}
