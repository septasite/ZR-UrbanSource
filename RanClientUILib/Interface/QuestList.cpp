...
void CQuestList::CreateSubControl ()
{
	...
	m_pButtonMenuL = CreateTextButton ( "QUEST_LIST_BUTTON_L_NEW", QUEST_LIST_BUTTON_L, (char*)ID2GAMEWORD("QUEST_LIST_BUTTON",0) );
	m_pButtonMenuL->SetFlip ( FALSE );
	m_pButtonMenuR = CreateTextButton ( "QUEST_LIST_BUTTON_R_NEW", QUEST_LIST_BUTTON_R, (char*)ID2GAMEWORD("QUEST_LIST_BUTTON",1) );	
	...
	pBasicLineBox->CreateBaseBoxQuestList ( "QUEST_LIST_BACK_NEW" );
	...
	//added by : geger009 | 11-03-2012 | add new interface for quest window
	{
		...
		pTextBox->CreateSub ( this, "QUEST_LIST_TEXTBOX_NEW", UI_FLAG_DEFAULT, QUEST_LIST_TEXTBOX_PROG );
		...
		pScrollBar->CreateBaseScrollBar ( "QUEST_LIST_SCROLLBAR_NEW" );
	...
	{
		CBasicTextBoxEx* pTextBox = new CBasicTextBoxEx;
		pTextBox->CreateSub ( this, "QUEST_LIST_TEXTBOX_NEW", UI_FLAG_DEFAULT, QUEST_LIST_TEXTBOX_END );
		pTextBox->SetFont ( pFont9 );		
		pTextBox->SetSensitive ( true );
		pTextBox->SetLineInterval ( 3.0f );
		pTextBox->SetLimitLine ( nLimitQuest );
...
void CQuestList::Update ( int x, int y, BYTE LB, BYTE MB, BYTE RB, int nScroll, float fElapsedTime, BOOL bFirstControl )
{
	...
	if ( bProg )
	{
		if ( m_pListTextProg->GetCount () && 0 <= m_nSelectIndex )
		{
			...
			m_pListTextProg->SetTextColor ( m_nSelectIndex, NS_UITEXTCOLOR::IDNAMECOLOR );
		}
	}
	else
	{
		if ( m_pListTextEnd->GetCount () && 0 <= m_nSelectIndex )
		{
			...
			m_pListTextEnd->SetTextColor ( m_nSelectIndex, NS_UITEXTCOLOR::IDNAMECOLOR );
		}
	}
	...
	case QUEST_LIST_TEXTBOX_PROG:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				m_pListTextProg->SetOverColor ( nIndex, NS_UITEXTCOLOR::IDNAMECOLOR );
			}
		}
		break;
...
CBasicTextButton*  CQuestList::CreateTextButton ( const char* szButton, UIGUID ControlID, const char* szText )
{
...
bool CQuestList::LoadQuestList ()
{
	...
	{
		//m_pListTextProg->SetVisibleSingle ( TRUE );
...
//		m_pListTextProg->AddText ( "프로크" );
	...
	{
		//m_pListTextEnd->SetVisibleSingle ( TRUE );
