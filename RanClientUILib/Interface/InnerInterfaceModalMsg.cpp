...
void CInnerInterface::MsgProcess ( LPVOID msgBuffer )
{
	...
	case NET_MSG_CHAT_CTRL_FB:			// 관리자용 채팅메시지
		{//edit for add staffchat
			...
			if ( pNc->bStaff )
			{
				DisplayChatMessage ( CHAT_TYPE_GLOBAL_STAFF, NULL, pNc->szChatMsg, pNc->sITEMLINK );
			}
			else if ( pNc->bStaff2 )
			{
				DisplayChatMessage ( CHAT_TYPE_GLOBAL_STAFF2, NULL, pNc->szChatMsg, pNc->sITEMLINK );
			}
			//else if ( pNc->bStaff3 )
			//{
			//	DisplayChatMessage ( CHAT_TYPE_GLOBAL_STAFF3, NULL, pNc->szChatMsg, pNc->sITEMLINK );
			//}
			else
			{
...
//					PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("EMTAKE_FB_DISTANCE") );
	...
	case NET_MSG_GCTRL_CONFRONT_TAR:  //invited to duel ,recieve net message for invitation , code to show modal for invitation
		{
...
void CInnerInterface::ModalMsgProcess ( UIGUID nCallerID, DWORD dwMsg )
{
	...
	case MODAL_DROP_ITEM:
		{
			...
			if ( bCollision )
			{
				D3DXVECTOR3 vDir = vCollisionPos - GLGaeaClient::GetInstance().GetCharacter()->GetPosition ();
				vDir.y = 0.0f;

				D3DXVec3Normalize ( &vDir, &vDir );
				D3DXVECTOR3 vDropPos = GLGaeaClient::GetInstance().GetCharacter()->GetPosition() + vDir*float(GLCONST_CHAR::wBODYRADIUS+12);
				...
				vDropPos = GLGaeaClient::GetInstance().GetCharacter()->GetPosition ();
	...
	case MODAL_PARTY_REQ:
		{
			if ( dwMsg & UIMSG_MODAL_OK )
			{
				//added by : tobets | 20-03-2012 | added party button friend and club
				if ( m_pPtoPWindow )
				{
	...
	/*case MODAL_ITEM_BOX_OPEN:
		{
			if ( UIMSG_MODAL_OK & dwMsg )
			{
				GLGaeaClient::GetInstance().GetCharacter ()->ReqReqBoxOpen ( );
			}
		}
		break;*/
	...
	case MODAL_PRIVATE_MARKET_SELL: // Vend Price Max Increased | 07-27-2013
		{
			if ( UIMSG_MODAL_OK & dwMsg )
			{
				WORD wPosX, wPosY;
...
//					DWORD dwPrice = static_cast<DWORD>(pItem->sBasicOp.dwBuyPrice*fSHOP_RATE/100.0f+0.1f);
	...
	case MODAL_ITEMREBUILD_OKQUESTION: //sealed card 
		{
			if( UIMSG_MODAL_OK & dwMsg )
			{
				...
				GLGaeaClient::GetInstance().GetCharacter()->ReqRebuildResult( sData1, sData2 );
			}
		}
		break;
	...
	case MODAL_CALLPET:
		{
			if ( dwMsg & UIMSG_MODAL_OK )
			{
				...
				if ( pCharacter )
				{
					WORD wPosX, wPosY;
					GetInventoryWindow ()->GetSplitPos ( &wPosX, &wPosY );
					BOOL bOK = pCharacter->ReqInvenDrug ( wPosX, wPosY );
				}
			}
		}
		break;

	case MODAL_CALLBACKPET:
		{
			if ( dwMsg & UIMSG_MODAL_OK )
			{
				GLCharacter* pCharacter = GLGaeaClient::GetInstance().GetCharacter();
	...
	case MODAL_SKILLFACT_ASK_RESET: //disable skill effect NaJDeV
	{
		if ( dwMsg & UIMSG_MODAL_OK )
		{
	...
	case MODAL_EXIT_TOWERWARS:
		{
			if ( UIMSG_MODAL_OK & dwMsg )
			{
				GLGaeaClient::GetInstance().GetCharacter()->ExitTowerWars();
			}

		}
		break;

	case MODAL_REMOVE_WRAP:
		{
			if ( UIMSG_MODAL_OK & dwMsg )
			{
				WORD wPosX, wPosY;
				GetInventoryWindow ()->GetSplitPos ( &wPosX, &wPosY );
				GLGaeaClient::GetInstance().GetCharacter()->ReqRemoveWrap(wPosX, wPosY);
			}

		}
		break;

	case MODAL_SCHOOL_CHANGE_END:
		{
			if( UIMSG_MODAL_OK & dwMsg )
			{
				WAITSERVER_DIALOGUE_OPEN ( ID2GAMEINTEXT("WAITSERVER_MESSAGE"), WAITSERVER_TOLOBY );
	...
	case MODAL_SCHOOL_CHANGE_SG:
		{
			if( UIMSG_MODAL_OK & dwMsg )
			{
				GLGaeaClient::GetInstance().GetCharacter()->ReqInvenSchoolChange(0);
			}
		}
		break;

	case MODAL_SCHOOL_CHANGE_MP:
		{
			if( UIMSG_MODAL_OK & dwMsg )
			{
				GLGaeaClient::GetInstance().GetCharacter()->ReqInvenSchoolChange(1);
			}
		}
		break;

	case MODAL_SCHOOL_CHANGE_PH:
		{
			if( UIMSG_MODAL_OK & dwMsg )
			{
				GLGaeaClient::GetInstance().GetCharacter()->ReqInvenSchoolChange(2);
			}
		}
		break;
