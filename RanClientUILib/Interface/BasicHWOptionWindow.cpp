...
void CBasicHWOptionWindow::GamePlayOption_OK()
{
	...
	DXPARAMSET::GetInstance().m_bCHARACTER_SIMPLIFY = m_pGamePlayOption->m_bCharacterSimplify;
	RANPARAM::bNON_Rebirth		= m_pGamePlayOption->m_bNon_Rebirth;
	RANPARAM::bFPSLATENCY		= m_pGamePlayOption->m_bFpsLatency;
	RANPARAM::bAutoTarget		= m_pGamePlayOption->m_bAutoTarget;
	RANPARAM::bSkillEffects		= m_pGamePlayOption->m_bSkillEffects;

	RANPARAM::bIdleAnim		= m_pGamePlayOption->m_bIdleAnim;
	GLGaeaClient::GetInstance().GetCharacter()->ReqNonRebirth( RANPARAM::bNON_Rebirth );

	DXPARAMSET::GetInstance().GameOK( DxGlobalStage::GetInstance().GetD3DApp ());
}
