...
{
	D3DCOLOR dwColor = NS_UITEXTCOLOR::SILVER;
   ...
   // CreateControl ( "SKILL_SLOT_BACK_SKILL_TEXT_L" );	
	...
	{
//		GASSERT ( 0 && "¹«È¿È­ÇÑ ID ÀÔ´Ï´Ù. È®ÀÎÇÏ½Ê½Ã¿À." );
	}
	else
	{
...
{	
	...
	if ( m_pTextBoxLine1 && m_pTextBoxLine2 ) 
	{
		...
		else	
		{
			...
			else if ( NEEDLEVEL && pNeedSkill  ) // µéÍ§¡ÒÃàÇÅ?áÅ?Ê¡ÔÇ¡èÍ¹Ë¹éÒ
			{
				m_strLine2.Format ( "%s.%d/%s ", (char*)ID2GAMEWORD ( "RNSKILL_SLOT", 3 ), sLVL.dwLEVEL ,pNeedSkill->GetName() );
			}
