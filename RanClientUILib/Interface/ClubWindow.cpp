...
const int CClubWindow::nLIMIT_CHAR = 20;
...
CClubWindow::CClubWindow ()
	...
	, m_pClubUnion(NULL)
	, m_pLeaveButton(NULL)
	, m_pPartyButton(NULL)		//added by : tobets | 08-03-2012 | add party button in club window
...
{
	CD3DFontPar* pFont = DxFontMan::GetInstance().LoadDxFont ( _DEFAULT_FONT, 9, D3DFONT_SHADOW );
	
	CBasicTextBox* pTextBox = NULL;	

	{
		m_bg_white = new CUIControl;
		m_bg_white->CreateSub ( this, "CLUB_WINDOW_BG_WHITE", UI_FLAG_DEFAULT);	
...
void CClubWindow::TranslateUIMessage ( UIGUID cID, DWORD dwMsg )
{
	...
	case CLUB_LEAVE_BUTTON:
		...
		break;
	//added by : tobets | 08-03-2012 | add party button in club window
...
void CClubWindow::ResetState ()
{
	...
	m_pPartyButton->SetVisibleSingle ( FALSE );		//added by : tobets | 08-03-2012 | add party button in club window
	...
	case CLUB_MEMBER_PAGE:
		{
			if( sCLUB.IsMaster( dwCharID ) )
			{
				...
				m_pPartyButton->SetVisibleSingle( TRUE );	//added by : tobets | 08-03-2012 | add party button in club window
			}
