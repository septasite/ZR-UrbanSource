	...
	case PTOPMENU_TRADE:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				if ( dwMsg & UIMSG_LB_UP )
				{
					...
					{
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, "Trade is not allowed on school wars." );
					return ;
					}
	...
	case PTOPMENU_PARTY:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				{
					PLANDMANCLIENT pLand = GLGaeaClient::GetInstance().GetActiveMap();
					if ( pLand->IsRoyalRumble() )
					{
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, "Party is not allowed on Royal Rumble." );
					return ;
					}
	...
	case PTOPMENU_CONFLICT:
		{
			if ( CHECK_MOUSE_IN ( dwMsg ) )
			{
				...
				if ( dwMsg & UIMSG_LB_UP )
				{
					...
					if (pCharacter->IsSafeZone() && GLGaeaClient::GetInstance().GetActiveMap()->GetMapID().wMainID == 22)
					{
						CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, "Duel is not allowed in safezone areas." );
						return ;
					}
					...
					if ( pLand->IsSchoolWars() || pLand->IsRoyalRumble() )
					{
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, "Duel is not allowed on school wars." );
					return ;
					}
