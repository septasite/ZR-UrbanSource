...
IMPLEMENT_DYNAMIC(AEffTab, CPropertySheet)

AEffTab::AEffTab(CRect& rect, LOGFONT logfont, CWnd* pParentWnd)
	 ...
	 m_pFont ( NULL )
{
	...
	m_EffTrace.SetEffTab ( this );
	AddPage ( &m_EffTrace );
	m_EffFaceOff.SetEffTab ( this );
	AddPage ( &m_EffFaceOff );
...
BEGIN_MESSAGE_MAP(AEffTab, CPropertySheet)
	//{{AFX_MSG_MAP(AEffTab)
	ON_WM_CTLCOLOR_REFLECT()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

void AEffTab::ActiveEffList( SANIMCONINFO::EFFANILIST m_listEffAni )
{
	SetActivePage ( ZEFF_LIST );
	m_EffList.SetEffectData( m_listEffAni );
}

void AEffTab::ActiveEffListNormal()
{
	SetActivePage ( ZEFF_LIST );
}

void AEffTab::ActiveEffSingle( DxEffAniData_Single* EffSingle )
{
	SetActivePage ( ZEFF_SINGLE );
	m_EffSingle.SetEffSingle (EffSingle );
}

void AEffTab::ActiveEffGhosting( DxEffAniData_Ghosting* EffGhosting )
{
	SetActivePage ( ZEFF_GHOSTING );
	m_EffGhosting.SetEffGhosting (EffGhosting );
}

void AEffTab::ActiveEffTrace( DxEffAniData_Trace* EffTrace )
{
	SetActivePage ( ZEFF_TRACE );
	m_EffTrace.SetEffTrace ( EffTrace );
}

void AEffTab::ActiveEffFaceOff( DxEffAniData_FaceOff* EffFaceOff )
{
	SetActivePage ( ZEFF_FACEOFF );
	m_EffFaceOff.SetEffFaceOff ( EffFaceOff);
}

void AEffTab::ActiveEffSingleReturn( DxEffAniData_Single* EffSingle )
{
	SetActivePage ( ZEFF_LIST );
	m_EffList.SetEffSingle( EffSingle );
}

void AEffTab::ActiveEffGhostingReturn( DxEffAniData_Ghosting* EffGhosting )
{
	SetActivePage ( ZEFF_LIST );
	m_EffList.SetEffGhosting( EffGhosting );
}

void AEffTab::ActiveEffTraceReturn( DxEffAniData_Trace* EffTrace )
{
	SetActivePage ( ZEFF_LIST );
	m_EffList.SetEffTrace( EffTrace );
}

void AEffTab::ActiveEffFaceOffReturn( DxEffAniData_FaceOff* EffFaceOff)
{
	SetActivePage ( ZEFF_LIST );
	m_EffList.SetEffFaceOff( EffFaceOff );
}

HBRUSH AEffTab::CtlColor(CDC* pDc, UINT uCtlColor) 
{
...
BOOL AEffTab::PreTranslateMessage(MSG* pMsg)
{
	switch ( pMsg->message )
	{
	case WM_KEYDOWN:
...
void AEffTab::ChangeDialogFont(CWnd* pWnd, CFont* pFont, int nFlag)
{
	CRect windowRect;
