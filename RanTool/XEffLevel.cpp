// XEffLevel.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CEffLevel, CPropertyPage)
CEffLevel::CEffLevel(LOGFONT logfont)
	: CPropertyPage(CEffLevel::IDD)
	,m_pFont( NULL )
	,EffLevelEdit(NULL)
	,m_pMaterials(NULL)
	,nMaterialSize(0)
{
	...
	EffLevelEdit = new DxEffCharLevel;
}
...
void CEffLevel::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CEffLevel, CPropertyPage)
...
// CEffLevel message handlers
BOOL CEffLevel::OnInitDialog()
{
...
void CEffLevel::SetEffLevel ( DxEffCharLevel*	EffLevelX)
{ 
	EffLevelEdit = EffLevelX; 
	Update();
}

void CEffLevel::OnBnClickedButtonOk()
{
	if (Save())
	{
		m_pEffTab->ActiveEffLevelReturn( EffLevelEdit ,nMaterialSize);
	}
}

void CEffLevel::OnBnClickedButtonCancel()
{
	m_pEffTab->ActiveEffListNormal();
}

BOOL CEffLevel::Update()
{
	EFFCHAR_PROPERTY_LEVEL EffEdit = *((EFFCHAR_PROPERTY_LEVEL*)EffLevelEdit->GetProperty() );
	...
	SetWin_Num_float ( this, IDC_EDIT_POWER,  EffEdit.m_fPower );
	...
	m_pMaterialsX = EffLevelEdit->GetMaterials();
	m_pVecMatX.clear();
...
BOOL CEffLevel::Save()
{
	EFFCHAR_PROPERTY_LEVEL* EffEdit = new EFFCHAR_PROPERTY_LEVEL;
	...
	EffEdit->m_fPower = GetWin_Num_float ( this, IDC_EDIT_POWER );
	...
	EffLevelEdit->SetProperty(EffEditProp);
	...
	EffLevelEdit->SetMaterials(NULL,nSize,m_pMaterials);
	m_pVecMatX.clear();

	return TRUE;
}

BOOL CEffLevel::UpdateList()
{
	CString strTemp;
...
void CEffLevel::OnBnClickedButtonAdd()
{
...
void CEffLevel::OnBnClickedButtonEdit()
{
...
void CEffLevel::OnBnClickedButtonDel()
{
	int nIndex = m_list_mat.GetCurSel();
	if ( nIndex != LB_ERR )
	{
...
void CEffLevel::OnBnClickedButtonCopy()
{
	int nIndex = m_list_mat.GetCurSel();
	if ( nIndex != LB_ERR )
	{
...
void CEffLevel::OnBnClickedButtonClear()
{
...
void CEffLevel::OnBnClickedButtonAok()
{
	DXMATERIAL_CHAR_EFF* m_pMat = new DXMATERIAL_CHAR_EFF;
...
void CEffLevel::OnBnClickedButtonAcan()
{
	UpdateList();
...
void CEffLevel::OnCbnSelchangeComboColop()
{
