...
HRESULT DxSkinAniControl::Render ( D3DXMATRIX &matRot )
{
	...
	size_t nVecSize = m_vecBoneScale.size();
	for ( size_t n=0; n<nVecSize; ++n )
	{
		DxBoneTrans *pBoneCur = NULL;
		pBoneCur = m_pSkeleton->FindBone( m_vecBoneScale[n].m_strBONE.c_str() );
		if ( pBoneCur )
		{
			pBoneCur->m_pQuatPosCUR.m_vScale.x *= m_vecBoneScale[n].m_vBoneScale.x;
			pBoneCur->m_pQuatPosCUR.m_vScale.y *= m_vecBoneScale[n].m_vBoneScale.y;
			pBoneCur->m_pQuatPosCUR.m_vScale.z *= m_vecBoneScale[n].m_vBoneScale.z;
			pBoneCur->m_pQuatPosPREV.m_vScale.x *= m_vecBoneScale[n].m_vBoneScale.x;
			pBoneCur->m_pQuatPosPREV.m_vScale.y *= m_vecBoneScale[n].m_vBoneScale.y;
			pBoneCur->m_pQuatPosPREV.m_vScale.z *= m_vecBoneScale[n].m_vBoneScale.z;
		}
	}
