...
public:
	...
	struct	BONESCALE
	{
		D3DXVECTOR3			m_vBoneScale;
		std::string			m_strBONE;

		BONESCALE()	:
		m_vBoneScale(0.0f,0.0f,0.0f)
		{
			m_strBONE = "";
		}
	};
	...
	typedef std::vector<BONESCALE>		VECBONESCALE;
	typedef VECBONESCALE::iterator		VECBONESCALE_ITER;
...
public:
	void InsertBoneScale( BONESCALE sBoneScale );
	void ClearBoneScale()	{	m_vecBoneScale.clear();	}
	DWORD GetBoneScaleSize()	{	return (DWORD)m_vecBoneScale.size();	}
	VECBONESCALE&		GetBoneScaleList ()		{ return m_vecBoneScale; }
