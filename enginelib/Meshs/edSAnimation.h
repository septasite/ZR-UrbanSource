...
//Added by geger009 | 11-8-2012 | add new version for animation bin
struct SAnimation
{
	//enum { OLD_VERSION = 0x0102, VERSION = 0x0103, };
	enum { VERSION = 0x0104 };
...
//we will copy the struct like this
struct SANIMCONINFO_107				// 107
{
	...
	//after that we must match it with the earlier version that one
	SANIMCONINFO_107 () :
	m_dwFlag(ACF_LOOP|ACF_NEWINFO),
	m_dwSTime(0),
	m_dwETime(0),
...
//thats what i do. but we need add more so just copy paste the 107 struct in above..look this first
...
{
	//all struct is done the rest is just edit the load function see the load function this the fast way
	enum { VERSION = 0x0111 };
