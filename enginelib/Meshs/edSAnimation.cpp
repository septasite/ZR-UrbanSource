...
HRESULT SAnimation::LoadFromFile ( CSerialFile &SFile, const DxSkeleton* pSkeleton )
{
	...
	else if ( 0x103==dwVER )	Load_0103( SFile, pSkeleton );
...
HRESULT SAnimContainer::SaveToFile ()
{
	...
	/*DWORD	dwGabageValue;
	dwGabageValue = (DWORD)m_listAniUPBODY.size() + (DWORD)m_listAnimation.size() + VERSION;
	SFile << dwGabageValue;*/

	SFile << (DWORD)m_listAnimation.size();
	
	SANILIST_ITER iter = m_listAnimation.begin();
	SANILIST_ITER iter_end = m_listAnimation.end();
	for ( ; iter!=iter_end; ++iter )
	{
		(*iter)->SaveToFile(SFile);
	}
...
//try to compile it.tell me th
//whole prj?yes..and test it in game
//okay
...
//animation v04.bin
//how bout this one?
//i will tell you..Bip01 = listANI Bip01_Spine1 = listANIUPBODY..in v4 Bip01 come first
