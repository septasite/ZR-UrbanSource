...
HRESULT CD3DMesh::SetFVF( LPDIRECT3DDEVICE8 pd3dDevice, DWORD dwFVF )
{
    ...
    if( m_pSysMemMesh )
	{
		if ( m_pSysMemMesh->GetFVF() == (D3DFVF_XYZ|D3DFVF_NORMAL|D3DFVF_TEX1) )
		{
			struct VERTEX { D3DXVECTOR3 vPos, Nor; D3DXVECTOR2 vTex; };

			DWORD dwVertices = m_pSysMemMesh->GetNumVertices();

			VERTEX* pVertices;
			m_pSysMemMesh->LockVertexBuffer ( 0L, (BYTE**)&pVertices );
		...
		else if ( m_pSysMemMesh->GetFVF() & D3DFVF_NORMAL )
		{
    ...
    if( m_pLocalMesh )
	{
		if ( m_pLocalMesh->GetFVF() == (D3DFVF_XYZ|D3DFVF_NORMAL|D3DFVF_TEX1) )
		{
			...
			VERTEX* pVertices;
			m_pLocalMesh->LockVertexBuffer ( 0L, (BYTE**)&pVertices );
			for ( DWORD i=0; i<dwVertices; ++i )
			{
				D3DXVec3Normalize ( &pVertices[i].Nor, &pVertices[i].Nor );
			}
                           ...
                           BOOL bDrawAlphaSubsets, D3DXMATRIX* pmatWorldMatrix )
{
	...
	D3DXMatrixMultiply_MM( matWorld, m_mat, matSavedWorld );
...
HRESULT CD3DFile::Render( LPDIRECT3DDEVICE8 pd3dDevice, D3DXMATRIX* pmatWorldMatrix )
{
	...
	D3DXMatrixMultiply_MM( matWorld, matSavedWorld, m_mat );
    pd3dDevice->SetTransform( D3DTS_WORLD, &matWorld );
