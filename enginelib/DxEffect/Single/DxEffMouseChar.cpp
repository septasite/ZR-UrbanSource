...
DxEffMouseChar& DxEffMouseChar::GetInstance()
{
	static DxEffMouseChar Instance;
	return Instance;
}

DxEffMouseChar::DxEffMouseChar(void) :
	m_fTimer(0.0f),
	m_pd3dDevice(NULL),
	m_bCreate(false),
	m_pEffSingleG(NULL)
...
D3DXVECTOR3 DxEffMouseChar::GetTargetIDPos ( STARGETID *pTargetID )
{
	D3DXVECTOR3 vPos;
...
HRESULT DxEffMouseChar::PassiveEffect ( const char* szFileName, const D3DXMATRIX &matTrans, const STARGETID &sTargetID )
{
...
HRESULT DxEffMouseChar::DeletePassiveEffect ( const STARGETID &sTargetID )
{
	if ( sTargetID.dwID==EMTARGET_NULL )	return E_FAIL;
...
HRESULT DxEffMouseChar::DeletePassiveEffect ( const char* szFileName, const STARGETID &sTargetID )
{
	if ( sTargetID.dwID==EMTARGET_NULL )	return E_FAIL;
...
BOOL DxEffMouseChar::FindPassiveEffect ( const char* szFileName, const STARGETID &sTargetID )
{
...
HRESULT DxEffMouseChar::NewEffGroup ( EFF_PROPGROUP* pPropGroup, EFF_PROPERTY* pTargetProp, const D3DXMATRIX &matTrans )
{
...
DxEffSingleGroup* DxEffMouseChar::NewEffGroup ( EFF_PROPGROUP* pPropGroup, const D3DXMATRIX &matTrans, const STARGETID* pTargetID )
{
	//	Note : Eff Group instance 생성.
	//
	GASSERT(pPropGroup);
...
DxEffSingleGroup* DxEffMouseChar::NewEffGroup ( const char* szFileName, const D3DXMATRIX &matTrans, const STARGETID* pTargetID )
{
...
HRESULT DxEffMouseChar::NewEffBody ( const char* szFileName, const STARGETID* pTargetID, D3DXVECTOR3 *pDir )
{
	GASSERT(pTargetID);
...
HRESULT DxEffMouseChar::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	GASSERT(pd3dDevice);
...
HRESULT DxEffMouseChar::RestoreDeviceObjects ()
{
	EFFSGMAP_ITER iter = m_mapESG.begin();
	EFFSGMAP_ITER iter_end = m_mapESG.end();
	for ( ; iter!=iter_end; ++iter )
...
HRESULT DxEffMouseChar::InvalidateDeviceObjects ()
{
	EFFSGMAP_ITER iter = m_mapESG.begin();
	EFFSGMAP_ITER iter_end = m_mapESG.end();
	for ( ; iter!=iter_end; ++iter )
...
HRESULT DxEffMouseChar::DeleteDeviceObjects ()
{
	RemoveAllEff ();
...
HRESULT DxEffMouseChar::RemoveAllEff ()
{
	m_pEffSingleG = NULL;
	m_bCreate = false;
...
HRESULT DxEffMouseChar::DeleteEff ( const char* szFile )
{
...
HRESULT DxEffMouseChar::DeleteAllEff ( const char* szFile )
{
	if ( !szFile )						return S_FALSE;
	if ( m_mapESG.empty() )				return S_FALSE;
...
void DxEffMouseChar::NewAfterEff ( DxEffSingleGroup* pEffSG )
{
...
HRESULT DxEffMouseChar::FrameMove ( float fTime, float fElapsedTime )
{
...
HRESULT DxEffMouseChar::Render ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	EFFSGMAP_ITER iter = m_mapESG.begin();
	EFFSGMAP_ITER iter_end = m_mapESG.end();
	for ( ; iter!=iter_end; ++iter )
