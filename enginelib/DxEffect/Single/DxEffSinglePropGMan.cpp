...
EFF_PROPERTY* EFF_PROPERTY::NEW_PROP ( DWORD TypeID,const char* szFileName )
{
...
HRESULT EFF_PROPERTY::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( bValid )
	{
		DWORD dwTypeID;
		SFile >> dwTypeID;
		EFF_PROPERTY* pNewProp = NEW_PROP ( dwTypeID, SFile.GetFileName() );
		GASSERT(pNewProp);
...
//Added by tobets | 11-8-2012 | upgrade EGP version.
HRESULT EFF_PROPGROUP::LoadFile ( const char* szFileName, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( bValid )
	{
		...
		EFF_PROPERTY* pNewProp = EFF_PROPERTY::NEW_PROP ( dwTypeID, szFileName );
		GASSERT(pNewProp);
