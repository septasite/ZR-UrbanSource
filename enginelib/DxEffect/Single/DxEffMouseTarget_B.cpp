...
DxEffMouseTarget_B& DxEffMouseTarget_B::GetInstance()
{
	static DxEffMouseTarget_B Instance;
	return Instance;
}

DxEffMouseTarget_B::DxEffMouseTarget_B (void) :
	m_fTimer(0.0f),
	m_pd3dDevice(NULL),
	m_pEffSingleGroup(NULL)
{
}
...
D3DXVECTOR3 DxEffMouseTarget_B::GetTargetIDPos ( STARGETID *pTargetID )
{
	D3DXVECTOR3 vPos;
...
HRESULT DxEffMouseTarget_B::PassiveEffect ( const char* szFileName, const D3DXMATRIX &matTrans, const STARGETID &sTargetID )
{
...
HRESULT DxEffMouseTarget_B::DeletePassiveEffect ( const STARGETID &sTargetID )
{
	if ( sTargetID.dwID==EMTARGET_NULL )	return E_FAIL;
...
HRESULT DxEffMouseTarget_B::DeletePassiveEffect ( const char* szFileName, const STARGETID &sTargetID )
{
	if ( sTargetID.dwID==EMTARGET_NULL )	return E_FAIL;
...
BOOL DxEffMouseTarget_B::FindPassiveEffect ( const char* szFileName, const STARGETID &sTargetID )
{
...
HRESULT DxEffMouseTarget_B::NewEffGroup ( EFF_PROPGROUP* pPropGroup, EFF_PROPERTY* pTargetProp, const D3DXMATRIX &matTrans )
{
...
DxEffSingleGroup* DxEffMouseTarget_B::NewEffGroup ( EFF_PROPGROUP* pPropGroup, const D3DXMATRIX &matTrans, const STARGETID* pTargetID )
{
	//	Note : Eff Group instance 생성.
	//
	GASSERT(pPropGroup);

	DxEffSingleGroup* pEffSingleGroup;

	if ( m_pEffSingleGroup )
	{
		m_pEffSingleGroup->m_matWorld = matTrans;
		m_pEffSingleGroup->SetTargetID ( pTargetID );
	}
	else
	{
		pEffSingleGroup = pPropGroup->NEWEFFGROUP ();
		...
		m_mapESG.insert ( std::make_pair(sTargetID,pEffSingleGroup) );
		m_pEffSingleGroup = pEffSingleGroup;
	}
	return pEffSingleGroup;
}

DxEffSingleGroup* DxEffMouseTarget_B::NewEffGroup ( const char* szFileName, const D3DXMATRIX &matTrans , const STARGETID* pTargetID )
{
...
HRESULT DxEffMouseTarget_B::NewEffBody ( const char* szFileName, const STARGETID* pTargetID, D3DXVECTOR3 *pDir )
{
	GASSERT(pTargetID);
...
HRESULT DxEffMouseTarget_B::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	GASSERT(pd3dDevice);
...
HRESULT DxEffMouseTarget_B::RestoreDeviceObjects ()
{
	EFFSGMAP_ITER iter = m_mapESG.begin();
	EFFSGMAP_ITER iter_end = m_mapESG.end();
	for ( ; iter!=iter_end; ++iter )
...
HRESULT DxEffMouseTarget_B::InvalidateDeviceObjects ()
{
	EFFSGMAP_ITER iter = m_mapESG.begin();
	EFFSGMAP_ITER iter_end = m_mapESG.end();
	for ( ; iter!=iter_end; ++iter )
...
HRESULT DxEffMouseTarget_B::DeleteDeviceObjects ()
{
	RemoveAllEff ();
...
HRESULT DxEffMouseTarget_B::RemoveAllEff ()
{
	m_pEffSingleGroup = NULL;
...
HRESULT DxEffMouseTarget_B::DeleteEff ( const char* szFile )
{
...
HRESULT DxEffMouseTarget_B::DeleteAllEff ( const char* szFile )
{
	if ( !szFile )						return S_FALSE;
	if ( m_mapESG.empty() )				return S_FALSE;
...
void DxEffMouseTarget_B::NewAfterEff ( DxEffSingleGroup* pEffSG )
{
...
HRESULT DxEffMouseTarget_B::FrameMove ( float fTime, float fElapsedTime )
{
...
HRESULT	DxEffMouseTarget_B::Render ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	EFFSGMAP_ITER iter = m_mapESG.begin();
	EFFSGMAP_ITER iter_end = m_mapESG.end();
	for ( ; iter!=iter_end; ++iter )
