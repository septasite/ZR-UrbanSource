...
DxEffMouseCrow& DxEffMouseCrow::GetInstance()
{
	static DxEffMouseCrow Instance;
	return Instance;
}

DxEffMouseCrow::DxEffMouseCrow (void) :
	m_fTimer(0.0f),
	m_pd3dDevice(NULL),
...
D3DXVECTOR3 DxEffMouseCrow::GetTargetIDPos ( STARGETID *pTargetID )
{
	D3DXVECTOR3 vPos;
...
HRESULT DxEffMouseCrow::PassiveEffect ( const char* szFileName, const D3DXMATRIX &matTrans, const STARGETID &sTargetID )
{
...
HRESULT DxEffMouseCrow::DeletePassiveEffect ( const STARGETID &sTargetID )
{
	if ( sTargetID.dwID==EMTARGET_NULL )	return E_FAIL;
...
HRESULT DxEffMouseCrow::DeletePassiveEffect ( const char* szFileName, const STARGETID &sTargetID )
{
	if ( sTargetID.dwID==EMTARGET_NULL )	return E_FAIL;
...
BOOL DxEffMouseCrow::FindPassiveEffect ( const char* szFileName, const STARGETID &sTargetID )
{
...
HRESULT DxEffMouseCrow::NewEffGroup ( EFF_PROPGROUP* pPropGroup, EFF_PROPERTY* pTargetProp, const D3DXMATRIX &matTrans )
{
...
DxEffSingleGroup* DxEffMouseCrow::NewEffGroup ( EFF_PROPGROUP* pPropGroup, const D3DXMATRIX &matTrans, const STARGETID* pTargetID )
{
	//	Note : Eff Group instance 생성.
	//
	GASSERT(pPropGroup);
...
DxEffSingleGroup* DxEffMouseCrow::NewEffGroup ( const char* szFileName, const D3DXMATRIX &matTrans , const STARGETID* pTargetID )
{
...
HRESULT DxEffMouseCrow::NewEffBody ( const char* szFileName, const STARGETID* pTargetID, D3DXVECTOR3 *pDir )
{
	GASSERT(pTargetID);
...
HRESULT DxEffMouseCrow::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	GASSERT(pd3dDevice);
...
HRESULT DxEffMouseCrow::RestoreDeviceObjects ()
{
	EFFSGMAP_ITER iter = m_mapESG.begin();
	EFFSGMAP_ITER iter_end = m_mapESG.end();
	for ( ; iter!=iter_end; ++iter )
...
HRESULT DxEffMouseCrow::InvalidateDeviceObjects ()
{
	EFFSGMAP_ITER iter = m_mapESG.begin();
	EFFSGMAP_ITER iter_end = m_mapESG.end();
	for ( ; iter!=iter_end; ++iter )
...
HRESULT DxEffMouseCrow::DeleteDeviceObjects ()
{
	RemoveAllEff ();
...
HRESULT DxEffMouseCrow::RemoveAllEff ()
{
	m_pEffSingleGroup = NULL;
...
HRESULT DxEffMouseCrow::DeleteEff ( const char* szFile )
{
...
HRESULT DxEffMouseCrow::DeleteAllEff ( const char* szFile )
{
	if ( !szFile )						return S_FALSE;
	if ( m_mapESG.empty() )				return S_FALSE;
...
void DxEffMouseCrow::NewAfterEff ( DxEffSingleGroup* pEffSG )
{
...
HRESULT DxEffMouseCrow::FrameMove ( float fTime, float fElapsedTime )
{
...
HRESULT	DxEffMouseCrow::Render ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	EFFSGMAP_ITER iter = m_mapESG.begin();
	EFFSGMAP_ITER iter_end = m_mapESG.end();
	for ( ; iter!=iter_end; ++iter )
