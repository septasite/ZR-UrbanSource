...
DxEffMouseTarget_A& DxEffMouseTarget_A::GetInstance()
{
	static DxEffMouseTarget_A Instance;
	return Instance;
}

DxEffMouseTarget_A::DxEffMouseTarget_A (void) :
	m_fTimer(0.0f),
	m_pd3dDevice(NULL),
...
D3DXVECTOR3 DxEffMouseTarget_A::GetTargetIDPos ( STARGETID *pTargetID )
{
	D3DXVECTOR3 vPos;
...
HRESULT DxEffMouseTarget_A::PassiveEffect ( const char* szFileName, const D3DXMATRIX &matTrans, const STARGETID &sTargetID )
{
...
HRESULT DxEffMouseTarget_A::DeletePassiveEffect ( const STARGETID &sTargetID )
{
	if ( sTargetID.dwID==EMTARGET_NULL )	return E_FAIL;
...
HRESULT DxEffMouseTarget_A::DeletePassiveEffect ( const char* szFileName, const STARGETID &sTargetID )
{
	if ( sTargetID.dwID==EMTARGET_NULL )	return E_FAIL;
...
BOOL DxEffMouseTarget_A::FindPassiveEffect ( const char* szFileName, const STARGETID &sTargetID )
{
...
HRESULT DxEffMouseTarget_A::NewEffGroup ( EFF_PROPGROUP* pPropGroup, EFF_PROPERTY* pTargetProp, const D3DXMATRIX &matTrans )
{
...
DxEffSingleGroup* DxEffMouseTarget_A::NewEffGroup ( EFF_PROPGROUP* pPropGroup, const D3DXMATRIX &matTrans, const STARGETID* pTargetID )
{
	//	Note : Eff Group instance 생성.
	//
	GASSERT(pPropGroup);
...
DxEffSingleGroup* DxEffMouseTarget_A::NewEffGroup ( const char* szFileName, const D3DXMATRIX &matTrans , const STARGETID* pTargetID )
{
...
HRESULT DxEffMouseTarget_A::NewEffBody ( const char* szFileName, const STARGETID* pTargetID, D3DXVECTOR3 *pDir )
{
	GASSERT(pTargetID);
...
HRESULT DxEffMouseTarget_A::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	GASSERT(pd3dDevice);
...
HRESULT DxEffMouseTarget_A::RestoreDeviceObjects ()
{
	EFFSGMAP_ITER iter = m_mapESG.begin();
	EFFSGMAP_ITER iter_end = m_mapESG.end();
	for ( ; iter!=iter_end; ++iter )
...
HRESULT DxEffMouseTarget_A::InvalidateDeviceObjects ()
{
	EFFSGMAP_ITER iter = m_mapESG.begin();
	EFFSGMAP_ITER iter_end = m_mapESG.end();
	for ( ; iter!=iter_end; ++iter )
...
HRESULT DxEffMouseTarget_A::DeleteDeviceObjects ()
{
	RemoveAllEff ();
...
HRESULT DxEffMouseTarget_A::RemoveAllEff ()
{
	m_pEffSingleGroup = NULL;
...
HRESULT DxEffMouseTarget_A::DeleteEff ( const char* szFile )
{
...
HRESULT DxEffMouseTarget_A::DeleteAllEff ( const char* szFile )
{
	if ( !szFile )						return S_FALSE;
	if ( m_mapESG.empty() )				return S_FALSE;
...
void DxEffMouseTarget_A::NewAfterEff ( DxEffSingleGroup* pEffSG )
{
...
HRESULT DxEffMouseTarget_A::FrameMove ( float fTime, float fElapsedTime )
{
...
HRESULT	DxEffMouseTarget_A::Render ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	EFFSGMAP_ITER iter = m_mapESG.begin();
	EFFSGMAP_ITER iter_end = m_mapESG.end();
	for ( ; iter!=iter_end; ++iter )
