...
#include "./DxEffTargetCrow.h"

#ifdef _DEBUG
#define new DEBUG_NEW
#endif

DxEffTargetCrow& DxEffTargetCrow::GetInstance()
{
	static DxEffTargetCrow Instance;
	return Instance;
}

DxEffTargetCrow::DxEffTargetCrow(void) :
	m_fTimer(0.0f),
	m_pd3dDevice(NULL),
	m_bCreate(false),
	m_pEffSingleG(NULL)
...
D3DXVECTOR3 DxEffTargetCrow::GetTargetIDPos ( STARGETID *pTargetID )
{
	D3DXVECTOR3 vPos;
...
HRESULT DxEffTargetCrow::PassiveEffect ( const char* szFileName, const D3DXMATRIX &matTrans, const STARGETID &sTargetID )
{
...
HRESULT DxEffTargetCrow::DeletePassiveEffect ( const STARGETID &sTargetID )
{
	if ( sTargetID.dwID==EMTARGET_NULL )	return E_FAIL;
...
HRESULT DxEffTargetCrow::DeletePassiveEffect ( const char* szFileName, const STARGETID &sTargetID )
{
	if ( sTargetID.dwID==EMTARGET_NULL )	return E_FAIL;
...
BOOL DxEffTargetCrow::FindPassiveEffect ( const char* szFileName, const STARGETID &sTargetID )
{
...
HRESULT DxEffTargetCrow::NewEffGroup ( EFF_PROPGROUP* pPropGroup, EFF_PROPERTY* pTargetProp, const D3DXMATRIX &matTrans )
{
...
DxEffSingleGroup* DxEffTargetCrow::NewEffGroup ( EFF_PROPGROUP* pPropGroup, const D3DXMATRIX &matTrans, const STARGETID* pTargetID )
{
	//	Note : Eff Group instance 생성.
	//
	GASSERT(pPropGroup);
...
DxEffSingleGroup* DxEffTargetCrow::NewEffGroup ( const char* szFileName, const D3DXMATRIX &matTrans, const STARGETID* pTargetID )
{
...
HRESULT DxEffTargetCrow::NewEffBody ( const char* szFileName, const STARGETID* pTargetID, D3DXVECTOR3 *pDir )
{
	GASSERT(pTargetID);
...
HRESULT DxEffTargetCrow::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	GASSERT(pd3dDevice);
...
HRESULT DxEffTargetCrow::RestoreDeviceObjects ()
{
	EFFSGMAP_ITER iter = m_mapESG.begin();
	EFFSGMAP_ITER iter_end = m_mapESG.end();
	for ( ; iter!=iter_end; ++iter )
...
HRESULT DxEffTargetCrow::InvalidateDeviceObjects ()
{
	EFFSGMAP_ITER iter = m_mapESG.begin();
	EFFSGMAP_ITER iter_end = m_mapESG.end();
	for ( ; iter!=iter_end; ++iter )
...
HRESULT DxEffTargetCrow::DeleteDeviceObjects ()
{
	RemoveAllEff ();
...
HRESULT DxEffTargetCrow::RemoveAllEff ()
{
	m_pEffSingleG = NULL;
	m_bCreate = false;
...
HRESULT DxEffTargetCrow::DeleteEff ( const char* szFile )
{
...
HRESULT DxEffTargetCrow::DeleteAllEff ( const char* szFile )
{
	if ( !szFile )						return S_FALSE;
	if ( m_mapESG.empty() )				return S_FALSE;
...
void DxEffTargetCrow::NewAfterEff ( DxEffSingleGroup* pEffSG )
{
...
HRESULT DxEffTargetCrow::FrameMove ( float fTime, float fElapsedTime )
{
...
HRESULT DxEffTargetCrow::Render ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	EFFSGMAP_ITER iter = m_mapESG.begin();
	EFFSGMAP_ITER iter_end = m_mapESG.end();
	for ( ; iter!=iter_end; ++iter )
