...
DxTaregetEffect& DxTaregetEffect::GetInstance()
{
	static DxTaregetEffect Instance;
	return Instance;
}

DxTaregetEffect::DxTaregetEffect (void) :
	m_fTimer(0.0f),
	m_pd3dDevice(NULL),
...
D3DXVECTOR3 DxTaregetEffect::GetTargetIDPos ( STARGETID *pTargetID )
{
	D3DXVECTOR3 vPos;
...
HRESULT DxTaregetEffect::PassiveEffect ( const char* szFileName, const D3DXMATRIX &matTrans, const STARGETID &sTargetID )
{
...
HRESULT DxTaregetEffect::DeletePassiveEffect ( const STARGETID &sTargetID )
{
	if ( sTargetID.dwID==EMTARGET_NULL )	return E_FAIL;
...
HRESULT DxTaregetEffect::DeletePassiveEffect ( const char* szFileName, const STARGETID &sTargetID )
{
	if ( sTargetID.dwID==EMTARGET_NULL )	return E_FAIL;
...
BOOL DxTaregetEffect::FindPassiveEffect ( const char* szFileName, const STARGETID &sTargetID )
{
...
HRESULT DxTaregetEffect::NewEffGroup ( EFF_PROPGROUP* pPropGroup, EFF_PROPERTY* pTargetProp, const D3DXMATRIX &matTrans )
{
...
DxEffSingleGroup* DxTaregetEffect::NewEffGroup ( EFF_PROPGROUP* pPropGroup, const D3DXMATRIX &matTrans, const STARGETID* pTargetID )
{
	//	Note : Eff Group instance 생성.
	//
	GASSERT(pPropGroup);
...
DxEffSingleGroup* DxTaregetEffect::NewEffGroup ( const char* szFileName, const D3DXMATRIX &matTrans , const STARGETID* pTargetID )
{
...
HRESULT DxTaregetEffect::NewEffBody ( const char* szFileName, const STARGETID* pTargetID, D3DXVECTOR3 *pDir )
{
	GASSERT(pTargetID);
...
HRESULT DxTaregetEffect::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	GASSERT(pd3dDevice);
...
HRESULT DxTaregetEffect::RestoreDeviceObjects ()
{
	EFFSGMAP_ITER iter = m_mapESG.begin();
	EFFSGMAP_ITER iter_end = m_mapESG.end();
	for ( ; iter!=iter_end; ++iter )
...
HRESULT DxTaregetEffect::InvalidateDeviceObjects ()
{
	EFFSGMAP_ITER iter = m_mapESG.begin();
	EFFSGMAP_ITER iter_end = m_mapESG.end();
	for ( ; iter!=iter_end; ++iter )
...
HRESULT DxTaregetEffect::DeleteDeviceObjects ()
{
	RemoveAllEff ();
...
HRESULT DxTaregetEffect::RemoveAllEff ()
{
	m_pEffSingleGroup = NULL;
...
HRESULT DxTaregetEffect::DeleteEff ( const char* szFile )
{
...
HRESULT DxTaregetEffect::DeleteAllEff ( const char* szFile )
{
	if ( !szFile )						return S_FALSE;
	if ( m_mapESG.empty() )				return S_FALSE;
...
void DxTaregetEffect::NewAfterEff ( DxEffSingleGroup* pEffSG )
{
...
HRESULT DxTaregetEffect::FrameMove ( float fTime, float fElapsedTime )
{
...
HRESULT	DxTaregetEffect::Render ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	EFFSGMAP_ITER iter = m_mapESG.begin();
	EFFSGMAP_ITER iter_end = m_mapESG.end();
	for ( ; iter!=iter_end; ++iter )
