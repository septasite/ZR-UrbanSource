...
DWORD		DxEffCharTotalShader::VERSION			= 0x0100;
DWORD		DxEffCharTotalShader::VERSION_SAVE		= 0x0100;
char		DxEffCharTotalShader::NAME[MAX_PATH]	= "EffCharNormal";
...
DxEffChar* DxEffCharTotalShader::CloneInstance ( LPDIRECT3DDEVICEQ pd3dDevice, DxCharPart* pCharPart, DxSkinPiece* pSkinPiece )
{
	HRESULT hr;
...
void DxEffCharTotalShader::Render( LPDIRECT3DDEVICEQ pd3dDevice, BOOL bPieceRender )
{
	if( !m_pmcMesh )		return;
...
HRESULT	DxEffCharTotalShader::LoadFile ( basestream &SFile, LPDIRECT3DDEVICEQ pd3dDevice,bool bTool,bool bRan2 )
{
	DWORD dwVer, dwSize;
	...
	{
		CString strTemp = "";
		strTemp.Format( "DxEffCharTotalShader::LoadFile Error! Name:%s Version:%d Size:%d Current Version:%d" ,SFile.GetFileName(),dwVer,dwSize,VERSION);
...
HRESULT	DxEffCharTotalShader::SaveFile ( basestream &SFile,bool bTool )
{
	SFile << TYPEID;
	SFile << VERSION_SAVE;
