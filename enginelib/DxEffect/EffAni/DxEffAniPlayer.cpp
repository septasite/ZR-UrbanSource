...
//add abl
HRESULT	DxEffAniPlayer::Render ( const LPDIRECT3DDEVICEQ pd3dDevice, DxSkinCharPiece* pSkinCharPiece, const D3DXMATRIX& matCurPos )
{
	EFFLIST_ITER iter = m_listAni.begin();
	EFFLIST_ITER iter_end = m_listAni.end();
	for ( ; iter!=iter_end; ++iter )
	{
		(*iter)->Render ( pd3dDevice, pSkinCharPiece, matCurPos );
	}
	return S_OK;
}

//add vcf
HRESULT	DxEffAniPlayer::Render ( const LPDIRECT3DDEVICEQ pd3dDevice, DxSkinVehicle* pSkinVehicle, const D3DXMATRIX& matCurPos )
{
	EFFLIST_ITER iter = m_listAni.begin();
	EFFLIST_ITER iter_end = m_listAni.end();
	for ( ; iter!=iter_end; ++iter )
	{
		(*iter)->Render ( pd3dDevice, pSkinVehicle, matCurPos );
	}
	return S_OK;
}
