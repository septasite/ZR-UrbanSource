		...
		if ( m_dTotalColumns != 0 )
		{
			...
			while ( !m_rSheet->IsEOF() )
			{
				...
				try
				{
					...
					{
						// CODBCFieldInfo
						m_rSheet->GetFieldValue ( column, varValue );
						m_stempSql = "";

						// string
						switch (varValue.m_dwType)
						{
						case DBVT_NULL: // error
							m_stempSql = "DBVT_NULL";
							break;

						case DBVT_BOOL: // bool
							m_stempSql.Format ( "%d", varValue.m_boolVal );
							break;

						case DBVT_UCHAR: // Unicode char
							m_stempSql = *varValue.m_pstringW;
							break;

						case DBVT_SHORT:
							m_stempSql.Format ( "%d", varValue.m_iVal );
							break;

						case DBVT_LONG:
							m_stempSql.Format ( "%d", varValue.m_lVal );
							break;

						case DBVT_SINGLE:
							m_stempSql = "DBVT_SINGLE";
							break;

						case DBVT_DOUBLE:
							{
								char szTemp[260];
								_gcvt ( varValue.m_dblVal, 12, szTemp );
								m_stempSql = szTemp;
							}
							break;

						case DBVT_DATE: // Time stamp ??
							m_stempSql = "DBVT_DATE";
							break;

						case DBVT_STRING:
							m_stempSql = *varValue.m_pstring;
							break;

						case DBVT_BINARY:
							m_stempSql = "DBVT_BINARY";
							break;
