...
namespace RUNTIME
{
	...
	{
		pClass;
		_ASSERTE(pObject->IsKindOf(pClass));
	...
	{
		return (pObject->IsKindOf(pClass) ? static_cast<CLASS*>(pObject) : NULL);
	}
	...
	{
		return (pObject->IsKindOf(pClass) ? static_cast<const CLASS*>(pObject) : NULL);
	}
}
