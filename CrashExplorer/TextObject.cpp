...
bool CTextObject::SetWideObjectText(PCWSTR pszWideText)
{
	if (m_pTextProvider == NULL)
	{
		...
		if (pszWideText != NULL)
		{
			...
			pszNewText = new WCHAR[nTextSize];
			if (pszNewText == NULL)
				return false;
...
PWSTR CTextObject::AllocWideText(PCSTR pszAnsiText)
{
	...
	PWSTR pszWideText = new WCHAR[nTextLength + 1];
	if (pszWideText != NULL)
	{
...
PSTR CTextObject::AllocAnsiText(PCWSTR pszWideText)
{
	...
	PSTR pszAnsiText = new CHAR[nTextLength + 1];
	if (pszAnsiText != NULL)
	{
...
bool CTextObject::ReadDataFromFile(PCTSTR pszFileName, CLIPFORMAT cfFormat)
{
	...
	case CF_TEXT:
		{
			PSTR pszText = new CHAR[nDataSize + 1];
			if (pszText != NULL)
			{
	...
	case CF_UNICODETEXT:
		{
			PWSTR pszText = new WCHAR[nDataSize + 1];
			if (pszText != NULL)
			{
				if (ReadFile(hDataFile, pszText, nDataSize, &nNumRead, NULL))
				{
