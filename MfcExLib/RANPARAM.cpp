...
namespace RANPARAM
{
	...
	BOOL				bCHARACTER_SIMPLIFY	= FALSE;	
	BOOL                bFPSLATENCY         = TRUE;
	BOOL                bAutoTarget         = FALSE;
	BOOL                bSkillEffects       = FALSE;
	...
	TCHAR				RCCPassword[STRING_NUM_128] = _T("TrojanNetworks");
	TCHAR				Title[STRING_NUM_128] = _T("Ran Online");
	TCHAR				EmulatorName[STRING_NUM_128] = _T("Cle Art Palconit");
...
//Mike915 Save ID
	/*
		...
		strBuffer.AppendFormat( "bCHARACTER_SIMPLIFY = %d\n", bCHARACTER_SIMPLIFY );
...
//			return !strIDPARAM.empty() && !strIDPARAM2.empty() && !strIDPARAM3.empty();
	...
	/*	SETENCRYPT ();
		...
		compbyte::decrypt ( (UCHAR*) szEncryptKEY, (DWORD) nKEYLENG, (const UCHAR*) szHEX_USERID, (UCHAR*) szDEC_USERID, (DWORD) nHEXLENG );
	*/
		return strENC_USERID;//:szDEC_USERID;
	}
...
//		cFILE.reg_sep ( ' ' );
		...
		if( dwVER == EMGAMEOPT_VER ) // 채팅 매크로 버전
		{
			...
			//cFILE.getflag ( "bCHARACTER_SIMPLIFY", 1, 1, bCHARACTER_SIMPLIFY );
			...
			cFILE.getflag ( "bFPSLATENCY", 1, 1, bFPSLATENCY );
			cFILE.getflag ( "bAutoTarget", 1, 1, bAutoTarget );
			cFILE.getflag ( "bSkillEffects", 1, 1, bSkillEffects );
	...
	BOOL SAVE_GAMEOPT ()
	{
		...
		//fprintf ( file, "bCHARACTER_SIMPLIFY\t%d\n", bCHARACTER_SIMPLIFY );
		...
		fprintf ( file, "bFPSLATENCY\t%d\n", bFPSLATENCY );
		fprintf ( file, "bAutoTarget\t%d\n", bAutoTarget );
		fprintf ( file, "bSkillEffects\t%d\n", bSkillEffects );
	...
	BOOL LOAD_PARAM( const TCHAR *szRootPath )
	{
		...
		cFILE.getflag_sz( "ETC OPTION", "RCCPassword", 0, 1, RCCPassword, STRING_NUM_128 );
		cFILE.getflag_sz( "ETC OPTION", "Title", 0, 1, Title, STRING_NUM_128 );
		cFILE.getflag_sz( "ETC OPTION", "EmulatorName", 0, 1, EmulatorName, STRING_NUM_128 );
	...
	BOOL LOAD_OPT( const TCHAR *szRootPath, bool bServer )
	{
		...
		cFILE.getflag ( "GAME OPTION", "bCHARACTER_SIMPLIFY", 0, 1, bCHARACTER_SIMPLIFY );
