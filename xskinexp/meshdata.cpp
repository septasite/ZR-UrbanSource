    ...
    )
{
    ...
    for( iFace = 0; iFace < pMeshData->m_cFaces; iFace++ )
    {
        for( iPoint = 0; iPoint < 3; iPoint++ ) // vertex indices
        {
			...
			//	EDITED : (pMesh->faces[iFace].flags & HAS_TVERTS) 체크를 하게 되어 있었으나,
			//			이 속성이 없을때 텍스처 uv좌표가 출력이 되지 않는다.
			//if ( (pMesh->faces[iFace].flags & HAS_TVERTS)
			//	&& (NULL != pMesh->tvFace)
			//	&& ((int)pMesh->tvFace[iFace].t[iPoint] < pMesh->numTVerts) )

            if ( (NULL != pMesh->tvFace )
                    && ((int)pMesh->tvFace[iFace].t[iPoint] < pMesh->numTVerts) )
            {
            ...
            else
            {
                ...
                if (!bFound)
                {
                    ...
                    if (pMeshData->m_cVertices == cVerticesMax)
                    {
						...
						//	EDITED : 버텍스 배열 제 할당시에 추가 할당 부분을 초기화 해주어야
						//			한다.
						for ( int i = cbackVertMax-1; i < cVerticesMax; i++ )
						{
							rgVerticesNew[i].iPointRep = i;
							rgVerticesNew[i].iWedgeList = i;

							// default values that should be reset if the vertex is actually used by a face...
							rgVerticesNew[i].vNormal.x = 0;
							rgVerticesNew[i].vNormal.y = 0;
							rgVerticesNew[i].vNormal.z = 0;
							rgVerticesNew[i].iSmoothingGroupIndex = 0;
							rgVerticesNew[i].iTextureIndex = 0;
							rgVerticesNew[i].iMaterial = 0;
						}
