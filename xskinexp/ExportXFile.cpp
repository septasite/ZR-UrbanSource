    ...
    if (bDetailMap)
    {
        ...
        else
        {
			...
			if ( psc->m_bTexEncrypt )
			{
				...
				strcpy( szTextureOutName, findTextureFilename.m_szTextureName );
				char* szTextureFileExt = strrchr ( szTextureOutName, '.' );
				if ( szTextureFileExt ) strcpy( szTextureFileExt, ".mtf" );

				EncryptTexture( findTextureFilename.m_szTextureName, szTextureOutName );
			}

			//	EDITED : 텍스쳐 이름 수정.
			//
			char *szTextureFileName = strrchr ( findTextureFilename.m_szTextureName, '\\' );
			if ( szTextureFileName )
			{
				szTextureFileName = szTextureFileName + 1;	//	'\\' 부분 삭제.
			}
			...
			{
				szTextureFileName = findTextureFilename.m_szTextureName;
			}
