...
{	
	...
	for ( int i = nPartIndex; i < nPartSize; ++i )
	{
		...
		for ( int j = nLineIndex; j < nLineCount; ++j )
		{
			...
			fTEXT_POS_Y += fSizeY;				//	Post line
			if ( j != (nLineCount-1) ) fTEXT_POS_Y += GetLineInterval();	//	Inter-line
			...
			if ( m_nAlign & TEXT_ALIGN_BOTH_X && 1 < vectorTextWord.size() && !m_bChat )
			{
				...
				//	NOTE
				//		The first line is not posted intentionally
				//		But if its empty output..
				...
				//	NOTE
				//		If the last column contains spaces, it does not output				
...
int	CBasicTextBox::AddTextChat ( STEXTCHAT strText, const D3DCOLOR& TextColor )
{
	if ( !m_pFont )
	{
		GASSERT ( 0 && "폰트가 널입니다. SetFont()를 이용하여 폰트부터 값을 넣으십시오." );
		...
		else
		{
			...
			else
			{
				...
				if ( m_bChat  && i == 0 && strText.strName.GetLength() )
				{
					...
					sTextWord.dwColor = D3DCOLOR_ARGB(0xFF,0x00,0xA8,0xFF);
					m_pFont->GetTextExtent ( sTextWord.strWord.GetString(), strSize );
					sTextWord.strSize = strSize;
