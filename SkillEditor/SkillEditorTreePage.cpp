...
void SkillEditorTreePage::RunSelectMenu ( int nSelect )
{
	...
	case ID_MENU_SKILLDATAEDIT:
		{
			if ( GetSelectSkill () )
			{				
				m_pSheetTab->ActiveSkillBasicPage ( SKILLBASICPAGE, pSkill,0,0);
			}			
		}
		break;
	case ID_MENU_SKILLLEVELEDIT:
		{
			if ( GetSelectSkill () )
			{
				m_pSheetTab->ActiveSkillLevelPage ( SKILLLEVELPAGE, pSkill,0,0 );
			}			
		}
		break;
	case ID_MENU_ADD:
	...
	case ID_MENU_JDEV:
		{
			MessageBox ( "If you Found Bugs, you can contact me Yahoo ID: i990x@yahoo.com", "Hi!", MB_OK );
		}
		break;
	};
}
