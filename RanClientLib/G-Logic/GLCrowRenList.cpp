...
void CROWREN::SETTYPEFLAG ( GLCharacter *pMYCHAR, GLCharClient *pCHAR )
{
	...
	PLANDMANCLIENT pLAND = GLGaeaClient::GetInstance().GetActiveMap();
	if ( pLAND->m_bRoyalRumble ) m_dwCLUB = CLUB_NULL; m_dwCLUB_MARK_VER = 0;
}
...
void CROWREN::SETTYPEFLAG ( GLCharacter *pMYCHAR, GLCrowClient *pCROW )
{
	...
	//add mobschoolreq
	else if ( !(pCROW->m_pCrowData->m_sAction.dwMobReqSchool & index2school(pMYCHAR->m_wSchool) ) ) 
	{
		m_dwCOLOR = CROWCOLOR::MOB_COLOR_GRAY;
	}
	//else if ( !(pCROW->m_pCrowData->m_wSP == index2school(pMYCHAR->m_wSchool) ) ) 
	//{
	//	m_dwCOLOR = CROWCOLOR::MOB_COLOR_IDNAMECOLOR;
	//}
	else
	{
...
void CROWREN::SETTYPEFLAG ( GLCharacter *pMYCHAR, CItemClientDrop *pITEM ) //added color item name by drops
{
	SITEM *pITEM_DATA = GLItemMan::GetInstance().GetItem ( pITEM->sItemClient.sNativeID );
	if ( pITEM_DATA->sBasicOp.emLevel == LEVEL_NORMAL )
	{
		m_dwCOLOR = NS_UITEXTCOLOR::WHITE;
	}
	else if ( pITEM_DATA->sBasicOp.emLevel == LEVEL_RARE || LEVEL_VERYRARE)
	{
		m_dwCOLOR = NS_UITEXTCOLOR::VIOLET;
	}
	else
	{
		m_dwCOLOR = NS_UITEXTCOLOR::WHITE;
