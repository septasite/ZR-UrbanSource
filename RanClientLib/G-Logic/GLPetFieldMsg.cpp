...
{
	//dmk14 | 11-21-16 | dual pet skill update
	...
	if ( m_sActiveSkillID[pNetMsg->nIndex] != NATIVEID_NULL() )
	{
		if( m_sActiveSkillID[pNetMsg->nIndex].wMainID != 26 )
		{
			CDebugSet::ToLogFile( "ERR : GLPetField::MsgChangeActiveSkill_01, MID = %d, SID = %d",  
																		m_sActiveSkillID[pNetMsg->nIndex].wMainID,
																		m_sActiveSkillID[pNetMsg->nIndex].wSubID );
		}

		// DB에 스킬 변경 저장
		PETSKILL sPetSkill( m_sActiveSkillID[pNetMsg->nIndex], 0 );
		CSetPetSkill* pDbAction = new CSetPetSkill ( m_pOwner->m_dwCharID, m_dwPetID, sPetSkill, false, pNetMsg->nIndex );
	...
	if ( pNetMsg->sSkillID == NATIVEID_NULL() )
	{
		...
		// 보조스킬 팩터에 추가
		m_pOwner->m_sPETSKILLFACT[pNetMsg->nIndex].RESET ();
		...
		NetMsgBRD.nIndex   = pNetMsg->nIndex;	//dmk14 | 11-21-16 | dual pet skill
		m_pOwner->SendMsgViewAround ( ( NET_MSG_GENERIC* ) &NetMsgBRD );
		...
		m_sActiveSkillID[pNetMsg->nIndex] = pNetMsg->sSkillID;
	...
	if( s == 1 && !m_bDualSkill )					break;
	if ( m_sActiveSkillID[s] == NATIVEID_NULL() )	continue;

	PGLSKILL pSkill = GLSkillMan::GetInstance().GetData ( m_sActiveSkillID[s] );
	if ( !pSkill )	return E_FAIL;

	PETSKILL_MAP_ITER iter = m_ExpSkills.find ( m_sActiveSkillID[s].dwID );
	...
	// 보조스킬 팩터에 추가
	m_pOwner->m_sPETSKILLFACT[s].emTYPE = pSkill->m_sAPPLY.emBASIC_TYPE;
	m_pOwner->m_sPETSKILLFACT[s].fMVAR = sSKILL_DATA.fBASIC_VAR;
	m_pOwner->m_sPETSKILLFACT[s].fAGE = sSKILL_DATA.fLIFE;
	m_pOwner->m_sPETSKILLFACT[s].sNATIVEID = m_sActiveSkillID[s];

	// 클라이언트에 팩터 추가 알림
	GLMSG::SNETPET_ADD_SKILLFACT NetMsgAddSkill;
	//dmk14 | 11-21-16 | dual pet skill
	
	NetMsgAddSkill.sNATIVEID = m_sActiveSkillID[s];
	...
	NetMsgAddSkill.nIndex	 = s;
	GLGaeaServer::GetInstance().SENDTOCLIENT(m_pOwner->m_dwClientID,&NetMsgAddSkill);

	// DB에 스킬 변경 저장
	CSetPetSkill* pDbAction = new CSetPetSkill ( m_pOwner->m_dwCharID, m_dwPetID, sPetSkill, true, s );
	GLDBMan* pDBMan = GLGaeaServer::GetInstance().GetDBMan ();
	if ( pDBMan ) pDBMan->AddJob ( pDbAction );
	}
