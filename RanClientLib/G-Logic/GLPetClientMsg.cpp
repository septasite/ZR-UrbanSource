...
void GLPetClient::MsgSkillChange ( NET_MSG_GENERIC* nmg )
{
	...
	m_pOwner->m_sPETSKILLFACT[pNetMsg->nIndex].sNATIVEID	= pNetMsg->sNATIVEID;
	m_pOwner->m_sPETSKILLFACT[pNetMsg->nIndex].emTYPE		= pNetMsg->emTYPE;
	m_pOwner->m_sPETSKILLFACT[pNetMsg->nIndex].fMVAR		= pNetMsg->fMVAR;
	m_pOwner->m_sPETSKILLFACT[pNetMsg->nIndex].fAGE			= pNetMsg->fAge;

	// 이전 이펙트 제거
	//dmk14 | 11-21-16 | dual pet skill
	for( unsigned s=0; s<PETSLOT_SIZE;++s) 
	FACTEFF::DeleteSkillFactEffect ( STARGETID(CROW_PC,m_pOwner->m_dwGaeaID,m_pOwner->GetPosition() ), m_pOwner->GetSkinChar (), m_sActiveSkillID[s] );
	
	//dmk14 | 11-21-16 | dual pet skill
	m_sActiveSkillID[pNetMsg->nIndex] = pNetMsg->sNATIVEID;

	//	지속형 스킬의 경우 스킬 팩터 추가됨.
	//dmk14 | 11-21-16 | dual pet skill
	for( unsigned s=0; s<PETSLOT_SIZE;++s) {
	if ( m_sActiveSkillID[s] != SNATIVEID(false) )
	{
		// 주인이 타겟이된다.
		STARGETID sTargetID( CROW_PC, m_pOwner->m_dwGaeaID, m_pOwner->GetPosition () );
		FACTEFF::NewSkillFactEffect ( sTargetID, m_pOwner->GetSkinChar (), m_sActiveSkillID[s], m_matTrans, sTargetID.vPos );
	}
	}
...
//dmk14 | 11-21-16 | dual pet skill
void GLPetClient::MsgLearnDualSkill ( NET_MSG_GENERIC* nmg )
{
	...
	case EMPET_REQ_DUAL_SKILL_FB_OK:
		{
			...
			CInnerInterface::GetInstance().GetPetWindow()->RefreshSkillSlot();

			CInnerInterface::GetInstance().PrintMsgText ( 
						NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMPET_REQ_SKILLSLOT_EXPANSION_FB_OK"), sPet.m_szName );
		}
		break;
	case EMPET_REQ_DUAL_SKILL_FB_FAIL:
			CInnerInterface::GetInstance().PrintMsgText ( 
				NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMPET_REQ_SKILLSLOT_EXPANSION_FB_FAIL") );
			break;
	case EMPET_REQ_DUAL_SKILL_FB_NOITEM:
			CInnerInterface::GetInstance().PrintMsgText ( 
				NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMPET_REQ_SKILLSLOT_EXPANSION_FB_NOITEM") );
			break;
	case EMPET_REQ_DUAL_SKILL_FB_BADITEM:
			CInnerInterface::GetInstance().PrintMsgText ( 
				NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMPET_REQ_SKILLSLOT_EXPANSION_FB_BADITEM") );
			break;
	case EMPET_REQ_DUAL_SKILL_FB_ALREADY:
			CInnerInterface::GetInstance().PrintMsgText ( 
				NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMPET_REQ_SKILLSLOT_EXPANSION_FB_ALREADY") );
			break;
	}
}
...
void GLPetClient::MsgRemoveSkillFact ( NET_MSG_GENERIC* nmg )
{
	...
	//dmk14 | 11-21-16 | dual pet skill
	for( unsigned s=0; s<PETSLOT_SIZE;++s){
		m_pOwner->m_sPETSKILLFACT[s].RESET ();
		m_sActiveSkillID[s] = NATIVEID_NULL();
	}
...
void GLPetClient::MsgProcess ( NET_MSG_GENERIC* nmg )
{
	...
	case NET_MSG_PET_DUAL_SKILL_FB:								MsgLearnDualSkill( nmg );	//dmk14 | 11-21-16 | dual pet skill			
		break;
	default:
		CDebugSet::ToListView ( "[WARNING]GLPetField::MsgProcess Illegal Message(%d)", nmg->nType );
	  	break;
	};
}