...
BOOL GLInventory::InsertItem ( const SITEMCUSTOM &ItemCustom )
{
	...
	if ( bOk )
	{
		...
		// test fix wrapper Eduj
		if (  pInvenItem->sItemCustom.bWrap != true ) pInvenItem->sItemCustom.bWrap = false;
		//nondrop card Eduj
		if (  pInvenItem->sItemCustom.bNonDrop != true ) pInvenItem->sItemCustom.bNonDrop = false;
...
BOOL GLInventory::SETITEM_BYBUFFER ( CByteStream &ByteStream )
{
	...
	for ( DWORD i=0; i<dwNum; i++ )
	{
		...
		else if ( dwVersion==0x0112) //wrapper
		{
			...
			if ( !bOk )
			{
				CDebugSet::ToLogFile ( "Error: Inventory Save byte stream" );
				return FALSE;	
			}
