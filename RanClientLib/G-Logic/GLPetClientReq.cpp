...
void GLPetClient::ReqChangeActiveSkill ( SNATIVEID sSkillID, unsigned nIndex )
{
	// 스킬 취소를 위한
	if ( !GLGaeaClient::GetInstance().GetPetClient()->IsVALID() )	return;		//fix pet bug skill

	if ( sSkillID == NATIVEID_NULL() )
	{
		GLMSG::SNETPET_REQ_SKILLCHANGE NetMsg;
		NetMsg.sSkillID = NATIVEID_NULL();
		NetMsg.nIndex = nIndex;
		NETSENDTOFIELD ( &NetMsg );
		return;
	}
...
void GLPetClient::ReqDualSkillPet ( WORD wPosX, WORD wPosY )
{
	...
	if ( !DxGlobalStage::GetInstance().IsEmulator() )
	{
		PETCARDINFO_MAP_ITER iter = pOwner->m_mapPETCardInfo.find ( m_dwPetID );
		if ( iter==pOwner->m_mapPETCardInfo.end() ) return;
		sPet = (*iter).second;
	}
