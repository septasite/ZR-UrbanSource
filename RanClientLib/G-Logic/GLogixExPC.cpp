...
void GLCHARLOGIC::RESET_DATA ()
{
	...
	//m_gdDAMAGE = GLPADATA2();
	//m_gdDAMAGE_SKILL = GLPADATA2();
	//m_gdDAMAGE_PHYSIC = GLPADATA2();
								 ...
								 const WORD _wSchool, const WORD wHair, const WORD wFace, const WORD _wHairColor, const WORD _wSex )
{
	...
	if ( emCIndex>=GLCI_NUM_NEWEST )			emCIndex = GLCI_FIGHTER_M;// 
	...
	m_dwServerID	= dwServerID;		// ¼­¹ö±×?E
	StringCchCopy ( m_szName, CHAR_SZNAME, szCharName );	// Ä³¸¯ÅÍ?E
...
void GLCHARLOGIC::SUM_ADDITION ( float fCONFT_POINT_RATE )
{
	...
	EMCHARINDEX emCI = GETCHARINDEX ();										//	ÄÉ¸¯ÅÍÀÇ Á¾?EÀÎµ¦½º.
	const GLCONST_CHARCLASS &cCHARCONST = GLCONST_CHAR::cCONSTCLASS[emCI];	//	ÄÉ¸¯ÅÍÀÇ »ó¼ö?E
	int ZBLEVEL = (nLEVEL-1);												//	Zero base Level.

	//	Note : Æä½Ã?E½ºÅ³ °¡?E?ÇÕ?E
	//
	SUM_PASSIVE ();

	//	Note : ¾ÆÀÌÅÛ¿¡ ºÙÀº ºÎ°¡ È¿°ú?EÇÕ?E
	//
	SUM_ITEM ();
	...
	GLOGICEX::VARIATION ( m_wPA, UINT_MAX, nSUM_PA );	// º¯È­µÇ´Â °ªÀ» 0 <= x < 0xffff ±îÁö·Î Á¦ÇÑ.

	int nSUM_SA = m_sSUMITEM.nSA + m_sSUM_PASSIVE.m_nSA;
	GLOGICEX::VARIATION ( m_wSA, UINT_MAX, nSUM_SA );	// º¯È­µÇ´Â °ªÀ» 0 <= x < 0xffff ±îÁö·Î Á¦ÇÑ.

	int nSUM_MA = m_sSUMITEM.nMA + m_sSUM_PASSIVE.m_nMA;
	GLOGICEX::VARIATION ( m_wMA, UINT_MAX, nSUM_MA );	// º¯È­µÇ´Â °ªÀ» 0 <= x < 0xffff ±îÁö·Î Á¦ÇÑ.
	...
	//add cp
	m_sCP.dwMax = DWORD ( GLCONST_CHAR::m_sCP_MAX + m_sSUMITEM.nCP + m_sSUM_PASSIVE.m_nCP_RATE );
	m_sCP.dwMax = DWORD ( m_sCP.dwMax * fCONFT_POINT_RATE );
	m_sCP.LIMIT();
	
	//	Note : ¸íÁßÀ², È¸ÇÇÀ² °è»ê.
	m_nHIT = int ( m_sSUMSTATS.dwDex*cCHARCONST.fHIT_DEX + m_sSUMITEM.nHitRate + m_sSUM_PASSIVE.m_nHIT );
	m_nAVOID = int ( m_sSUMSTATS.dwDex*cCHARCONST.fAVOID_DEX + m_sSUMITEM.nAvoidRate + m_sSUM_PASSIVE.m_nAVOID );
	...
	//	Note : ¹æ¾ûÓÂ ( DP + POW*°è?E+ ITEM_DEF ) * °ø°£ÀûÀÀ º¸Á¤.
	...
	m_fCP_GAIN = GLCONST_CHAR::dwCP_RATE + m_sSUMITEM.fCP_GAIN + m_sSUM_PASSIVE.m_nCP_GAIN; //add cp
...
void GLCHARLOGIC::SUM_ITEM ()
{
	...
	for ( int i=0; i<SLOT_NSIZE_S_2; i++ )
	{
				...
				{	//combine addons
					for ( DWORD addon=0; addon<ITEM::SSUIT::ADDON_SIZE; ++addon )
					{
						...
						case EMADD_SA:
							...
							break;
						case EMADD_MA:
							m_sSUMITEM.nMA	+= pItem->sSuitOp.sADDON[addon].nVALUE;
							break;
		...
		// ÀÌµ¿¼Óµµ ·£´ý¿É¼Ç Àû?E ÆÄÃ÷´Â À§¿¡¼­ Àû?Em_fVehicleSpeedVol ) 
...
//		else if ( m_bVehicle ) m_sSUMITEM.fInc_MoveSpeed += sItemCustom.GETOptVALUE( EMR_OPT_MOVE_SPEED );
		...
		m_sSUMITEM.sStats.dwPow += sItemCustom.GET_STAT_POW();// add direct value refixed by Ocram Networks
		m_sSUMITEM.sStats.dwSpi += sItemCustom.GET_STAT_INT();// add direct value refixed by Ocram Networks
		m_sSUMITEM.sStats.dwDex += sItemCustom.GET_STAT_DEX();// add direct value refixed by Ocram Networks
		m_sSUMITEM.sStats.dwSta += sItemCustom.GET_STAT_STM();// add direct value refixed by Ocram Networks
		m_sSUMITEM.sStats.dwStr += sItemCustom.GET_STAT_VIT();// add direct value refixed by Ocram Networks
		...
		m_sSUMITEM.fRateHit_Per += sItemCustom.GETHITRATE_PER(); // ejsayaaa
		//
...
void GLCHARLOGIC::SUM_PASSIVE ()
{
	...
	for ( ; iter!=iter_end; ++iter )
	{
		...
		case SKILL::EMFOR_CP_GAIN:
			m_sSUM_PASSIVE.m_nCP_GAIN += int ( sDATA_LVL.fBASIC_VAR ); //add cp
			break;
		...
		case SKILL::EMFOR_CP_RATE: //add cp
			m_sSUM_PASSIVE.m_nCP_RATE += int ( sDATA_LVL.fBASIC_VAR );;
			break;
		...
		for( unsigned s=0;s<SKILL::SAPPLY::MAX_DESCRIPTOR;++s)
		{
			...
			const float &fADDON = desc.fADDON_VAR[sCharSkill.wLevel];
			switch ( desc.emADDON )
			{
			...
			case EMIMPACTA_CP_RATE:
				m_sSUM_PASSIVE.m_nCP_RATE += (int)fADDON; //add cp
				break;
			...
			case EMSPECA_BLOW_DMG:
				m_sSUM_PASSIVE.m_fBlow_Dmg += fADDON; //add blowdmgskill
				break;

				
			case EMSPECA_CRUSHING_BLOW:
				m_sSUM_PASSIVE.m_fBlow_Rate += fADDON; //add Blowrateskill
				break;
				

			case EMSPECA_CRIT_RATE:
				m_sSUM_PASSIVE.m_fCrit_Rate += fADDON; //add critrateskill
				break;

			case EMSPECA_CRIT_DMG:
				m_sSUM_PASSIVE.m_fCrit_Dmg += fADDON; //add critdmgskill
				break;
				*/
			};
		}
		...
		for( unsigned s=0;s<SKILL::SAPPLY::MAX_DESCRIPTOR;++s)
		{
			...
			const SKILL::SSPEC &sSPEC = desc.sSPEC[sCharSkill.wLevel];
			switch ( desc.emSPEC )
			{
			case EMSPECA_PIERCE:
...
// 2006-03-28 Jgkim ±âÈ¹ÆÀ ¿äÃ»À¸·Î µ¥¹Ì?E°ø½Ä º¯°æÇÔ
	...
	const dwDivCount)
{
	...
	const SRESIST &sRESIST = pActor->GETRESIST (); // »ó?E?¹æ¾ûÍ¸ ÀúÇ×°ª
	...
	if ( pSkill )
	{
		...
		//	Note : ½ºÅ³ »ç?E?ºÎÀûÀÌ ¼Ò¸ðµÇ?E ºÎÀûÀÇ °ø°ÝÄ¡ °¡?E
		//
    	...
    	case SKILL::EMAPPLY_MAGIC:
			nDEFENSE = 0;						//	¸¶¹ý °ø°ÝÀÏ¶§´Â ÀÏ¹Ý ¹æ¾ûÓÂ ¹«½ÃµÊ.
			nDEFAULT_DEFENSE = 0;				//	¸¶¹ý °ø°ÝÀÏ¶§´Â ÀÏ¹Ý ¹æ¾ûÓÂ ¹«½ÃµÊ.
			nITEM_DEFENSE = 0;					//	¸¶¹ý °ø°ÝÀÏ¶§´Â ÀÏ¹Ý ¹æ¾ûÓÂ ¹«½ÃµÊ.
			gdDamage.VAR_PARAM ( m_wSUM_MA );
			// ¸¶¹ý °ø°Ý½Ã ¼Ó¼º ÀúÇ×Ä¡ Àû?EºñÀ²
			// default.charset ¿¡¼­ °¡Á®¿Â
			// fRESIST_G ÀÇ °ª
		...
		default:
			GASSERT(0&&"CALCDAMAGE() ¹°¸® µ¥¹ÌÁö°¡ ?EâµÇÁE¾Ê´Â ½ºÅ³.");
			break;
		};
		...
		//	½ºÅ³ÀÇ µ¥¹Ì?E°è?E?¿µÇâÀ» ÁÖ´Â °ªÀÌ ±âº»ÀÎ?EÆ¯¼öÀÎ?E°Ë?E
		float fSKILL_VAR = sSKILL_DATA.fBASIC_VAR;

		// 2006-03-28 Jgkim±âÈ¹ÆÀ ¿äÃ»À¸·Î ÁÖ¼®Ã³¸®, µ¥¹Ì?E°ø½Äº¯°æµÊ
		// int nVAR = abs ( int(fSKILL_VAR*fPOWER) );
		// nVAR = nVAR - (int) ( nVAR*nRESIST/100.0f*fRESIST_G );
		// if ( nVAR<0 )	nVAR = 0;

		// //	Note : ?EÌÁö°?Áõ?EÈ´? ?EÌÁE¹Ý¿µÀ².
		...
		// ½ºÅ³µ¥¹Ì?E= ±âº»½ºÅ³µ¥¹Ì?E³¯¾¾¿¡ µû¸¥ µ¥¹ÌÁö¹Ý¿µÀ²
		int nVAR = abs ( int(fSKILL_VAR*fPOWER) );
		...
		// ?E¼ÀúÇ×°?= »ó?E?ÀúÇ×°ª / 100 * ¼Ó¼ºÀúÇ×Ä¡ Àû?EñÀ?
	...
	//	?EÌÁEÃÖ?EÃÖ¼Ò°ª »çÀÌÀÇ ·£´ý ?EÌÁE°áÁ¤.
	int  nDAMAGE_NOW = 0;
...
void GLCHARLOGIC::UPDATE_MAX_POINT ( float fCONFT_POINT_RATE )
{
	...
	m_sCP.dwMax = DWORD ( GLCONST_CHAR::m_sCP_MAX + m_sSUMITEM.nCP + m_sSUM_PASSIVE.m_nCP_RATE ); //add cp
	m_sCP.dwMax = DWORD ( m_sCP.dwMax + m_fCP_RATE * fCONFT_POINT_RATE );
	m_sCP.LIMIT();

	m_fCP_GAIN = GLCONST_CHAR::dwCP_RATE + m_sSUMITEM.fCP_GAIN + m_sSUM_PASSIVE.m_nCP_GAIN + m_fCP_SKILLGAIN; //add cp
...
void GLCHARLOGIC::UPDATE_DATA ( float fTime, float fElapsedTime, BOOL bClient, float fCONFT_POINT_RATE )
{
	...
	if(m_fPkTimer >= 10.0f) m_dwPkStreak = 0;
	...
	for ( int i=0; i<SKILLFACT_SIZE; ++i )
	{
		...
		//	Å¬¶óÀÌ¾ðÆ®°¡ ¾Æ´Ò °æ?E¿©±â¼­ ½ºÅ³ ÀÌÆåÆ®¸¦ ºñÈ°¼ºÈ­, Å¬¶óÀÌ¾ðÆ®´Â UpdateSkillEffect()¿¡¼­ ÇÔ.
		...
		for( unsigned s=0;s<SKILL::SAPPLY::MAX_DESCRIPTOR;++s)
		{
			...
			switch ( desc.emADDON )
			{
			case EMIMPACTA_HITRATE:			m_nSUM_HIT += int(desc.fADDON_VAR);					break;
			case EMIMPACTA_AVOIDRATE:		m_nSUM_AVOID += int(desc.fADDON_VAR);					break;
			case EMIMPACTA_DAMAGE:			m_gdDAMAGE_SKILL.VAR_PARAM ( int(desc.fADDON_VAR) );	break;
			case EMIMPACTA_DEFENSE:			m_nDEFENSE_SKILL += int(desc.fADDON_VAR);				break;
			case EMIMPACTA_VARHP:			fINCR_HP += desc.fADDON_VAR;							break;
			case EMIMPACTA_VARMP:			fINCR_MP += desc.fADDON_VAR;							break;
			case EMIMPACTA_VARSP:			fINCR_SP += desc.fADDON_VAR;							break;
			case EMIMPACTA_VARAP:
				fINCR_HP += desc.fADDON_VAR;
				fINCR_MP += desc.fADDON_VAR;
				fINCR_SP += desc.fADDON_VAR;
				break;

			case EMIMPACTA_DAMAGE_RATE:		m_fDamageRate += desc.fADDON_VAR;					break;
			case EMIMPACTA_DEFENSE_RATE:	m_fDefenseRate += desc.fADDON_VAR;				break;		

			case EMIMPACTA_PA:				nSUM_PA += int(desc.fADDON_VAR);					break;
			case EMIMPACTA_SA:				nSUM_SA += int(desc.fADDON_VAR);					break;
			case EMIMPACTA_MA:				nSUM_MA += int(desc.fADDON_VAR);					break;
			case EMIMPACTA_HP_RATE:			m_fHP_RATE += desc.fADDON_VAR;					break;
			case EMIMPACTA_MP_RATE:			m_fMP_RATE += desc.fADDON_VAR;					break;
			case EMIMPACTA_SP_RATE:			m_fSP_RATE += desc.fADDON_VAR;					break;
			case EMIMPACTA_RESIST:			m_sSUMRESIST_SKILL += int(desc.fADDON_VAR);					break;
			case EMIMPACTA_CP_RATE:			m_fCP_RATE += desc.fADDON_VAR;			break; //add cp
			case EMIMPACTA_CP_GAIN:			m_fCP_SKILLGAIN += desc.fADDON_VAR;		break; //add cp
			};
		}
		...
		for( unsigned s=0;s<SKILL::SAPPLY::MAX_DESCRIPTOR;++s)
		{
			...
			switch ( desc.emSPEC )
			{
			case EMSPECA_NONBLOW:		m_dwHOLDBLOW = desc.dwSPECFLAG;			break;
			case EMSPECA_IMMUNE:		
				{
					m_bAntiPot = false;
					m_bAntiSkill = false;
				...
				break;
			case EMSPECA_PIERCE:		m_nSUM_PIERCE += int ( desc.fSPECVAR1 );	break;
			case EMSPECA_TARRANGE:		
			case EMSPECA_ATTACKRANGE:	m_fSUM_TARRANGE += desc.fSPECVAR1;		break;
			case EMSPECA_MOVEVELO:		m_fSKILL_MOVE += desc.fSPECVAR1;			break;
			case EMSPECA_INVISIBLE:		m_bINVISIBLE = true;						break;
			case EMSPECA_RECVISIBLE:	m_bRECVISIBLE = true;						break;
			//	°ø°Ý ¼Óµµ °è³äÀÌ¹Ç·Î ( °ø°Ý ½Ã°£À» -0.1 (-10%) °¨¼Ò ½ÃÅ°?EÀ§ÇØ¼­´Â ºÎÈ£ ¹Ý?E?ÇÏ¿©¾ßÇÔ. )
			case EMSPECA_ATTACKVELO:	m_fATTVELO -= desc.fSPECVAR1;				break;
			case EMSPECA_SKILLDELAY:	
			case EMSPECA_DURATION:		m_fSKILLDELAY += desc.fSPECVAR1;			break;

				/*
			case EMSPECA_BLOW_DMG:	m_fBlow_Dmg_Skill += desc.fSPECVAR1;			break; //add blowdmgskill
			case EMSPECA_CRIT_RATE:	m_fCrit_Rate_Skill += desc.fSPECVAR1;			break; //add critrateskill
			case EMSPECA_CRIT_DMG:	m_fCrit_Dmg_Skill += desc.fSPECVAR1;			break; //add critdmgskill
			case EMSPECA_CRUSHING_BLOW:	m_fBlow_Rate_Skill += desc.fSPECVAR2;			break; //add Blowrateskill
				*/
			case EMSPECA_NOPOTION:		//add antipot
				{
					if( !m_bImmune )	m_bAntiPot = true;		//dmk14 | 9-11-16 | immune
					//else m_bAntiPot = false;
				}								
				break; 
			case EMSPECA_NOSKILL:		//add antiskill
				{
					if( !m_bImmune )	m_bAntiSkill = true;	//dmk14 | 9-11-16 | immune
					//else m_bAntiSkill = false;
				}							
				break; 
	...
	for ( i=0; i<EMBLOW_MULTI; ++i )
	{
		...
		case EMBLOW_FROZEN:
			m_fSTATE_MOVE += sSTATEBLOW.fSTATE_VAR1;			//	ÀÌµ¿¼Óµµ °¨¼Ò.
			m_fSTATE_DAMAGE += sSTATEBLOW.fSTATE_VAR2;			//	?EÌÁEÁõ°¡.
			break;
	...
	//dmk14 | 11-21-16 | dual pet skill
	for( unsigned s=0; s<PETSLOT_SIZE;++s){
		if ( m_sPETSKILLFACT[s].sNATIVEID !=NATIVEID_NULL() )
		{
			switch ( m_sPETSKILLFACT[s].emTYPE )
			{
			case SKILL::EMFOR_PET_HEAL:				//	ÁÖÀÎÀÇ HP È¸º¹ ¼Óµµ »ó½Â
				fINCR_HP += m_sPETSKILLFACT[s].fMVAR;
				break;
			case SKILL::EMFOR_PET_SUPPROT:			//	ÁÖÀÎÀÇ HP, MP, SP È¸º¹·ü »ó½Â
				fINCR_HP += m_sPETSKILLFACT[s].fMVAR;
				fINCR_MP += m_sPETSKILLFACT[s].fMVAR;
				fINCR_SP += m_sPETSKILLFACT[s].fMVAR;
				break;
			case SKILL::EMFOR_PET_BACKUP_ATK:		//	ÁÖÀÎÀÇ °ø°Ý·Â(À²) »ó½Â
				m_fDamageRate += m_sPETSKILLFACT[s].fMVAR/100.0f;
				break;
			case SKILL::EMFOR_PET_BACKUP_DEF:		//	ÁÖÀÎÀÇ ¹æ¾î·Â(À²) »ó½Â
				m_fDefenseRate += m_sPETSKILLFACT[s].fMVAR/100.0f;
				break;
			};

			// ½ºÅ³ ¶óÀÌÇÁÅ¸ÀÓ °»½Å
			m_sPETSKILLFACT[s].fAGE -= fElapsedTime;
		}
	}
...
//			for ( i=0; i<SKILLFACT_SIZE; ++i )		DISABLESKEFF ( i );
	...
	//	°ø°Ý ¼Óµµ. ( Áö¼Ó skill ¼Ó¼º °¡?E?ºÎºÐ¿¡ state, passive skill ¼Ó¼º °¡?E )
	...
	if( m_fGameTimer >= 600.0f )
	{
		m_tLoginTime+=1;
		m_fGameTimer = 0;
		CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("INC_GAMETIME"), 1 );
	}*/
...
{
	float fMOVE = m_fSTATE_MOVE + m_fSKILL_MOVE + m_fOPTION_MOVE + m_sSUMITEM.fIncR_MoveSpeed + m_fLandBonusMSpeed;
...
//test gun
	...
	if ( emSuit==SUIT_HANDHELD )
	{
		...
		{
			if ( sItem.sSuitOp.emHand!=HAND_BOTH )	//	¾ç¼Õ ¸ðµÎ °¡´ÉÇÏ?EÁ¡°Ë ¾ÈÇÔ.
			{
			...
			{
				//	È­?EÂø?EÃ¿¡´?¿À¸¥¼Õ¿¡ È°À» °¡Áö?EÀÖ?E¾Ê´Ù?EÂø?EºÒ°¡´É.
			...
			else if ( sItem.sBasicOp.emItemType==ITEM_BULLET )
			{
				...
				else if (m_pITEMS[emRHand]->sSuitOp.emAttack ==ITEMATT_GUN ) return TRUE;
...
//	³ÖÀ»·Á´Â ½½·ÔÀÌ ºñ¿´´Â?E°Ë»çÇÑ´Ù.
//	´Ü, È°ÀÌ ¾ç¼Õ ¹«±âÀÌÁö¸¸ ¿Þ¼Õ¿¡ È­?E?ÀåÂøÇÒ?EÀÖ´Ù.
BOOL GLCHARLOGIC::ISEMPTY_SLOT ( SNATIVEID sNativeID, EMSLOT emSlot )
{
	...
	if ( pItem->sSuitOp.emSuit==SUIT_HANDHELD )
	{
		...
		if ( pItem->sSuitOp.IsBOTHHAND() )
		{
			...
			//	¸¸?E³ÖÀ»·Á´Â°Ô È°ÀÌ¶ó?E ¿Þ¼Õ¿¡ È­?E?ÀÖ¾ûÑµ µÊ. ´Ü ¿À¸¥¼ÕÀº ºñ?EÀÖ¾ûÚß.
...
BOOL GLCHARLOGIC::ACCEPT_ITEM ( SNATIVEID sNativeID )
{
	...
	//add reborn req
	if ( m_nChaReborn < sItem.sBasicOp.nRebornReq )				return FALSE;
...
BOOL GLCHARLOGIC::SIMPLE_CHECK_ITEM ( SNATIVEID sNativeID, SNATIVEID sDisguiseID )
{
	...
	//	Memo : ÄÚ½ºÆ¬?E¹æ¾ûÍ¸ÀÇ ÀÚ°Ý¿ä°ÇÀ» Ã¼Å©ÇÑ´Ù.
	//		µÎ ¹æ¾ûÍ¸ÀÇ ±³ÁýÇÕÀÌ Âø?E¡´?Á¶°ÇÀÌ´Ù.
...
void GLCHARLOGIC::PKSTREAK() //add pk streak by njd
{
	m_dwPkStreak += 1;	
	m_fPkTimer = 0.0f;
}
...
/*
BOOL GLCHARLOGIC::DuelWinReward ( LONGLONG llLoss, WORD wType  ) //add duel bet
{
...
BOOL GLCHARLOGIC::DuelLossReward ( LONGLONG llLoss, WORD wType  ) //add duel bet
{
	if ( wType == 1 )
	{
		if  ( m_lnMoney < llLoss ){
...
EMSKILL_LEARNCHECK GLCHARLOGIC::CHECKLEARNABLE_SKILL ( SNATIVEID skill_id )
{
	...
	if ( m_nChaReborn < sSKILL_LEARN_LVL.nREBORN )				return EMSKILL_LEARN_NOTTERM;
...
EMSKILLCHECK GLCHARLOGIC::CHECHSKILL ( SNATIVEID skill_id, WORD wStrikeNum, bool bNotLearn )
{
	...
	{
		SKILL_MAP_ITER learniter = m_ExpSkills.find ( skill_id.dwID );
		if ( learniter==m_ExpSkills.end() )										return EMSKILL_NOTLEARN; // ¹è?E½ºÅ³ÀÌ ¾Æ´Ò °æ?E
		sSkill = (*learniter).second;

		//	Note : ½ºÅ³ µô·¡ÀÌ Å¸ÀÓÀÌ Áö³´´Â?EÁ¡°Ë.
		//
		DELAY_MAP_ITER delayiter = m_SKILLDELAY.find ( skill_id.dwID );
		if ( delayiter!=m_SKILLDELAY.end() )									return EMSKILL_DELAYTIME; // ½ºÅ³ µô·¡ÀÌ ½Ã°£ÀÌ Áö³ª?E¾ÊÀ½.
	}
	...
	if ( emSKILL_LITEM!=ITEMATT_NOCARE )
	{
		...
		if ( pItem->sSuitOp.emAttack != ITEMATT_EXTREMEFIST )
		{
	...
	//	Note : È­?E¼Ò¸ð½Ã ¼Ò¸ð·® ¿©À¯ Ã¼Å©.
	//		--> ¼Ò¸ð·®ÀÌ 2 °³ÀÎ?EÇÑ°³¸¸ ³²¾ÆÀÖ´Â »óÈ²? ¿¡´Â ³²Àº ¼ýÀÚ¸¸ ¼ÒÁøÇÏ?E½ºÅ³Àº ¹ßµ¿µÇ°Ô ÇØÁÜ.
	//
...
{
	//	Note : Ä³¸¯ÅÍ°¡ ¹è?E½ºÅ³ Á¤º¸ °¡Á®¿È.
	// ¹ßµ¿½ºÅ³Àº ¹è¿ûÙºÅ³ ¸ñ·Ï¿¡ ¾øÀ½
	SCHARSKILL sSkill;
	...
	{
		SKILL_MAP_ITER learniter = m_ExpSkills.find ( skill_id.dwID );
		if( learniter==m_ExpSkills.end() )										return;	// ¹è?E½ºÅ³ÀÌ ¾Æ´Ò °æ?E
		sSkill = (*learniter).second;
	}
	...
	//	Note : ¼­¹ö¿¡¼­´Â ¸Þ½Ã?Eµô·¡ÀÌ ¸¹Å­ °¨¼Ò½ÃÄÑ ÁØ´Ù.
			...
			if ( bOK )
			{
				...
				if ( bTER )
				{
					...
					if ( bOK )
					{
						...
						{
							emANTIDISAPPEAR = EMANTIDISAPPEAR_USE; // ¼Ò¹æÁÖ¸¦ »ç?Eß´?
						}
						else
						{
							_bTERMINATE = true;		// ¾ÆÀÌÅÛÀÌ ¼Ò?EÇ¾ú´?
						}
					}
				}
			}
...
//	Note : Çö?Epk ·¹º§À» ¾Ë¾Æº»´Ù. ( UINT_MAX ÀÏ°æ?Epk ÇØ´ç»çÇ× ¾øÀ½. )
...
//	Note : Àû?E?µûÓÏ. ( pk ?E?),	true : ½Å±Ô µûÓÏ½Ã, false : ±âÁ¸ µûÓÏ °»½Å½Ã.
bool GLCHARLOGIC::ADD_PLAYHOSTILE ( DWORD dwCHARID, BOOL bBAD )
{
	...
	if ( pos!=m_mapPlayHostile.end() )
	{
		...
		//	Note : ½Ã°£¸¸ °»½Å. Àû?EbBAD´Â °»½ÅÇÏ?E¾ÊÀ½. ( Ã³À½ °ø°ÝÀÚÀÇ °ª À¯?E )
		if( pos->second )
		{
