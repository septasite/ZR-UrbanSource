...
void GLPetField::UpdateSkillState ( float fElapsedTime )
{
	...
	for( unsigned s=0; s<PETSLOT_SIZE;++s) {
		if ( m_sActiveSkillID[s] == NATIVEID_NULL () ) count ++;	
	}

	if( count >= PETSLOT_SIZE )	return;
	
	//dmk14 | 11-21-16 | dual pet skill
	for( unsigned s=0; s<PETSLOT_SIZE;++s) {
	if ( m_pOwner->m_sPETSKILLFACT[s].fAGE < 0.0f )
	{
		if ( m_pOwner->m_sPETSKILLFACT[s].emTYPE == SKILL::EMFOR_PET_PROTECTITEMS )
		{
