...
enum VEHICLE_TYPE
{
	...
	//Added by   | 11-8-2012 | added new vehicle
	...
	VEHICLE_TYPE_BIKE_GHOST	= 7,
	VEHICLE_TYPE_BIKE_JOKER	= 8,
	VEHICLE_TYPE_BIKE_FERRARI	= 9,
	VEHICLE_TYPE_BIKE_REAR	= 10,
...
enum VEHICLE_ACCESSORYTYPE
{
	...
	ACCE_TYPE_PARTS_A	= 1, // parts & head
	ACCE_TYPE_PARTS_B	= 2, // parts & body
	ACCE_TYPE_PARTS_C	= 3, // parts & tire
	//Added by   | 11-8-2012 | added new vehicle
	ACCE_TYPE_PARTS_D	= 4, // manhura
	ACCE_TYPE_PARTS_E	= 5, // sadle & back
	ACCE_TYPE_PARTS_F	= 6, // flag
	ACCE_TYPE_SIZE		= 7,
};
...
//Added by   | 11-8-2012 | Add VPS and VCF code
EMVPSINDEX VehicleTypeToVpsInd ( const VEHICLE_TYPE emType );*/
// Added new function Reborn Requirement.
//#define ITEMEDIT_VER 0x0115
#define ITEMEDIT_VER 0x0116
...
namespace ITEM
{
	...
	struct SSUIT // By 경대
	{
		enum { ADDON_SIZE = 5, VERSION = 0x0109 };
	...
	struct SBASIC_115
	{
		...
		std::string		strFieldFile;				//	바닥 형상 파일.
		std::string		strInventoryFile;			//	인벤토리 형상 파일.
		std::string		strWearingFile[GLCI_NUM_NEWEST];	//	형상 파일.
		std::string		strComment;					//	아이템에 대한 간단한 설명.
		...
		std::string		strABLFile[CPSIND_SIZE];

		std::string		strVPSFile;
		...
		struct SBASIC
	{
		...
		std::string		strInventoryFile;			//	인벤토리 형상 파일.
		std::string		strWearingFile[GLCI_NUM_NEWEST];	//	형상 파일.
		std::string		strWearingFile2[GLCI_NUM_NEWEST];
		std::string		strComment;					//	아이템에 대한 간단한 설명.
		...
		bool IsGarbage()		{ return (dwFlags&TRADE_GARBAGE)!=NULL; }
			//add ownership
		bool IsOwner()		{ return (dwFlags&ITEM_OWNERSHIP)!=NULL; }

		bool IsCostumeCombine()		{ return (dwFlags&ITEM_COSTUME_COMBINE)!=NULL; }
		bool IsPointRecover()		{ return (dwFlags&ITEM_POINT_RECOVER)!=NULL; }//add point sell
        // add perfect grinder Eduj
		bool IsNoFail()				{ return (dwFlags&ITEM_NO_FAIL)!=NULL; } //add nofailups



		//bool IsDETERGENT()		{ return (dwFlags&TRADE_DETERGENT)!=NULL; }
...
struct SITEM
{
	...
	const char* GetCpsFile ( EMCPSINDEX emCpsInd ) { return sBasicOp.strABLFile[emCpsInd].c_str(); }
...
public:
	...
	DWORD		dwVehicleID; // 탈것아뒤(DB)
	bool		bWrap; // wrapper   
	//bool		bNonDrop; //nondrop card Eduj  
};
//nondrop card Eduj
...
public:
	...
	void Assign ( const SITEMCUSTOM_112 &sOld );// wrapper
	void Assign ( const SITEMCUSTOM_113 &sOld );//nondrop card Eduj
    ...
    //nondrop card Eduj
	bool IsNonDrop_Ex () const
	{
		return bNonDrop == true;
	}
...
public:
	...
	bool IsWrap()	{ return bWrap; } // wrapper
	bool IsNonDrop()	{ return bNonDrop; } //nondrop card Eduj
...
public:
	...
	short		nOptVALUE3;
	short		nOptVALUE4;
	bool        bWrap;     // This code are useless i just add this for fun
	bool        bNonDrop;  // fun is like fucking fun a pussy - Eduj
...
struct SITEMCLIENT
{
	...
	SITEMCLIENT () 
		...
		, nOptVALUE1(0) 
		, bWrap( false ) // wrapper
		, bNonDrop( false ) //nondrop card Eduj
	{
	}
	...
	SITEMCLIENT& Assign ( const SITEMCUSTOM &Input )
	{
		...
		bWrap = Input.bWrap; // wrapper
		bNonDrop = Input.bNonDrop; //nondrop card Eduj
...
struct SINVENITEM_SAVE_112 // wrapper
{
	WORD		wPosX;
	WORD		wPosY;
...
struct SINVENITEM_SAVE_113 //nondrop card Eduj
{
	WORD		wPosX;
	WORD		wPosY;
...
struct SINVENITEM_SAVE
{
	...
	void Assign ( const SINVENITEM_SAVE_112 &sOld ) // wrapper
	{
		wPosX = sOld.wPosX;
		wPosY = sOld.wPosY;
	...
	void Assign ( const SINVENITEM_SAVE_113 &sOld ) //nondrop card Eduj
	{
		wPosX = sOld.wPosX;
		wPosY = sOld.wPosY;
...
//struct SITEMSEALDATA //sealed card NaJDeV
//{
//	int				nOption;
//	EMRANDOM_OPT	emOpt;
//	float			fValue;
