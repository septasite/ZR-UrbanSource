...
void GLLandMan::DoSkillActEx ( const SSKILLACTEX &sACTEX )
{
	...
	{
		pREACTOR->BUFF_REMOVE ( sACTEX.dwRemoveFlag, sACTEX.idSKILL );
	}
	...
	{
		PushPullAct( sACTEX.sID, sACTEX.sID_TAR, sACTEX.fPUSH_PULL, sACTEX.fPUSH_PULL_B, sACTEX.bPush, sACTEX.bPull, sACTEX.bDash, sACTEX.bTeleport );  //add pushpull //modify //add teleport -   2-8-2014
	}
	// 밀어내기
	else
	{
		if ( sACTEX.bPush || sACTEX.bPull || sACTEX.bDash || sACTEX.bTeleport  )
		{
			PushPullAct( sACTEX.sID, sACTEX.sID_TAR, sACTEX.fPUSH_PULL, sACTEX.fPUSH_PULL_B, sACTEX.bPush, sACTEX.bPull, sACTEX.bDash, sACTEX.bTeleport );  //add pushpull //modify //add teleport -   2-8-2014
		}
		else
		{
			PushPullAct( sACTEX.sID_TAR, sACTEX.sID_TAR, sACTEX.fPUSH_PULL, sACTEX.fPUSH_PULL_B, false, false, false, false );  //add pushpull //modify// add teleport -   2-8-2014
		}
	}
}

void GLLandMan::PushPullAct( const STARGETID dwActorID, const STARGETID dwReActorID, const float fPUSH_PULL, const float fPUSH_PULL_B, const bool bPush, const bool bPull, const bool bDash, const bool bTeleport )//add dash - SeiferXIII 6/30/13 // add teleport -   2-8-2014
{
	if ( fPUSH_PULL != 0.0f || bDash || bTeleport || bPull )
	{	
		...
		float fPushPull = 0.0f;

		if( bPull ) fPushPull = D3DXVec3Length ( &D3DXVECTOR3( pACTOR->GetPosition() - pREACTOR->GetPosition() ) );
		if( bPush ) fPushPull = fPUSH_PULL_B;
			...
			if ( bCoA )
			{
				...
				if ( bCoB )
				{
					if ( pNavi->LineOfSightTest ( dwCoA, vCoA, dwCoB, vCoB ) )
					{
						if ( bDash || bTeleport ) 
						{
							pACTOR->ReceivePushPull ( vCoB, false, true, 2000, true, bTeleport );
						}
						else
						{
							if ( bPush || bPull ) //fix me ( dont know if push or pull must cancel movements )
							{
								pREACTOR->ReceivePushPull ( vCoB, true, true, fPUSH_PULL_B, false, false ); // cancel when push /pull
							}
							else
							{
								pREACTOR->ReceivePushPull ( vCoB, true, false, 0.0f, false, false  );
							}
						}
					}
				}
...
{
	DWORD dwAllPlayer = 0;
	for(int i=0; i<SW_SCHOOL_MAX; ++i ) m_dwPcCount[i] = 0;
	GLCHARNODE* pCharNode = m_GlobPCList.m_pHead;
	for ( ; pCharNode; pCharNode = pCharNode->pNext )
	{
		if ( pCharNode->Data->m_dwUserLvl >= USER_SPECIAL ) continue;

        dwAllPlayer++;
        switch ( pCharNode->Data->m_wSchool )
		{
		   case 0: m_dwPcCount[0]++; break;
		   case 1: m_dwPcCount[1]++; break;
		   case 2: m_dwPcCount[2]++; break;
		}
	}
	DWORD dwPcTotal = dwAllPlayer;
	for( int i=0; i<SW_SCHOOL_MAX; ++i )
	{
		if ( m_dwPcCount[i] > dwPcTotal || dwPcTotal == 0 ) continue;
