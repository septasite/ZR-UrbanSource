...
namespace GLOGICEX
{
	//	Note : 주위 환경에 따른 ?E/ 엉喫  캐릭터의 엉?E디스 판단.
	...
	// Note : Strock TRUE/FALSE ?E갋	예) 적이 공격 동작시 ?E柰?되?E다시 동작을 ?E瞞?한다.
...
EMANI_SUBTYPE CHECK_ANISUB ( SITEM* pRHAND, SITEM* pLHAND )
{
	...
	case ITEMATT_CUBE:
		emANISUBTYPE = AN_SUB_CUBE;
		break;

	case ITEMATT_ROPE:
		emANISUBTYPE = AN_SUB_ROPE;
		break;

	case ITEMATT_EXTREMEFIST:
		emANISUBTYPE = AN_SUB_EXTREMEFIST;
		break;
