...
enum EMREQ_RECALL_FB
{
	...
	EMREQ_RECALL_FB_SP_BP_CRC_DISABLED	= 8,	// 진입불가 ( 이벤트 지역 )
};
...
namespace GLMSG
{
	...
	struct SNETPC_SERVER_ROYAL_RUMBLE_INFO
	{
		NET_MSG_GENERIC		nmg;
		bool			bRoyalRumble;

		SNETPC_SERVER_ROYAL_RUMBLE_INFO () :
			bRoyalRumble(false)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_SERVER_ROYAL_RUMBLE_INFO;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_SERVER_ROYAL_RUMBLE_REMAIN_AG
	{
		NET_MSG_GENERIC		nmg;
		DWORD			dwGaeaID;

		SNETPC_SERVER_ROYAL_RUMBLE_REMAIN_AG ()
			: dwGaeaID(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_REQ_SERVER_ROYAL_RUMBLE_REMAIN_AG;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
