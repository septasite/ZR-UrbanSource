...
SPETCARDINFO& SPETCARDINFO::operator= ( const SPETCARDINFO& rvalue )
{
	...
	for( unsigned s=0; s<PETSLOT_SIZE;++s) {
		m_sActiveSkillID[s]	= rvalue.m_sActiveSkillID[s];
	}
...
void GLPET::ASSIGN ( const GLPET& sPetData )
{
	...
	//dmk14 | 11-21-16 | dual pet skill
	for( unsigned s=0; s<PETSLOT_SIZE;++s) {
	m_sActiveSkillID[s]	= sPetData.m_sActiveSkillID[s];
	}
...
namespace COMMENT
{
	std::string PET_TYPE[PETTYPE_SIZE] = { "팻타입_A", "팻타입_B", "팻타입_C", "팻타입_D", "PET_TYPE_E", "PET_TYPE_F", "PET_TYPE_G", "PET_TYPE_H", "PET_TYPE_I", "PET_TYPE_J", "PET_TYPE_K", "PET_TYPE_L" };
};