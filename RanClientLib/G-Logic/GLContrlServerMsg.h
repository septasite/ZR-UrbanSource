...
namespace GLMSG
{
	...
	{
		NET_MSG_GENERIC nmg;
		__time64_t loginTime;   // ｷﾎｱﾗﾀﾎ ｽﾃｰ｣ﾀﾌｳｪ ﾀﾌｺ･ﾆｮ ｽﾃﾀﾛｽﾃ ｽﾃｰ｣
	...
	struct SNET_GM_LIMIT_EVENT_TIME_REQ_FB
	{
		...
		bool bEventStart;       // ﾀﾌｺ･ﾆｮ ｽﾃﾀﾛ ｿｩｺﾎｳｪ ﾀ釤・ｴ・ﾌ ｾﾆｴﾏｸ・FALSE ｸｮﾅﾏ
		__time64_t loginTime;   // ｷﾎｱﾗﾀﾎ ｽﾃｰ｣ﾀﾌｳｪ ﾀﾌｺ･ﾆｮ ｽﾃﾀﾛｽﾃ ｽﾃｰ｣
		int play_Time;			// ﾀﾌｺ･ﾆｮ ﾀ釤・ｽﾃﾀﾛ ｽﾃｰ｣
		int buster_Time;		// ﾀﾌｺ･ﾆｮ ﾀ釤・ｽﾃｰ｣
	...
	struct SNET_GM_SLOT_EVENT
	{
		...
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GM_SLOT_EVENT;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNET_GM_SLOT_EVENT_FLD
	{
		NET_MSG_GENERIC		nmg;
		...
		SNET_GM_SLOT_EVENT_FLD () :
			dwGaeaID(0),
			bOn(false),
			nSlotNumber(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GM_SLOT_EVENT_FLD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNET_GM_SLOT_EVENT_BRD
	{
		NET_MSG_GENERIC		nmg;
		...
		SNET_GM_SLOT_EVENT_BRD () :
			bOn(false),
			nSlotNumber(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GM_SLOT_EVENT_BRD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	...
	struct SNET_USER_CHAR_INFO_FLD_FB
	{
		...
		//	ﾇﾊｵ蠢｡ｼｭ ｿ｡ﾀﾌﾁｯﾆｮｷﾎ ｼﾛｽﾅﾈﾄ ｿ｡ﾀﾌﾁｯﾆｮｿ｡ｼｭ ｴ莠ｯﾇﾒ ﾅｬｶﾌｾｮ ｾﾆﾀﾌｵ・
		DWORD				dwCLIENTID_FB;
	...
	struct SNET_GM_CHAR_INFO_FLD_FB
	{
		...
		//added by   | 21-6-2012 | Bypass HP+MP+SP
  		GLPADATA			sCP;//march 23,2012 by ramispo
		GLLLDATA			sEXP;
		char				szCLUB[CHAR_SZNAME];
