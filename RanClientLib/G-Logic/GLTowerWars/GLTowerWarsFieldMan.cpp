...
bool GLTowerWarsFieldMan::FrameMove ( float fElapsedAppTime )
{
	for ( SW_VEC::size_type i=0; i<m_vecSchoolWAR.size(); ++i )
	{
		GLTowerWars &sSW = m_vecSchoolWAR[i];
		...
		if ( sSW.m_bKickOut )
		{
			sSW.m_fKickTimer += fElapsedAppTime;
			sSW.m_fKickTotal +=fElapsedAppTime;

			if ( sSW.m_fKickTimer >= 5.0f )
			{
				GLMSG::SNET_TOWERWARS_CTF_EXIT_MAP	NetMsgExit;
				GLGaeaServer::GetInstance().SENDTOCLIENT_ONMAP( sSW.m_dwClubMap, &NetMsgExit );
				sSW.m_fKickTimer = 0.0f;
			}

			if ( sSW.m_fKickTotal >= 120.0f )
			{
				GLMSG::SNET_TOWERWARS_CTF_EXIT_MAP	NetMsgExit;
				GLGaeaServer::GetInstance().SENDTOCLIENT_ONMAP( sSW.m_dwClubMap, &NetMsgExit );
				sSW.m_fKickTotal = 0.0f;
				sSW.m_bKickOut = false;
			}
		}
	}
...
bool GLTowerWarsFieldMan::EndBattle ( DWORD dwID, bool bForceExit )
{
	...
	if( !bForceExit )
	{
		...
		if ( pLandMan && bForceExit )
		{
			...
			{
				pLandMan->m_dwPcCount[i] = 0;
				pLandMan->m_fHpMultiplier[i] = 0.0f;
			}

			pSW->m_bKickOut = true;

			GLMSG::SNET_TOWERWARS_CTF_EXIT_MAP	NetMsgExit;
			GLGaeaServer::GetInstance().SENDTOCLIENT_ONMAP( pSW->m_dwClubMap, &NetMsgExit );
...
//dmk14 | 1-11-17
void GLTowerWarsFieldMan::UpdateCTFRanking( DWORD dwID )
{
	...
	for ( ; pos != end; pos++ )
	{
		...
		PGLCHAR _pChar = GLGaeaServer::GetInstance().GetCharID( sRankInfo.dwCharID );
		if ( _pChar )
		{
			GLGaeaServer::GetInstance().SENDTOCLIENT( _pChar->m_dwClientID, &NetMsgMy );
		}
	}
	...
	if ( !pLandMan ) return;
	{
		...
		for ( ; pCharNode; pCharNode = pCharNode->pNext )
		{
			...
			if ( iter != pSW->m_mapCtfScore.end() ) 
			{
				...
				if ( sSwPRankInfo.wCharRanking == 0 )	continue;
...
bool GLTowerWarsFieldMan::WinnerField ( DWORD dwID, WORD wWINNER )
{
	GLTowerWars *pSW = Find ( dwID );
	if ( !pSW )	return false;
	pSW->m_wSchoolWinner = wWINNER;
	SetLastWinner( pSW->m_wSchoolWinner );
	GLGaeaServer::GetInstance().GetConsoleMsg()->Write( "CTF: FieldWinner ID:%d  %s", dwID, GLCONST_CHAR::strSCHOOLNAME[pSW->m_wSchoolWinner].c_str() );
