...
void GLCharacter::StartSkillProc ()
{
	...
	//	Note : 스킬 시작?E동시에 나타나는 이팩트. 발동.
	//
	EMELEMENT emELMT = ( pSkill->m_sAPPLY.emELEMENT==EMELEMENT_ARM ) ? GET_ITEM_ELMT() : (EMELEMENT_SPIRIT);

	if ( pSkill->m_sEXT_DATA.emSELFBODY==SKILL::EMTIME_FIRST && !RANPARAM::bSkillEffects)
	{
	...
	for( unsigned s=0;s<SKILL::SAPPLY::MAX_DESCRIPTOR;++s)
	{
		...
		{
			WORD wLevel = GETLEARNED_SKILL(pSkill->m_sBASIC.sNATIVEID)->wLevel;
			WORD wLevelLink = desc.sSPEC[ wLevel ].dwFLAG;
			SNATIVEID sNativeID = desc.sSPEC[ wLevel ].dwNativeID;
					
			ReqLinkSkill( sNativeID, wLevelLink );
			break;
		}
	}
	//	Note : 스킬 메시?E발생.
...
void GLCharacter::SkillReaction ( const STARGETID &sTargetID, const DWORD dwMR, const bool bcontinue, BOOL &bMove, D3DXVECTOR3 &vMoveTo )
{
	...
	if ( emTARGET==TAR_SPEC || emTARGET==TAR_SELF_TOSPEC )
	{
		...
		if ( pTarget )
		{
			...
			{
				// 타겟이 움직인 거리가 공격 가능거리보다 ?E멀?E다시 타겟정보 갱신
	...
	if ( !pAnicont )
	{
		CDebugSet::ToLogFile ( "스킨 에니메이션이 존제하?E않습니다. ANIM %d, ANIS %d", pSkill->m_sEXT_DATA.emANIMTYPE, pSkill->m_sEXT_DATA.emANISTYPE );
		return;
	}
	...
	else if ( emTARGET==TAR_ZONE_SPEC )
	{
		...
		m_vDir = vDirection;
		//D3DXMATRIX matTrans;
		//DxEffMouseChar::GetInstance().NewEffGroup ( GLCONST_CHAR::strGUI_ATTACK_AREA_CIRCLE_EFFECT.c_str(), matTrans, &vDirection );
...
void GLCharacter::SK_EFF_SELFZONE ( const SANIMSTRIKE &sStrikeEff, const SKILL::EMEFFECTPOS emPOS, const char* const szZONE_EFF, STARGETID *pTarget )
{
	...
	if ( emPOS == SKILL::EMPOS_STRIKE )
	{
		...
		else if ( m_pSkinChar->m_pSkinCharPiece[PIECE_RFINGER] ) //fix abl strike
		{
			STRIKE::CSELECTOR Selector ( vTARPOS, m_pSkinChar->m_pSkinCharPiece[PIECE_RFINGER]->m_PartArray );
			BOOL bOk = Selector.SELECT ( sStrikeEff, sStrike ,true);
			if ( bOk )
			{
				bTRANS = TRUE;
...
void GLCharacter::SK_EFF_TARG ( const PGLSKILL pSkill, const SANIMSTRIKE &sStrikeEff, const STARGETID &_sTarget )
{
	...
	if ( pSkill->m_sEXT_DATA.emTARG_POSA == SKILL::EMPOS_STRIKE )
	{
		...
		else if ( m_pSkinChar->m_pSkinCharPiece[PIECE_RFINGER] ) //fix abl strike
		{
			...
			STRIKE::CSELECTOR Selector ( vSTRPAR, m_pSkinChar->m_pSkinCharPiece[PIECE_RFINGER]->m_PartArray );
			BOOL bOk = Selector.SELECT ( sStrikeEff, sStrike ,true);
			if ( bOk )
			{
				bTRANS = TRUE;
...
//	D3DXVECTOR3 vDir = sTarget.vPos - m_vPos;
	...
	else
	{
		...
		//	Note : 목표 위치 이펙트.
		if ( pSkill->m_sEXT_DATA.emTARGZONE01==SKILL::EMTIME_TARG_OVR )
		{}//pEffSingleG->AddEffAfter ( EFFASINGLE, NULL );
		...
		//	Note : 목표 ?E이펙트.
		if ( pSkill->m_sEXT_DATA.emTARGBODY01==SKILL::EMTIME_TARG_OVR )
		{}//pEffSingleG->AddEffAfter ( EFFABODY, NULL );
...
void GLCharacter::SkillEffect ( const SANIMSTRIKE &sStrikeEff )
{
	...
	else if ( pSkill->m_sBASIC.emIMPACT_TAR == TAR_SELF_TOSPEC )
	{
		...
		//	Note : 목표 지?E이펙트에 실?E보내는 이펙트.
		//		( 실?E보낼때는 목표 위치 이펙트의 위치는 자동으로 지?E이팩트의 종?E지점으로 과府. )
...
BOOL GLCharacter::SkillProc ( float fElapsedTime )
{
	...
	{
		return TRUE;		// 예?E〈?FALSE 여서 IDLE 상태로 바끔勃다. 지금은 TRUE를 주엉幕 상태변화를 안준다.
	}
...
BOOL GLCharacter::RECEIVE_SKILLFACT ( const SNATIVEID skill_id, const WORD wlevel, const DWORD dwSELECT )
{
	...
	//	Note : 지속?E스킬일 경?E최?E슬롯을 찾아서 스킬 효과를 넣엉棚.
	//
	if ( bHOLD )
	{
