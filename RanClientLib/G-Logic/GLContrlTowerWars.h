...
namespace GLMSG
{
	...
	struct SNET_TOWERWARS_START_ENTRY
	{
		NET_MSG_GENERIC			nmg;

		SNET_TOWERWARS_START_ENTRY () 
		{
			nmg.dwSize = sizeof(SNET_TOWERWARS_START_ENTRY);
			nmg.nType = NET_MSG_GCTRL_TOWERWARS_START_ENTRY;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	...
	struct SNET_TOWERWARS_WINNER_FLD
	{
		NET_MSG_GENERIC			nmg;
		DWORD	dwID;
		WORD	wWINNERSCHOOL;

		SNET_TOWERWARS_WINNER_FLD () 
			: wWINNERSCHOOL(10)
			, dwID(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_TOWERWARS_WINNER_FLD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNET_TOWERWARS_WINNER_BRD
	{
		enum { TEXT_LEN = 33, };
		...
		SNET_TOWERWARS_WINNER_BRD ()
			: wSCHOOL(0)
		{
			memset(szName, 0, sizeof(char) * TEXT_LEN);

			nmg.dwSize = sizeof ( SNET_TOWERWARS_WINNER_BRD );
			nmg.nType  = NET_MSG_GCTRL_TOWERWARS_WINNER_BRD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNETPC_SERVER_TOWERWARS_WINNER_AG
	{
		NET_MSG_GENERIC		nmg;
		DWORD				dwID;
		WORD				wSCHOOL;

		SNETPC_SERVER_TOWERWARS_WINNER_AG ()
			: dwID(0)
			, wSCHOOL(0)
		{
			nmg.dwSize = sizeof(*this);
			nmg.nType = NET_MSG_GCTRL_REQ_TO_AGENT_TOWERWARS_WINNER;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNET_TOWERWARS_CTF_EXIT_MAP
	{
		NET_MSG_GENERIC				nmg;

		SNET_TOWERWARS_CTF_EXIT_MAP ()
		{
			nmg.dwSize = sizeof ( SNET_TOWERWARS_CTF_EXIT_MAP );
			nmg.nType  = NET_MSG_GCTRL_TOWERWARS_CTF_EXIT_MAP;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
