...
void GLItemMixMan::CleanUp()
{
	...
	//dmk14 | 11-1-16 | item compound
	m_ItemMixDummy.clear();
...
void GLItemMixMan::insert ( ITEM_MIX& sItemMix )
{
	if ( m_bServer )
	{
		...
		//dmk14 | 11-1-16 | item compound
		m_ItemMixDummy.insert( std::make_pair( sItemMix.dwKey, sItemMix ));
	}
	else
	{
...
//dmk14 | 11-1-16 | item compound
const ITEM_MIX* GLItemMixMan::GetItemMix( DWORD dwKey, bool bClient )
{
	if ( !bClient ) return NULL;
	
	ITEMMIX_KEYMAP_ITER pos = m_ItemMixDummy.find(dwKey);
	if ( pos==m_ItemMixDummy.end() )		return NULL;
