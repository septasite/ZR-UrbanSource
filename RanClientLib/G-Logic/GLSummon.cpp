...
BOOL GLSUMMON::LoadFile ( const char* szFileName, SUMMON_TYPE emType )
{
	...
	cFILE.getflag( "fWalkArea", 1, 1, m_fWalkArea ); // 걸엉雹만 하는 거리
	cFILE.getflag( "fOwnerDistance", 1, 1, m_fOwnerDistance ); // 주인과의 유?E거리
	cFILE.getflag( "fPetDistance", 1, 1, m_fPetDistance );  // ?E珦?유?E거리
...
EMSKILLCHECK GLSUMMON::CHECHSKILL ( DWORD dwAType,  bool bNotLearn )
{
	...
	else
	{
		...
		//	Note : 스킬 딜래이 타임이 지낫는?E점검.
		//
		if ( m_fACTIONDELAY[dwAType] > 0.0f )	
			return EMSKILL_DELAYTIME; // 스킬 딜래이 시간이 지나?E않음.
	}
						   ...
						   DWORD dwWeatherFlag/* =NULL */, DWORD dwDivCount/* =0 */ )
{
	...
	if ( pSkill )
	{
		...
		case SKILL::EMAPPLY_MAGIC:
			nDEFENSE = 0;						//	마법 공격일때는 일반 방엉澹 무시됨.
			nDEFAULT_DEFENSE = 0;				//	마법 공격일때는 일반 방엉澹 무시됨.
			nITEM_DEFENSE = 0;					//	마법 공격일때는 일반 방엉澹 무시됨.
...
void GLSUMMON::UPDATE_DATA ( float fTime, float fElapsedTime, BOOL bClient )
{
	...
	float m_fINCR_HP = 0.3f * 0.01f;	//	단위시간?EHP 회복율(%)
	float m_fINCR_MP = 0.3f * 0.01f;	//	단위시간?EMP 회복율(%)
	float m_fINCR_SP = 0.5f;			//	단위시간?EMP 회복율(%)
	...
	for ( int i=0; i<SKILLFACT_SIZE; ++i )
	{
		...
		//	클라이언트가 아닐 경?E여기서 스킬 이펙트를 비활성화, 클라이언트는 UpdateSkillEffect()에서 함.
		...
		for( unsigned s=0;s<SKILL::SAPPLY::MAX_DESCRIPTOR;++s)
		{
			...
			case EMSPECA_ATTACKVELO:
				//	공격 속도 계념이므로 ( 공격 시간을 -0.1 (-10%) 감소 시키?E위해서는 부호 반?E?하여야함. )
	...
	for ( i=0; i<EMBLOW_MULTI; ++i )
	{
		...
		case EMBLOW_FROZEN:
			m_fSTATE_MOVE += sSTATEBLOW.fSTATE_VAR1;			//	이동속도 감소.
			m_fSTATE_DAMAGE += sSTATEBLOW.fSTATE_VAR2;			//	타격증?E
			break;
...
EMSKILLCHECK GLSUMMON::CHECHSKILL ( DWORD dwAType )
{
	...
	//	Note : 스킬 딜래이 타임이 지낫는?E점검.
	//
	if ( m_fACTIONDELAY[dwAType] > 0.0f )									return EMSKILL_DELAYTIME; // 스킬 딜래이 시간이 지나?E않음.
...
namespace COMMENT
{
	std::string SUMMON_TYPE[SUMMON_TYPE_SIZE] = { "SUMMONTYPE_A"};
};