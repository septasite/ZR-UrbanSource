...
HRESULT GLCrowClient::CreateCrow ( GLLandManClient *pLandManClient, SDROP_CROW *pCrowDrop, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if( !m_pCrowData )
	{
		...
		CDebugSet::ToLogFile ( "GLCrowClient::CreateCrow()-GLCrowDataMan::GetCrowData() [%d/%d] 가져오?E실패.", sMobID.wMainID, sMobID.wSubID );
		return E_FAIL;
	}
	...
	if ( !pCharData )
	{
		...
		{
			CDebugSet::ToLogFile ( "GLCrowClient::CreateCrow()-DxSkinCharDataContainer::LoadData() [%s] 가져오?E실패.", m_pCrowData->GetSkinObjFile() );
		}
		return E_FAIL;
	}
...
HRESULT GLCrowClient::FrameMove ( float fTime, float fElapsedTime )
{
	...
	//	Note : 공격 목표의 유효성 검?E
	//		(주의) 타갯정보 사?E갋반드시 호출하여 유효성 검?E필?E
	//
...
void GLCrowClient::MsgProcess ( NET_MSG_GENERIC* nmg )
{
	...
	case NET_MSG_GCTRL_CROW_MOVETO:
		{
			...
			// 서?E클라이언트간의 셀이 서로 다르?E정정해주엉雹 정확한 웨이포인트로
			// 이동하게 된다. 하지만 추격중이?E지정해주?E않는다.
	...
	case NET_MSG_GCTRL_SKILLHOLD_RS_BRD:
		{
			...
			//	Note : 스킬 fact 들을 종?E
			//		바로 리?E하?E않?E여기서 시간 조종하여 정?E종료되게 함. ( 이팩트 종?E때문. )
			for ( int i=0; i<SKILLFACT_SIZE; ++i )
			{
				if ( pNetMsg->bRESET[i] )
				{
	...
	default:
		CDebugSet::ToListView ( "GLCrowClient::MsgProcess() 분류되?E않은 메시?E수신. TYPE[%d/%d]", nmg->nType, nmg->nType-NET_MSG_GCTRL );
		break;
	};
}
