...
HRESULT GLCharClient::UpateAnimation ( float fTime, float fElapsedTime )
{
	...
	if ( m_bSafeZone && GLGaeaClient::GetInstance().GetActiveMap()->GetMapID().wMainID == 22 ) bPeaceZone = TRUE;
...
//	if ( !IsSTATE (EM_ACT_PEACEMODE) ) SetSTATE(EM_ACT_PEACEMODE);
	...
	//	Note : ?E?EEÁö¿ªÀÏ¶§ Ç¥½Ã ¾ÈµÇ¾ß ÇÏ´Â ¾ÆÀÌÅÛ(´Ü°Ë,ÅõÃ´) ÀÎ?E°Ë»çÈÄ ·£?E²ô?E
	//
	if ( bPeaceZone )
	{
		EMSLOT emRHand = GetCurRHand();
...
HRESULT GLCharClient::FrameMove ( float fTime, float fElapsedTime )
{
	...
	//		(ÁÖÀÇ) Å¸°¹Á¤º¸ »ç?EE¹Ýµå½Ã È£ÃâÇÏ¿© À¯È¿¼º °Ë?EÇÊ?E
	//
