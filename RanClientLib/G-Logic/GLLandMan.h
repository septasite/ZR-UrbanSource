...
struct SSKILLACTEX
{
	...
	bool		bDash;	//add dash - SeiferXIII 6/30/13
	bool		bTeleport;	//add teleport -   2-8-2014
	...
	SSKILLACTEX () :
		...
		bDash (false),	//add dash - SeiferXIII 6/30/13
		bTeleport (false),	//add teleport -   2-8-2014
		fPUSH_PULL_B(0.0f)	//add pushpull
	{
	}
...
public:
	BOOL LoadFile ( const char *szFile, std::string strWLD = "" );
...
public:
	void PushPullAct( const STARGETID dwActorID, const STARGETID dwReActorID, const float fPUSH_PULL, const float fPUSH_PULL_B, const bool bPush, const bool bPull, const bool bDash, const bool  bTeleport ); //add pushpull //modify //add dash - SeiferXIII 6/30/13 //add teleport -   2-8-2014
