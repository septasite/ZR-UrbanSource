...
void GLCrow::SkillProc ( BOOL bLowSP )
{
	...
	for ( WORD i=0; i<m_wTARNUM; ++i )
	{
		...
		for ( WORD j=0; j<sSKILL_DATA.wAPPLYNUM; ++j )
		{
			...
			{
			case SKILL::EMFOR_HP:
				if ( sSKILL_DATA.fBASIC_VAR < 0.0f )		//	?E璨“?피해를 주는 스킬.
				{
//					bCRITICAL = CALCDAMAGE ( nVAR_HP, bShock, sTARID, m_pLandMan, pSkill, wSKILL_LVL, GLPeriod::GetInstance().GetWeather(), sAniAttack.m_wDivCount );
			...
			{
				const SKILL::SSPEC &sSKILL_SPEC = pSkill->m_sAPPLY.spec[s].sSPEC[wSKILL_LVL];
				switch ( pSkill->m_sAPPLY.spec[s].emSPEC )
				{
...
//			float fPOWER = GLOGICEX::WEATHER_BLOW_POW ( sBLOW.emTYPE, GLPeriod::GetInstance().GetWeather(), m_pLandMan->IsWeatherActive() );
		...
		for( unsigned s=0;s<SKILL::SAPPLY::MAX_DESCRIPTOR;++s)
		{
			...
			case EMSPECA_PUSHPULL:
				...
				if ( RANDOM_POS > sSKILL_SPEC.fVAR2 )		break;	//	Note : 발생확율 적?E
			...
			case EMSPECA_RECBLOW:
				//	Note : 상태 이?E치?E
				//
				sSKILLACTEX.dwCUREFLAG |= pSkill->m_sAPPLY.spec[s].sSPEC[wSKILL_LVL].dwFLAG;
				break;
...
BOOL GLCrow::SkillProcess ( float fElapsedTime )
{
	...
	//	이상증상에 의해 데이타가 ?E갋처리되?E않은경?E
	//	마지막에 한꺼번에 처리하?E FALSE를 리턴한다.
	//
...
void GLCrow::CURE_STATEBLOW ( DWORD dwCUREFLAG )
{
	...
	//	Note : 주변 클라이언트들에게 메세?E?E?
	//
	SendMsgViewAround ( (NET_MSG_GENERIC*) &NetMsgBRD );
}

void GLCrow::BUFF_REMOVE( DWORD dwBuffFlag, const SNATIVEID skill_id )
{
	...
	for ( int i=0; i<SKILLFACT_SIZE; ++i )
	{
		...
		else if ( dwBuffFlag == EMSPECA_BUFF_REMOVE && pSkillTarget->m_sBASIC.emIMPACT_SIDE == SIDE_OUR )
		{
...
void GLCrow::VAR_BODY_POINT ( const EMCROW emACrow, const DWORD dwAID, const BOOL bPartySkill, int nvar_hp, int nvar_mp, int nvar_sp )
{
	...
	if ( bDamage )
	{
		...
		//  소환?E공격일 경?ESummonDamageLog에 기록한다.
	...
	{
		//	Note :타겟이 존제하?E않을때 공격자가 있으?E타겟으로 따遝 시도.
		//
