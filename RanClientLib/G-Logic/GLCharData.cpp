...
void SSKILLFACT::RESET ()
{
	...
	std::fill_n( adon, (int)SKILL::SAPPLY::MAX_DESCRIPTOR, IMPACT_DESCRIPTOR() );
	std::fill_n( spec, (int)SKILL::SAPPLY::MAX_DESCRIPTOR, SPEC_DESCRIPTOR() );
	dwSpecialSkill = 0;
	bRanderSpecialEffect = FALSE;
}

//dmk14 | 11-13-16 | SIMPACT_DESCRIPTOR
void SSKILLFACT::IMPACT_DESCRIPTOR::Assign( const SKILL::SIMPACT_DESCRIPTOR &d, WORD level )
{
	emADDON = d.emADDON;
	fADDON_VAR = d.fADDON_VAR[level];
	fADDON_VAR2 = d.fADDON_VAR2[level];
}

const SSKILLFACT::IMPACT_DESCRIPTOR *SSKILLFACT::GetAdon( EMIMPACT_ADDON emADDON ) const
{
	for ( unsigned s = 0; s < SKILL::SAPPLY::MAX_DESCRIPTOR; ++s )
		if ( adon[ s ].emADDON == emADDON )
			return &adon[ s ];

	return 0;
}

void SSKILLFACT::SPEC_DESCRIPTOR::Assign( const SKILL::SSPEC_DESCRIPTOR &d, WORD level )
{
	emSPEC = d.emSPEC;
	fSPECVAR1 = d.sSPEC[ level ].fVAR1;
	fSPECVAR2 = d.sSPEC[ level ].fVAR2;
	dwSPECFLAG = d.sSPEC[ level ].dwFLAG;
	dwNativeID = d.sSPEC[ level ].dwNativeID;
}

const SSKILLFACT::SPEC_DESCRIPTOR *SSKILLFACT::GetSpec( EMSPEC_ADDON emSPEC ) const
{
	for ( unsigned s = 0; s < SKILL::SAPPLY::MAX_DESCRIPTOR; ++s )
