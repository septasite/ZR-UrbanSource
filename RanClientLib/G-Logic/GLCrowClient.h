...
public:
	...
	SGLNODE<GLCrowClient*>*	m_pGlobNode;		//	∏  ∏ÆΩ∫∆Æ ≥ÅEÅE
	CLIENTQUADNODE*			m_pQuadNode;		//	ƒıµÅE∆Æ∏Æ ≥ÅEÅE
	SGLNODE<GLCrowClient*>*	m_pCellNode;		//	ºø ∏ÆΩ∫∆Æ ≥ÅEÅE
...
protected:
	...
	STARGETID			m_sTargetID;			//	∏Ò«• ¿Ø¥÷.

	WORD				m_wTARNUM;				//	≈∏∞Ÿ º˝¿⁄.
	STARID				m_sTARIDS[EMTARGET_NET];//	≈∏∞ŸµÅE
	D3DXVECTOR3			m_vTARPOS;				//	≈∏∞Ÿ ¿ßƒ°.

	SCROWATTACK*		m_pAttackProp;			//	∞¯∞› º”º∫.

protected:
	float				m_fIdleTime;			//	»ﬁΩƒ Ω√∞£.
	float				m_fStayTimer;			//	»ﬁΩƒ µÙ∑°¿Ã.
	float				m_fAttackTimer;			//	∞¯∞› µÙ∑°¿Ã.
	float				m_fTargetTimer;			//	≈∏∞π ∞ÀªÅE≈∏¿Ã∏”.
	float				m_fMoveDelay;			//	¿Ãµø µÙ∑°¿Ã.
...
public:
	...
	virtual GLPWDATA GetHp () const			{ return GLPWDATA(m_dwNowHP,m_pCrowData->m_dwHP); }
