...
inline HRESULT GLChar::MsgAttack ( NET_MSG_GENERIC* nmg )
{
	...
	if ( pTARGET->GetCrow()==CROW_PC )
	{
		...
		bool bTowerWars = false;
			...
			if ( !(bGuidBattleMap || bClubDeathMatch || bCTF || bSchoolWars || bTowerWars || bSCHOOL_FREEPK || bFreePKMap || bBRIGHTEVENT || IsConflictTarget(pTARGET) || bClubBattle ) )
			{
...
inline void GLChar::DoReBirth2StartMap ( DWORD dwMapID, DWORD dwGateID, bool bBool )
{
	...
	if ( m_pLandMan->m_bTowerWarsMap )
	{
		...
		PDXLANDGATE pTWGATE = m_pLandMan->GetLandGateMan().FindLandGate(pTW->GetReviveGate( m_wSchool ));
		BOOL bOk = GLGaeaServer::GetInstance().RequestReBirth ( m_dwGaeaID, pTW->m_dwClubMap, pTWGATE->GetGateID(), pTWGATE->GetGenPos( DxLandGate::GEN_RENDUM ) );
	...
	{
		GLRoyalRumble* pRR = GLRoyalRumbleFieldMan::GetInstance().Find ( m_pLandMan->m_dwRoyalRumbleMapID );
		PDXLANDGATE pRRGATE = m_pLandMan->GetLandGateMan().FindLandGate( (DWORD)RANDOM_GATE );
		BOOL bOk = GLGaeaServer::GetInstance().RequestReBirth ( m_dwGaeaID, pRR->m_dwClubMap, pRRGATE->GetGateID(), pRRGATE->GetGenPos( DxLandGate::GEN_RENDUM ) );
		if ( !bOk )	DEBUGMSG_WRITE ( "GLGaeaServer::GetInstance().RequestReBirth () bOk false" );
		return;
	}
...
inline HRESULT GLChar::MsgReqReBirth ( NET_MSG_GENERIC* nmg )
{
	...
	if ( pNetMsg->bCTFMAP )
	{
		...
		if ( pLandManCTF )
		{
			...
			{
				GLTowerWars* pTW = GLTowerWarsFieldMan::GetInstance().Find ( pLandManCTF->m_dwTowerWarsMapID );
				if ( pTW && pTW->IsEnterMap() )	
				{
					PDXLANDGATE pTWGATE = pLandManCTF->GetLandGateMan().FindLandGate(pTW->GetReviveGate( m_wSchool ));
					if ( pTWGATE )
					{
						m_sStartMapID = pTW->m_dwClubMap;
						m_dwStartGate = pTWGATE->GetGateID();
						m_vStartPos = pTWGATE->GetGenPos( DxLandGate::GEN_RENDUM );
						DoReBirth2StartMap ();
					}
				}
			}
			...
			{
				GLRoyalRumble* pRR = GLRoyalRumbleFieldMan::GetInstance().Find ( pLandManCTF->m_dwRoyalRumbleMapID );
				if ( pRR && pRR->IsEnterMap( m_dwCharID ) )	
				{
					srand(time(NULL));
					PDXLANDGATE pRRGATE = pLandManCTF->GetLandGateMan().FindLandGate( (DWORD)RANDOM_GATE );
					if ( pRRGATE )
					{
						m_sStartMapID = pRR->m_dwClubMap;
						m_dwStartGate = pRRGATE->GetGateID();
						m_vStartPos = pRRGATE->GetGenPos( DxLandGate::GEN_RENDUM );
						DoReBirth2StartMap ();
						//DoReBirth2StartMap ( pRR->m_dwClubMap , pRRGATE->GetGateID(), true);
					}
				}
			}
		}
...
//link skill
inline HRESULT GLChar::MsgReqLinkSkillEff ( NET_MSG_GENERIC* nmg ) 
{
...
//			CDebugSet::ToLogFile ( "Current Active : %d, bActive : %d",m_bVehicle, bActive );
		...
		// 버프스킬 제거
		// Disable Remove Buff on vehicle -- SeiferXIII 7/20/13
		/*
...
//	case NET_MSG_GCTRL_REQ_HOLD_ARRAY:		MsgReqHoldArray(nmg);			break;
	...
	case NET_MSG_GCTRL_REQ_DISABLESKILLEFF: MsgReqDisableSkillEff(nmg);		break; //disable skill effect NaJDeV
...
//	case NET_MSG_GCTRL_REQ_HOLDARRAY_TO_INVEN: MsgReqHoldArrayToInven(nmg); break;
	...
	//case NET_MSG_GCTRL_REQ_STATSUPCMD:		MsgReqStatsUpCmd(nmg);			break;
	case NET_MSG_GCTRL_REQ_LINKSKILLEFF:	MsgReqLinkSkillEff(nmg);		break;		//link skill
	...
	case NET_MSG_INVEN_WRAP:				MsgReqInvenWrap(nmg);			break; // wrapper
	case NET_MSG_INVEN_REMOVE_WRAP:			MsgReqInvenRemoveWrap(nmg);		break; // wrapper
	case NET_MSG_INVEN_NONDROP:				MsgReqNonDrop(nmg);				break; //nondrop card Eduj
	case NET_MSG_INVEN_REMOVE_NONDROP:		MsgReqRemoveNonDrop(nmg);				break; //nondrop card Eduj
	case NET_MSG_REBUILD_INPUT_MONEY:		MsgReqRebuildInputMoney(nmg);	break;
	case NET_MSG_GCTRL_INVEN_SCHOOL_CHANGE: MsgReqInvenSchoolChange(nmg);		break; //Change School
	...
	case NET_MSG_GCTRL_INVEN_ITEM_COMPOUND: MsgReqItemCompound( nmg );		break;
		// PET
	...
	default:
		{			
			DEBUGMSG_WRITE(_T("[WARNING] GLCharMsg::MsgProcess illegal message (%d)"),
				nmg->nType );
		}
		break;
	}
