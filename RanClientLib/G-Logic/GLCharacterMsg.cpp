...
#include "../RanClientUILib/Interface/ItemRebuild/ItemRebuild.h" //sealed card NaJDeV
...
void GLCharacter::MsgProcess ( NET_MSG_GENERIC* nmg )
{
	...
	case NET_MSG_GCTRL_REQ_TYRANNY_GETREWARD_FB: //convert
		{
			...
			CInnerInterface::GetInstance().PrintConsoleText( "You have received: %d Contribution Points.", pNetMsg->lnValue );
		}
		break;
	...
	case NET_MSG_GCTRL_INVEN_SCHOOL_CHANGE_FB:
		{
			GLMSG::SNETPC_INVEN_SCHOOL_CHANGE_FB *pNetMsg = (GLMSG::SNETPC_INVEN_SCHOOL_CHANGE_FB *) nmg;
		
			switch ( pNetMsg->emFB )
			{
			case EMINVEN_SCHOOL_CHANGE_FB_FAIL:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMINVEN_SCHOOL_CHANGE_FB_FAIL") );
				break;
			case EMINVEN_SCHOOL_CHANGE_FB_OK:
				{
					m_wSchool = (WORD) pNetMsg->dwID;
					
					CString strSchool;
					if ( m_wSchool == 0 ) strSchool.Format ("Sacred Gate");
					else if ( m_wSchool == 1 ) strSchool.Format ("Mystic Peak");
					else if ( m_wSchool == 2 ) strSchool.Format ("Phoenix");
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("EMINVEN_SCHOOL_CHANGE_FB_OK"), strSchool );
					DoModal( "You are now being directed to lobby.",  MODAL_INFOMATION, OK, MODAL_SCHOOL_CHANGE_END );
				}
				break;
			case EMINVEN_SCHOOL_CHANGE_FB_NOITEM:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMINVEN_SCHOOL_CHANGE_FB_NOITEM") );
				break;
			case EMINVEN_SCHOOL_CHANGE_FB_BADITEM:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMINVEN_SCHOOL_CHANGE_FB_BADITEM") );
				break;
			case EMINVEN_SCHOOL_CHANGE_FB_BADCLASS:
				CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::NEGATIVE, ID2GAMEINTEXT("EMINVEN_SCHOOL_CHANGE_FB_BADCLASS") );
				break;
			case EMINVEN_SCHOOL_CHANGE_FB_COOLTIME:
				break;
			};
		}
		break;
	...
	case NET_MSG_GCTRL_UPDATE_PKWIN:
		{
			...
			//PKWINUP();
			PKSTREAK();
			CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::LIGHTSKYBLUE, ID2GAMEINTEXT("PK_KILLED"), pNetMsg->szName );
				
			SNATIVEID mapID = GLGaeaClient::GetInstance().GetActiveMapID();
			// SW / SG E-Room / PY / TH 
			//if( mapID.wMainID == 254 && mapID.wSubID == 0 || mapID.wMainID == 211 && mapID.wSubID == 0 || mapID.wMainID == 23 && mapID.wSubID == 0 || mapID.wMainID == 16 && mapID.wSubID == 0 )
			//{
			PKWINUP();
			//}

			if( m_dwPkStreak == 2) CInnerInterface::GetInstance().SET_PK_COMBO_ID ( PK_DOUBLEKILL );
			else if( m_dwPkStreak == 3) CInnerInterface::GetInstance().SET_PK_COMBO_ID ( PK_TRIPLEKILL );
			else if( m_dwPkStreak == 4) CInnerInterface::GetInstance().SET_PK_COMBO_ID ( PK_ULTRAKILL );
			else if( m_dwPkStreak >= 5) CInnerInterface::GetInstance().SET_PK_COMBO_ID ( PK_RAMPAGE );

			if( m_dwPkStreak == 2) DxSoundLib::GetInstance()->PlaySound ( "PK_DOUBLE_WAV" ); 
			else if( m_dwPkStreak == 3) DxSoundLib::GetInstance()->PlaySound ( "PK_TRIPLE_WAV" ); 
			else if( m_dwPkStreak == 4) DxSoundLib::GetInstance()->PlaySound ( "PK_QUADRA_WAV" );  
			else if( m_dwPkStreak >= 5) DxSoundLib::GetInstance()->PlaySound ( "PK_PENTA_WAV" );
			...
			/*if( m_dwPkStreak >= 5)
			{
				...
				strTEXT.Format ( ID2SERVERTEXT("EMPLAYERACT_REPULSE_RAMPAGE"), pChar->m_szName, szMAP_NAME );
	...
	case NET_MSG_GCTRL_UPDATE_PKLOSS:
		{
			...
			PKLOSSUP();
			{
				...
				m_sKILLFACT[m_sKILL] = sKILL;
				for ( int i=0; i<KILLFACT_SIZE; ++i )
				{
					SDROP_KILLFACT &sSKEFF = m_sKILLFACT[i];
				}
			...
			CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::DISABLE, ID2GAMEINTEXT("PK_KILLEDBY"), pNetMsg->szNameKilledBy );		
		}	
		break;
	...
	case NET_MSG_GCTRL_PICKUP_MONEY:
		{
			...
			GLMSG::SNET_SCHOOL_WARS_END_BRD *pNetMsgSw = (GLMSG::SNET_SCHOOL_WARS_END_BRD *)nmg;
			char szSchoolChar[32] = {0,};

			switch(pNetMsgFB->m_wSchool)
			{
				case 0:	strcpy( szSchoolChar, "Sacred Gate" ); break;
				case 1:	strcpy( szSchoolChar, "Mystic Peak" ); break;
				case 2:	strcpy( szSchoolChar, "Phoenix" ); break;
			}
		
			m_lnMoney = pNetMsg->lnMoney;
			if( GLGaeaClient::GetInstance().GetSwSchoolWinner() == m_wSchool)		CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::AQUA, ID2GAMEINTEXT("PICKUP_MONEY_SWBONUS"), pNetMsg->lnPickUp );
	...
	case NET_MSG_GCTRL_PICKUP_ITEM:
		{
			...
			if ( pITEM )
			{
				...
				{
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::VIOLET, ID2GAMEINTEXT("PICKUP_ITEM_RARE"), strName );
				}
				else if ( pITEM->sBasicOp.emLevel == LEVEL_VERYRARE)
				{
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::VIOLET, ID2GAMEINTEXT("PICKUP_ITEM_RARE"), strName );
				}
				else if ( pITEM->sBasicOp.emLevel == LEVEL_EPIC)
				{
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::VIOLET, ID2GAMEINTEXT("PICKUP_ITEM_RARE"), strName );
				}
				else
				{
					CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, ID2GAMEINTEXT("PICKUP_ITEM"), strName );
				}
				DxSoundLib::GetInstance()->PlaySound ( "PICKUP_ITEM" );
			}
		} 
	...
	case NET_MSG_GCTRL_INVEN_INSERT:
		{
			...
			{
				CInnerInterface::GetInstance().RefreshVehiclePage();
			}
		}
		break;
	...
	case NET_MSG_GCTRL_REQ_LEARNSKILL_FB:
		{
			...
			if ( pNetMsg->emCHECK == EMSKILL_OK )
			{
				...
				//	Note : ∆‰Ω√∫ÅEΩ∫≈≥¿Ã ∫Ø»≠ «“∂ß √ ±ÅEºˆƒ°µÈ¿ª ∏µŒ ¿ÅE∞ËªÅE—¥Ÿ.
				//
	...
	case NET_MSG_GCTRL_REQ_SKILLQ_FB:
		{
			...
			//	Note : ∑± Ω∫≈≥∑Œ ¡ˆ¡§µ» Ω∫≈≥¿Ã ¡∏¡¶«œ¡ÅEæ ¿ª ∞ÊøÅE¡ˆ¡§«ÿ¡‹.
	...
	case NET_MSG_GCTRL_REGEN_GATE_FB:
		{
			...
			case EMREGEN_GATE_SUCCEED:
				{
					// Note : Ω√¿€±Õ»Øƒ´µÂø°º≠ ªÁøÅE… ¡§∫∏∏¶ πﬁΩ¿¥œ¥Ÿ.
			...
			case EMREGEN_CHARRESET_SUCCEED:
				{
					...
					//	Note : ∏∏æÅESKILL ªÁøÅEﬂ¿Ã∂Û∏ÅE∏µŒ ∏Æº¬.
					//
...
//			CInnerInterface::GetInstance().SET_VNGAINTYPE_GAUGE( 0, 300 );
	...
	case NET_MSG_GCTRL_INVEN_GARBAGEOPEN_FB:
		...
		// ∂Ùƒø ∞ÅEÆ¿Œ∞˙¿« ¥ÅE≠∏¶ ¡¶ø‹«— ±◊π€¿« πÊπ˝¿∏∑Œ √¢∞ÅEø≠∂ÅE( ±‰±ﬁ √¢∞ÅEø¨∞ÅEƒ´µÅE.. etc )
	...
	case NET_MSG_GCTRL_PLAYERKILLING_ADD:
		{
			...
			if ( !( bBRIGHTEVENT||bSCHOOLFREEPK||pLand->m_bClubBattle||pLand->m_bClubDeathMatch||pLand->m_bSchoolWars||pLand->m_bRoyalRumble ||pLand->m_bTowerWars) )
			{
				bADD = ADD_PLAYHOSTILE ( pNetMsg->dwCharID, pNetMsg->bBAD );
			}

			// «–ø¯∞£ ¿⁄¿Ø««ƒ… ¡¯«‡¡ﬂ¿Ã∏ÅEªÛ¥ÅE° ∞∞¿∫ «–±≥¿œ∞ÊøÅE¿˚¥ÅE‡¿ß ¥ÅEÛ¿⁄∑Œ ±∏∫–«œø© ∞ÅEÆ«‘.
	...
	case NET_MSG_GCTRL_INVEN_BIKECOLOR_CHANGE_FB: //add bike color
		{
			...
			case EMINVEN_BIKE_CHANGE_FB_FAIL:
				BikeColorChange( m_sVehicle.m_wColor ); 
	...
	case NET_MSG_GCTRL_UPDATE_LASTCALL:
		{
			...
			// Note : ¡˜¿ÅEÕ»Øƒ´µÂø°º≠ ªÁøÅE… ¡§∫∏∏¶ πﬁΩ¿¥œ¥Ÿ.
...
//				CInnerInterface::GetInstance().WAITSERVER_DIALOGUE_OPEN ( ID2GAMEINTEXT("CHINA_GAIN_CLOSEGAME"), WAITSERVER_CLOSEGAME, 30 );
	...
	case NET_MSG_GM_LIMIT_EVENT_BEGIN_FB:
		{
			...
			/*CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, 
														  "%d ∑π∫ß ∫Œ≈Õ %d ∑π∫ß ±˚›ÅE%d∫– «√∑π¿Ã %d ∫ŒΩ∫≈Õ %.2f ∞Ê«Ëƒ° %.2f µÂ∂¯¿≤",
	...
	case NET_MSG_GM_LIMIT_EVENT_END_FB:
		{
			//CInnerInterface::GetInstance().PrintMsgText ( NS_UITEXTCOLOR::PALEGREEN, "¿Ã∫•∆Æ ¡æ∑ÅE!" );
	...
	case NET_MSG_REQ_GATHERING_RESULT:
		...
		break;
	default:
		CDebugSet::ToListView ( "GLCharacter::MsgProcess() ∫–∑˘µ«¡ÅEæ ¿∫ ∏ﬁΩ√¡ÅEºˆΩ≈. TYPE[%d]", nmg->nType );
		break;
	};
...
void GLCharacter::MsgDefenseSkillActive( GLMSG::SNETPC_DEFENSE_SKILL_ACTIVE* nmg )
{
	...
	//	Note : Reaction ø°º≠ ¿Ãµø¿ª ø‰√ª«— ∞ÊøÅE
	//
	if ( bMove )
	{
