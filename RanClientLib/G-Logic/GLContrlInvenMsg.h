...
{
	ITEMSHOP_ITEM_GET_FB_FAIL		= 0,	//	일반오류.
	ITEMSHOP_ITEM_GET_FB_OK		= 1,	//	성공.
	ITEMSHOP_ITEM_GET_FB_NOITEM	= 2,	//	아이템이 없음.
	ITEMSHOP_ITEM_GET_FB_NOINVEN	= 3,	//	인벤공간 부족.
};
...
namespace GLMSG
{
	...
	struct SNET_INVEN_WRAP // wrapper
	{
		NET_MSG_GENERIC		nmg;
		...
		SNET_INVEN_WRAP()
			...
			, wPosY( USHRT_MAX )
		{
			nmg.dwSize = sizeof( SNET_INVEN_WRAP );
			nmg.nType = NET_MSG_INVEN_WRAP;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNET_INVEN_REMOVE_WRAP // wrapper
	{
		NET_MSG_GENERIC		nmg;
		...
		SNET_INVEN_REMOVE_WRAP()
			: wPosX( USHRT_MAX )
			, wPosY( USHRT_MAX )
		{
			nmg.dwSize = sizeof( SNET_INVEN_REMOVE_WRAP );
			nmg.nType = NET_MSG_INVEN_REMOVE_WRAP;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	//---------------------------------------------------------------------------NET
	struct SNET_INVEN_NONDROP //nondrop card Eduj
	{
		NET_MSG_GENERIC		nmg;
		...
		SNET_INVEN_NONDROP()
			...
			, wPosY( USHRT_MAX )
		{
			nmg.dwSize = sizeof( SNET_INVEN_NONDROP );
			nmg.nType = NET_MSG_INVEN_NONDROP;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNET_INVEN_NONDROP_FB //nondrop card Eduj
	{
		NET_MSG_GENERIC		nmg;
        enum { MAX_ITEM_NAME = 70, };
		...
		SNET_INVEN_NONDROP_FB () 
			: bNonDrop(FALSE)
		{
			nmg.dwSize = sizeof(SNET_INVEN_NONDROP_FB);
			nmg.nType = NET_MSG_INVEN_NONDROP_BRD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);

			memset(szName, 0, sizeof(char) * CHAR_SZNAME);
            memset(szItemName, 0, sizeof(char) * MAX_ITEM_NAME);
		}
	};
	//--

	struct SNET_INVEN_REMOVE_NONDROP //nondrop card Eduj
	{
		NET_MSG_GENERIC		nmg;
		...
		SNET_INVEN_REMOVE_NONDROP()
			: wPosX( USHRT_MAX )
			, wPosY( USHRT_MAX )
		{
			nmg.dwSize = sizeof( SNET_INVEN_REMOVE_NONDROP );
			nmg.nType = NET_MSG_INVEN_REMOVE_NONDROP;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};

	struct SNET_INVEN_REMOVE_NONDROP_FB //nondrop card Eduj
	{
		NET_MSG_GENERIC		nmg;
        enum { MAX_ITEM_NAME = 70, };
		...
		SNET_INVEN_REMOVE_NONDROP_FB () 
			: bNonDrop(FALSE)
		{
			nmg.dwSize = sizeof(SNET_INVEN_REMOVE_NONDROP_FB);
			nmg.nType = NET_MSG_INVEN_REMOVE_NONDROP_BRD;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
	...
	struct SNET_GLOBAL_SUCCESS_GRIND_FB
	{
		NET_MSG_GENERIC		nmg;
		DWORD				dwCharID;
		DWORD				dwGrindUp;
		...
		SNET_GLOBAL_SUCCESS_GRIND_FB()
			:dwCharID(0),
			dwGrindUp(0)

		{
			nmg.dwSize = sizeof(SNET_GLOBAL_SUCCESS_GRIND_FB);
			nmg.nType = NET_MSG_GCTRL_GLOBAL_SUCCESS_GRIND_FB;
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	//---------------------------------------------------------------------------NET
	struct SNET_INVEN_GRINDING_FB
	{
		...
		EMGRINDING_RESIST_FB		emGrindResistFB;
		EMGRINDING_PERFECT_FB		emGrindPerfectFB;
		...
		SNET_INVEN_GRINDING_FB () 
			...
			, emGrindResistFB(EMGRINDING_RESIST_FAIL)
			, emGrindPerfectFB(EMGRINDING_PERFECT_FAIL)
		{
	...
	struct SNET_INVEN_ITEM_COMPOUND
	{
		NET_MSG_GENERIC		nmg;

		DWORD				dwKey;

		SNET_INVEN_ITEM_COMPOUND()
			: dwKey( UINT_MAX )
		{
			nmg.dwSize = sizeof( SNET_INVEN_ITEM_COMPOUND );
			nmg.nType = NET_MSG_GCTRL_INVEN_ITEM_COMPOUND;
			GASSERT( nmg.dwSize <= NET_DATA_BUFSIZE );
		}
	};
