...
std::vector<SFINDRESULT> GLPrivateMarket::FindItem( SSEARCHITEMDATA sSearchData )
{
	...
	for( ; pos != m_mapSearchItem.end(); ++pos )
	{
		...
		if( bFindUseStats )
		{
			if( sSearchData.sReqStats.dwPow != 0 && itemData.sReqStats.dwPow == 0 )	continue;
			if( sSearchData.sReqStats.dwDex != 0 && itemData.sReqStats.dwDex == 0 )	continue;
			if( sSearchData.sReqStats.dwSpi != 0 && itemData.sReqStats.dwSpi == 0 )	continue;
		}
