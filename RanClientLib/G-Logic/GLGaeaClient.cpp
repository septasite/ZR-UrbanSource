...
void GLGaeaClient::MsgProcess ( NET_MSG_GENERIC* nmg )
{
	...
	case NET_MSG_GCTRL_TOWERWARS_END_BRD:
		{
			...
			m_bTowerWars = false;
			m_fTowerWarsTimer = 0.0f;
			if ( m_vecCtfRankInfoAll.size() != 0 )
				{
					for(int i = 0; i < m_vecCtfRankInfoAll.size(); ++i )
					{
						STWP_RANK_INFO sPlayer = m_vecCtfRankInfoAll[i];
						if ( m_Character.GetCharID() == sPlayer.dwCharID )
						{
	...
	case NET_MSG_INVEN_NONDROP_BRD:
		...
		if ( pNetMsg->bNonDrop )		  
		 {
            //CInnerInterface::GetInstance().PrintMsgTextDlg ( NS_UITEXTCOLOR::AQUA, "'%s' used NondropCard '%s'  to 'Nondropable'.",  pNetMsg->szName, pNetMsg->szItemName );
		 }
		}
		 break;
