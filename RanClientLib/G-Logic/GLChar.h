...
public: //disable skill effect NaJDeV
	HRESULT MsgReqDisableSkillEff ( NET_MSG_GENERIC* nmg );

	HRESULT		MsgReqRebuildMoveSeal( NET_MSG_GENERIC* nmg ); //sealed card NaJDeV
//fix fieldcrash cause by itemmixfucking mix - Eduj
public:
	BOOL bCheckMixMaterial;
	void CheckMixMaterial( bool bCheck )
	{
		if ( bCheck ) bCheckMixMaterial = TRUE;
		else bCheckMixMaterial = FALSE;
	}
public:
...
public:
	...
	//add reborn
	virtual DWORD GetReborn () const				{ return m_nChaReborn; }
	...
	virtual bool IsImmune()		const			{ return m_bImmune; }	//dmk14 | 9-11-16 | immune
	...
	virtual void ReceivePushPull ( const D3DXVECTOR3 &vMovePos, const bool bReact , const bool bAdjustSpeed , const float fSpeed, const  bool  bDash, const  bool  bTeleport ); //add pushpull
...
public:
	...
	void ReceivePkLossPoint ( DWORD dwPkLoss, std::string strKilledBy , std::string strKilled  , DWORD dwGaeaID );
...
public:
	...
	DWORD DoRevive2Skill ( SNATIVEID skill_id, WORD wSKILL_LVL, unsigned descriptor );
...
public:
	...
	bool DeleteItem ( SNATIVEID sNID );	//dmk14 | 12-9-16 | item compound
...
//	HRESULT MsgReqHoldArrayToInven ( NET_MSG_GENERIC* nmg );
	...
	HRESULT	MsgReqLinkSkillEff ( NET_MSG_GENERIC* nmg );	//link skill
	...
	HRESULT MsgReqItemCompound( NET_MSG_GENERIC* nmg );		//dmk14 | 12-9-16 | item compound
	// PET
...
public:
	...
	HRESULT		MsgReqInvenWrap( NET_MSG_GENERIC* nmg ); // wrapper
	HRESULT		MsgReqNonDrop( NET_MSG_GENERIC* nmg ); //nondrop card Eduj
	HRESULT		MsgReqRemoveNonDrop( NET_MSG_GENERIC* nmg ); //nondrop card JohnArthurooo
	HRESULT		MsgReqInvenRemoveWrap ( NET_MSG_GENERIC* nmg );
