...
namespace GLMSG
{
	...
	struct SNETPET_REQ_USEPETCARD_FB
	{
		...
		DWORD			m_dwGUID;
		SNATIVEID		m_sPetID;
		SNATIVEID		m_sActiveSkillID[PETSLOT_SIZE];	//dmk14 | 11-21-16 | dual pet skill
		BOOL			m_bDualSkill;
			...
			m_bDualSkill(FALSE)
		{
			...
			std::fill_n( m_sActiveSkillID, (int)PETSLOT_SIZE ,NATIVEID_NULL());	//dmk14 | 11-21-16 | dual pet skill
			GASSERT(nmg.dwSize<=NET_DATA_BUFSIZE);
		}
	};
	...
	struct SNETPET_REQ_PETCARDINFO_FB
	{
		...
		PETTYPE				emTYPE;
		int					nFull;
		SNATIVEID			sActiveSkillID[PETSLOT_SIZE];	//dmk14 | 11-21-16 | dual pet skill
			...
			bDualSkill(FALSE)
		{
			...
			std::fill_n(sActiveSkillID, (int) PETSLOT_SIZE, NATIVEID_NULL() );	//dmk14 | 11-21-16 | dual pet skill
			nmg.dwSize = sizeof(*this);
...
//-----------------------------------------------------------------------------------------------------
	...
	struct SNETPET_REQ_DUAL_SKILL_FB
	{
		...
		SNETPET_REQ_DUAL_SKILL_FB () 
			: dwPetID(0)
			, emFB( EMPET_REQ_DUAL_SKILL_FB_INVALID_SKILL )
		{
			nmg.dwSize = sizeof(*this);
