...
HRESULT GLGaeaServer::Create ( DWORD dwMaxClient, DxMsgServer *pMsgServer, DxConsoleMsg* pConsoleMsg, GLDBMan* pDBMan, int nServiceProvider, const char* szMapList, DWORD dwFieldSID, int nChannel, BOOL bUseIntelTBB )
{
	...
	for ( ; iter!=iter_end; ++iter )
	{
		...
		pNewLandMan->SetSchoolWarsZone ( pMapNode->bSWZone ); //Added by JADev
		pNewLandMan->SetRoyalRumbleZone ( pMapNode->bRRZone ); //Added by JADev
...
PGLPETFIELD GLGaeaServer::CreatePET ( PGLPET pPetData, DWORD dwOwner, DWORD dwPetID, bool bValid )
{
	...
	if ( pPet->IsValid () )
	{
		...
		NetMsg.m_sActiveSkillID[s]= pPet->m_sActiveSkillID[s];
...
BOOL GLGaeaServer::DropOutPET ( DWORD dwGUID, bool bLeaveFieldServer, bool bMoveMap )
{
	...
	if ( pOwner && bValid )
	{
		...
		//dmk14 | 11-21-16 | dual pet skill
		for( unsigned s=0; s<PETSLOT_SIZE;++s)
		pOwner->m_sPETSKILLFACT[s].RESET ();
	...
	if ( bLeaveFieldServer )
	{
		...
		//dmk14 | 11-21-16 | dual pet skill
		for( unsigned s=0; s<PETSLOT_SIZE;++s) {
		if ( pPet->m_sActiveSkillID[s] != NATIVEID_NULL() )
		{
			if( pPet->m_sActiveSkillID[s].wMainID != 26 )
			{
				CDebugSet::ToLogFile( "ERR : GLGaeaServer::DropOutPET, MID = %d, SID = %d",  
													pPet->m_sActiveSkillID[s].wMainID,
													pPet->m_sActiveSkillID[s].wSubID );
			}

			PETSKILL sPetSkill( pPet->m_sActiveSkillID[s], 0 );
			CSetPetSkill* pSaveSkill = new CSetPetSkill ( pOwner->m_dwCharID, pPet->GetPetID (), sPetSkill, true, s );
...
void GLGaeaServer::CreatePETOnDB ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPET_CREATEPET_FROMDB_FB* pNetMsg )
{
	...
	*/
	...
	//dmk14 | 11-21-16 | dual pet skill
	for( unsigned s=0; s<PETSLOT_SIZE;++s)
	NetMsg.sActiveSkillID[s]	= pMyPet->m_sActiveSkillID[s];
...
void GLGaeaServer::GetPETInfoFromDB ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPET_GETPET_FROMDB_FB* pNetMsg )
{
	...
	if ( pNetMsg->bTrade || pNetMsg->bCardInfo )
	{
		...
		//dmk14 | 11-21-16 | dual pet skill
		for( unsigned s=0; s<PETSLOT_SIZE;++s)
		NetMsgFB.sActiveSkillID[s]	= pNetMsg->pPet->m_sActiveSkillID[s];
...
//								,pOwner->m_sVehicle.m_dwGUID, pOwner->m_dwCharID );
	...
	GLLandMan* pLandMan = GetByMapID ( pOwner->m_sMapID );
	if ( !pLandMan )					return FALSE;
	if ( !pLandMan->IsVehicleActivity () && pNetMsg->bActive && pOwner->m_dwUserLvl < USER_GM3 )
	{
...
void GLGaeaServer::SetActiveVehicle ( DWORD dwClientID, DWORD dwGaeaID, bool bActive )
{
	...
	GLLandMan* pLandMan = GetByMapID ( pOwner->m_sMapID );
	if ( !pLandMan )					return;
	if ( !pLandMan->IsVehicleActivity () && bActive && pOwner->m_dwUserLvl < USER_GM3 )
	{
		NetMsgFB.emFB = EMVEHICLE_SET_FB_MAP_FAIL;
		SENDTOCLIENT ( dwClientID, &NetMsgFB );
		return;
...
void GLGaeaServer::SaveVehicle( DWORD dwClientID, DWORD dwGaeaID, bool bLeaveFieldServer )
{
	...
	//jdev todo
	CSetVehicleColor* pSaveColor = new CSetVehicleColor( dwClientID, pChar->m_dwCharID, dwVehicleGUID, pChar->m_sVehicle.m_wColor ); //add bike color
...
{
	GLMSG::SNETPC_REQ_HOLD_ARRAY *pNetMsg = reinterpret_cast<GLMSG::SNETPC_REQ_HOLD_ARRAY*> ( nmg );
	
	for( int i = 0; i < 300; i++ )
	{
		if ( pNetMsg->wPosX[i] == -1 && pNetMsg->wPosY[i] == -1 ) continue; 

		SINVENITEM *pInvenItem = m_cInventory.GetItem ( pNetMsg->wPosX[i], pNetMsg->wPosY[i] );
		...
		HOLD_ARRAY ( sInven_BackUp.sItemCustom, i );
...
HRESULT GLChar::MsgReqHoldArrayToInven ( NET_MSG_GENERIC* nmg )
{
	SITEMCUSTOM sITEMCUSTOM[300];
	for( int a = 0; a < 300; a++ )
	{
		if ( GET_HOLDARRAY_ITEM(a).sNativeID != NATIVEID_NULL()) sITEMCUSTOM[a] = GET_HOLDARRAY_ITEM(a);

		RELEASE_HOLDARRAY_ITEM(a);
	}
	...
	for( int i = 0; i < 300; i++ )
	{
		...
		/*bOk = m_cInventory.InsertItem ( sITEMCUSTOM[i]/*GET_HOLDARRAY_ITEM(i)*0/, _X, _Y );
		if ( !bOk )		return E_FAIL;
