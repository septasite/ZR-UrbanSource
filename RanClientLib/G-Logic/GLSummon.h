...
enum EMSUMMONACTSTATE
{
	...
	EM_SUMMONACT_WAIT			  = 0x00000002,		// 잠시?E갋
	EM_SUMMONACT_STOP			  = 0x00000004,		// 완?Eㅑ갋
	EM_SUMMONACT_RUN			  = 0x00000010,		// 뛰기모?E
	EM_SUMMONACT_WALK			  = 0x00000020,		// 걷기모?E
	EM_SUMMONACT_RUN_CLOSELY	  = 0x00000040,		// 근접거리깩?E뛰기모?E
	EM_SUMMONACT_ATTACK			  = 0x00000100,		// Attack모?E
	EM_SUMMONACT_TRACING		  = 0x00000200,		// 추적모?E
};
...
struct GLSUMMON
{
	...
	float			m_fWalkArea; // 걸엉雹만 하는 거리
	float			m_fOwnerDistance; // 주인과의 유?E거리
	float			m_fPetDistance;  // ?E珦?유?E거리
...
struct SDROPSUMMON
{
	...
	WORD			m_wAniSub;					 // 애니메이션 ?E?
	DWORD			m_dwSummonID;

	DWORD			m_dwNowHP;						//	생뫄端.
	WORD			m_wNowMP;						//	정신량.

	//	SNATIVEID		m_sPosionID;					//  자동 사?E포션
