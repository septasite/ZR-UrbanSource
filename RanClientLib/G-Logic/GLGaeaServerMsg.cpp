...
BOOL GLGaeaServer::ChatMsgProc ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	case CHAT_TYPE_REGION : //add region chat by AnFire16 ( 7/31/2017)
		break;
...
// *****************************************************
// Desc: ¸Ê ÁøÀÔ Ã³¸® ( Normal Map, Instant Map °ø?E»ç?E)
// *****************************************************
BOOL GLGaeaServer::EntryLand( DWORD dwGaeaID, DWORD dwGateID, BOOL bInstantMap, SNATIVEID sMapID /*= NATIVEID_NULL()*/ )
{
	...
	if ( pInLandMan->m_bRoyalRumbleMap )
	{
		srand(time(NULL));
		dwTOGATEID =  (DWORD)RANDOM_GATE ;
	}

	//add sw landgate
	if ( pInLandMan->m_bSchoolWarsMap ) pInGate = pInGateMan->FindLandGate ( (DWORD)pPC->m_wSchool );
	...
	//	Note : »õ·Î?E¸Ê À§Ä¡·Î ³»ºñ°ÔÀÌ¼Ç ÃÊ±âÈ­.
	//
	...
	// Áß±¹ÀÏ °æ?EGainTypeÀ» ÃÊ±âÈ­ ÇÏ¿© ¸Þ½ÃÁö¸¦ ÇÑ ?E?E¶ç?E?ÇÑ´Ù.
#ifdef CH_PARAM_USEGAIN //chinatest%%%
	pPC->m_ChinaGainType = 255;
#endif


	//	Note : GLLandManÀÇ ¼¿¿¡ µûÓÏÇÏ´Â ÀÛ?E
	//			
	//	RegistChar(pPC); --> GLGaeaServer::RequestLandIn ()ÀÌ È£ÃâµÉ ¶§±û?EÀ¯º¸µÊ.
	...
	// ÀÚ½ÅÀÇ ¹öÇÁ¿Í »óÅÂÀÌ?EÈ¿°ú¸¦ ¸ðµÎ »ç¶óÁö°Ô ÇØÁØ´Ù.
...
{	
	...
	//fix autodeads hp multi - Eduj Kun
	PGLCHAR pPC = GetChar ( pNetMsg->dwGaeaID );
    // your tyranny mapid
	SNATIVEID sToMapID;
	sToMapID.wMainID = 223;
	sToMapID.wSubID = 0;
    
	GLLandMan* pInLandMan = NULL;
	pInLandMan = GetByMapID ( sToMapID );

	if ( pInLandMan->m_bTowerWarsMap  )
		pInLandMan->UpdateAddHPmultiplier( pPC->m_wSchool );
...
BOOL GLGaeaServer::MsgReqHoldArrayToInven ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_REQ_HOLDARRAY_TO_INVEN* pNetMsg )
{
	...
	for( int a = 0; a < 300; a++ )
	{
		if ( pPC->GET_HOLDARRAY_ITEM(a).sNativeID != NATIVEID_NULL()) sITEMCUSTOM[a] = pPC->GET_HOLDARRAY_ITEM(a);
	...
	for(int b=0; b<300; b++)
	{
		for(int c=0; c<300-1; c++)
		{
			...
			if ( bCOMPARE ) 
			{
				SITEMCUSTOM sCUSTOM = sITEMCUSTOM[c+1];
				sITEMCUSTOM[c+1] = sITEMCUSTOM[c];
				sITEMCUSTOM[c] = sCUSTOM;
			}
		}
	}
...
BOOL GLGaeaServer::RequestMustLeaveMap( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_REQ_MUST_LEAVE_MAP* pNetMsg )
{
	...
	{
		SENDTOCLIENT ( dwClientID, &NetMsgFB ); // ½ÇÆÐ ¸Þ½Ã?E
		return FALSE;
	}
...
BOOL GLGaeaServer::RequestCreateInstantMapReq( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETREQ_CREATE_INSTANT_MAP_REQ *pNetMsg )
{
	...
	// ¼­¹ö¿¡ »ý¼ºÇÒ?EÀÖ´Â ¸Þ¸ð¸®¸¦ ÃÊ°úÇßÀ» °æ?E»ý¼ºÀ» ÇÏ?E¾Ê´Â´Ù.
...
BOOL GLGaeaServer::RequestGateOutReq ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETREQ_GATEOUT_REQ* pNetMsg )
{
	...
	if ( pLandMan )
	{
		...
		if ( pMapNode->bSWZone )
		{
			...
			if ( pSW )
			{
				if ( !pSW->IsEnterMap(pPC->m_dwCharID) )
				{
					NetMsgFB.emFB = EMCHAR_GATEOUT_HOLD;
					SENDTOCLIENT ( pPC->m_dwClientID, &NetMsgFB );
					return FALSE;
		...
		if ( pMapNode->bRRZone )
		{
			...
			if ( pRR )
			{
				if ( !pRR->IsEnterMap(pPC->m_dwCharID) )
				{
					NetMsgFB.emFB = EMCHAR_GATEOUT_HOLD;
					SENDTOCLIENT ( pPC->m_dwClientID, &NetMsgFB );
					return FALSE;
				...
				DWORD dwMY_PARTYID = pPC->GetPartyID();
				GLPARTY_FIELD* pPARTY = m_cPartyFieldMan.GetParty ( dwMY_PARTYID );
				
				if ( pPARTY )
				{
					NetMsgFB.emFB = EMCHAR_GATEOUT_NOPARTY;
					SENDTOCLIENT ( pPC->m_dwClientID, &NetMsgFB );
					return FALSE;
				}
			}
...
BOOL GLGaeaServer::RequestReBirthOut ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_REBIRTH_OUT *pNetMsg )
{
	...
	if ( !pPC->IsSTATE(EM_ACT_DIE) )
	{
		GetConsoleMsg()->Write ( LOG_TEXT_CONSOLE, 
			"State : %d , RequestReBirthOut() : ºÎÈ°¸Þ½Ã?EÃ³¸®Áß »ç¸Á ÇÃ·¡±×°¡ ¾ø¾ûØ­ ¿äÃ»À» ¹«½ÃÇÏ¿´½À´Ï´Ù.", pPC->GetSTATE() );
	}
	...
	//	Note : AgentÀ¸·Î Ã³¸® ¿Ï?E¾Ë¸², actor¿¡ ¿¹¾àÇÔ.	( new µÈ °´Ã¼´Â db actor ¿¡¼­ »èÁ¦µÈ´Ù. )
	//
...
BOOL GLGaeaServer::RequestTradeItemResist ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_TRADE_ITEM_REGIST *pNetMsg )
{
	...
	// ÆÖÄ«µåÀÌ¸é¼­ ÆÖÀÌ È°¼ºÈ­ µÇ?EÀÖÀ¸?E°Å·¡ÇÒ ?E¾ø´Ù.
	...
	// GMCharEdit ·Î ³ÖÀº ¾ÆÀÌÅÛÀº ¹ö¸®?EºÒ°¡´ÉÇÏ´Ù.
	if ( pInvenItem->sItemCustom.IsGM_GENITEM() )			return FALSE;
	if ( pInvenItem->sItemCustom.IsNonDrop_Ex() )			return FALSE;
	}
...
BOOL GLGaeaServer::RequestTradeAgree ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_TRADE_AGREE *pNetMsg )
{
	...
	if ( pMyChar->m_sTrade.GetAgree() && pTarChar->m_sTrade.GetAgree() )
	{
		...
		do
		{
			...
			//	Note : ¹°Ç° ±³È¯ °ø°£ÀÌ ÀÎ?E?ÃæºÐÇÑ?E°Ë?E
			//
			BOOL bOk;
			...
			if ( !bOk ) break; // µ· ±Ý¾×ÀÌ ¸¶ÀÌ³Ê½º°¡ µÇ´Â °æ?E(?E?ÇØÅ·)
			//GASSERT ( bOk && "DoTrade()¿¡¼­´Â FALSE°¡ ÀÏ¾ûÏª¼­´Â ¾ÈµÈ´Ù." );
...
BOOL GLGaeaServer::RequestFieldSvrCharChk ( DWORD dwClientID, GLMSG::SNETPC_FIELDSVR_CHARCHK *pNetMsg  )
{
	...
	//	Note : °°Àº ÀÌ¸§ÀÇ Ä³¸¯ÅÍ°¡ ÀÌ¹Ì Á¢¼ÓµÇ?EÀÖ´Â?E°Ë»çÇÕ´Ï´Ù.
	{
		...
		if ( name_iter != m_PCNameMap.end() )
		{
			...
			//	Á¾?EÁ¢¼ÓÀÚ¿¡°Ô DropOut ¸Þ½ÃÁö¸¦ º¸³À´Ï´Ù.
	...
	//	Note : È¤½Ã user id °¡ db ÀÛ¾÷ÁßÀÎ?E°Ë?E
	...
	if ( bExist )
	{
		if ( m_pDBMan )
		{
			//	Note : DB ÀúÀåÈÄ¿¡ ¸Þ½Ã?EÃ³¸®ÇÏ?EÀ§ÇØ¼­ db action¿¡ µûÓÏÇÏ¿© ¸Þ½Ã?E?E?
...
BOOL GLGaeaServer::GMCtrolMove2MapPos ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_GM_MOVE2MAPPOS_FLD *pNetMsg )
{
	...
	else
	{
		...
		pChar->DISABLEALLLANDEFF();
		//fix autodeads hp multi - Eduj Kun
		if ( pChar->m_pLandMan->m_bTowerWarsMap ) pChar->m_pLandMan->UpdateAddHPmultiplier ( pChar->m_wSchool );
...
BOOL GLGaeaServer::GMCtrlSlotEVENT ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_GM_SLOT_EVENT_FLD* pNetMsg )
{
	GLMSG::SNET_GM_SLOT_EVENT_BRD NetMsgBrd;
	NetMsgBrd.bOn = pNetMsg->bOn;
...
BOOL GLGaeaServer::GMCtrolClassEvent   ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_GM_CLASS_EVENT *pNetMsg )
{
	...
	bool bClassEvent[GLCI_NUM_NEWEST];// 
	for( BYTE i = 0; i < GLCI_NUM_NEWEST; i++ )// 
		bClassEvent[i] = FALSE;
	...
	if( pNetMsg->dwClassIndex&GLCC_ASSASSIN_M ) bClassEvent[12] = TRUE;	
	if( pNetMsg->dwClassIndex&GLCC_ASSASSIN_W ) bClassEvent[13] = TRUE;	
	if( pNetMsg->dwClassIndex&GLCC_TESTING_M ) bClassEvent[14] = TRUE;	
	if( pNetMsg->dwClassIndex&GLCC_TESTING_W ) bClassEvent[15] = TRUE;	

	if( pNetMsg->bStart )
	{		
		for( i = 0; i < GLCI_NUM_NEWEST; i++ )// 
		{
...
BOOL GLGaeaServer::GMCtrolMoPrintCrowList ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNET_GM_PRINT_CROWLIST_FLD* pNetMsg )
{
	...
	//	Note : °¡½Ã ¿µ¿ª¿¡ ÀÖ´Â ¸ð?E¼¿ÀÇ ¿À?E§Æ®µéÀ?Å½»öÇÑ´Ù.
	//
	int i;
	PGLCROW		pCrow;
	PCROWDATA	pCrowData;
...
BOOL GLGaeaServer::RequestViewAllPlayer ( GLMSG::SNET_GM_VIEWALLPLAYER_FLD_REQ *pNetMsg )
{
	...
	for ( ; pQuadNode; pQuadNode = pQuadNode->pFindNext )
	{
		...
		GASSERT(pLandTree->GetCellNum()>pQuadNode->dwCID&&"ÀÎµ¦½º°¡ ¼¿»çÀÌÁûÔ¦ ÃÊ°úÇÏ¿´½À´Ï´Ù.");
	...
	{	
		...
		if( pNetMsg->bBasicInfo )
		{
			...
			strItemInfo.Format( "%d, %d, %d, %d, %I64d, %d, %f, %d, %f, %d, %f, %d, %f, %d, %d, %d, %d, %d, %d, %d\n", 
													   ...
													   searchItem.cDAMAGE,				// °ø°Ý ¿¬¸¶ µûÍÞ
													   searchItem.cDEFENSE,				// ¹æ?E¿¬¸¶ µûÍÞ

													   searchItem.cRESIST_FIRE,				//	ÀúÇ×(È­) °³Á¶ µûÍÞ.
													   searchItem.cRESIST_ICE,				//	ÀúÇ×(?E °³Á¶ µûÍÞ.
													   searchItem.cRESIST_ELEC,				//	ÀúÇ×(?E °³Á¶ µûÍÞ.
													   searchItem.cRESIST_POISON,			//	ÀúÇ×(µ¶) °³Á¶ µûÍÞ.
													   searchItem.cRESIST_SPIRIT );			//	ÀúÇ×(Á¤) °³Á¶ µûÍÞ.
...
BOOL GLGaeaServer::RequestConfrontParty ( GLMSG::SNETPC_CONFRONTPTY_START2_FLD *pNetMsg )
{
	...
	//	Note : µÎ¹øÂÅ ÆÄÆ¼¿øµé¿¡ 'ÆÄÆ¼?E? Á¤º¸ ¼³Á¤.
	//
...
BOOL GLGaeaServer::RequestConfrontPartyEnd ( GLMSG::SNETPC_CONFRONTPTY_END2_FLD *pNetMsg )
{
	if ( pNetMsg->dwPARTY_A!=PARTY_NULL && pNetMsg->dwPARTY_B!=PARTY_NULL )
	{
		...
		//	Note : ÆÄÆ¼ ?EÃÈ?ÆÄÆ¼°¡ »ç¶ó?EºÎºÐÀÌ ÀÖÀ¸?E¿À?EÃ³¸®.
		//
		if ( pPartyA && pPartyB )
		{
...
BOOL GLGaeaServer::ServerSchoolWarsReady ( GLMSG::SNET_SCHOOL_WARS_READY_FLD *pNetMsg )
{
	GLSchoolWarsFieldMan::GetInstance().ReadyBattle ( pNetMsg->dwID );

	return TRUE;}

BOOL GLGaeaServer::ServerSchoolWarsStart ( GLMSG::SNET_SCHOOL_WARS_START_FLD *pNetMsg )
{
	m_bSchoolWarsStarted = true;
	GLSchoolWarsFieldMan::GetInstance().BeginBattle ( pNetMsg->dwID );

	return TRUE;
}

BOOL GLGaeaServer::ServerSchoolWarsEnd ( GLMSG::SNET_SCHOOL_WARS_END_FLD *pNetMsg )
{
	m_bSchoolWarsStarted = false;
	GLSchoolWarsFieldMan::GetInstance().EndBattle ( pNetMsg->dwID );

	return TRUE;
}

BOOL GLGaeaServer::ServerRoyalRumbleReady ( GLMSG::SNET_ROYAL_RUMBLE_READY_FLD *pNetMsg )
{
	GLRoyalRumbleFieldMan::GetInstance().ReadyBattle ( pNetMsg->dwID );

	return TRUE;
}

BOOL GLGaeaServer::ServerRoyalRumbleStart ( GLMSG::SNET_ROYAL_RUMBLE_START_FLD *pNetMsg )
{
	m_bRoyalRumbleStarted = true;
	GLRoyalRumbleFieldMan::GetInstance().BeginBattle ( pNetMsg->dwID );

	return TRUE;
}

BOOL GLGaeaServer::ServerRoyalRumbleEnd ( GLMSG::SNET_ROYAL_RUMBLE_END_FLD *pNetMsg )
{
	m_bRoyalRumbleStarted = false;
	GLRoyalRumbleFieldMan::GetInstance().EndBattle ( pNetMsg->dwID );
...
BOOL GLGaeaServer::ServerTowerWarsEnd ( GLMSG::SNET_TOWERWARS_END_FLD *pNetMsg )
{
	...
	GLTowerWarsFieldMan::GetInstance().EndBattle ( pNetMsg->dwID, pNetMsg->bForceExit );
...
//add sw buff
BOOL GLGaeaServer::ServerSwCertify ( GLMSG::SNET_SCHOOL_WARS_CERTIFIED_FLD *pNetMsg )
{
	GLSchoolWarsFieldMan::GetInstance().ChangeSWSchoolID ( pNetMsg->dwID, pNetMsg->dwSCHOOL );
...
BOOL GLGaeaServer::RequestInvenTeleport ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_REQ_INVEN_TELEPORT* pNetMsg )
{
	...
	//	Note : pk µûÍÞÀÌ ?EÎÀ?µûÍÞ ÀÌ»óÀÏ °æ?E±ÍÈ¯ Ä«µåÀÇ »ç?E?¸·´Â´Ù.
	//
...
//	vPOS = pChar->m_vLastCallPos;	
	...
	//	Note : Á÷?EÍÈ?À§Ä¡ Àú?E ( ½Ã±Í³ª ÇÐ±ÍÀÏ °æ?E)
	//
	pChar->SaveLastCall ();

	//	Note : Á÷?EÍÈ?À§Ä¡ DB¿¡ Àú?E
	//
...
BOOL GLGaeaServer::RequestInvenRecall ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_REQ_INVEN_RECALL *pNetMsg )
{
	...
	//disable SPBPCRC on E-room , CDM, SW - SeiferXIII 7/12/13 /   4/23/2014
	SNATIVEID sCurMapID = pChar->m_sMapID;
	if ( sCurMapID.wMainID==214 ||  sCurMapID.wMainID==211 ||  sCurMapID.wMainID==254 ||  sCurMapID.wMainID==23 ||  sCurMapID.wMainID==24
		||  sCurMapID.wMainID==147 ||  sCurMapID.wMainID==148 )
	{
	NetMsgFB.emFB = EMREQ_RECALL_FB_SP_BP_CRC_DISABLED;
	SENDTOCLIENT ( dwClientID, &NetMsgFB );
	return FALSE;
	}
	...
	if ( pChar->m_dwUserLvl < USER_GM3 )
	{
		...
		// MapList ¿¡ Move flag °¡ ¼³Á¤µÇ?EÀÖ?E¾ÊÀ¸?E
		// Á÷?EÍÈ¯Ä«µå·?ÀÌµ¿ºÒ°¡´É
	...
	if ( pMapNode->bSWZone ) 
	{
		...
		GLSchoolWars *pSW = GLSchoolWarsFieldMan::GetInstance().Find ( pLandMan->m_dwSchoolWarMapID );
		if ( pSW && !pSW->IsEnterMap(pChar->m_dwCharID) )
		{
			NetMsgFB.emFB = EMREQ_RECALL_FB_JOINCON;
			SENDTOCLIENT ( pChar->m_dwClientID, &NetMsgFB );
			pChar->ReSetStartMap();
...
BOOL GLGaeaServer::RequestBus ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_REQ_BUS *pNetMsg )
{
	...
	//	Note : pk µûÍÞÀÌ ?EÎÀ?µûÍÞ ÀÌ»óÀÏ °æ?E¹ö½º »ç?E?¸·´Â´Ù.
	//
	//DWORD dwPK_LEVEL = pChar->GET_PK_LEVEL();
	//if ( dwPK_LEVEL != UINT_MAX && dwPK_LEVEL>GLCONST_CHAR::dwPK_RECALL_ENABLE_LEVEL )
	//{
...
BOOL GLGaeaServer::RequestClubInfo ( GLMSG::SNET_CLUB_INFO_2FLD *pNetMsg )
{
	...
	//	Note : ¸¶½ºÅÍ, ¸â?EÁÖº¯ »ç¶÷µé¿¡°Ô Å¬·´ °£·« Á¤º¸ ?E?
...
BOOL GLGaeaServer::RequestClubDissolution2Fld ( GLMSG::SNET_CLUB_DISSOLUTION_2FLD *pNetMsg )
{
	...
	if ( pMASTER )
	{
		...
		if ( tDisTime.GetYear() != 1970 )
		{
			...
			// Á×¾úÀ» °æ?E?°»½ÅµÇ°Ô ÇÏ?EÀ§ÇÏ¿© ¹Ì¸® ÀúÀåÇØ³õ?E´Ù½Ã ÀúÀåÇÑ´Ù.
...
BOOL GLGaeaServer::RequestClubAdd2Fld ( GLMSG::SNET_CLUB_MEMBER_ADD_2FLD *pNetMsg )
{
	...
	{
		const GLCLUBMEMBER &cMEMBER = (*pos).second;
		if ( cMEMBER.dwID==pNetMsg->dwMember )	continue;		//	ÀÚ?EÀÚ½ÅÀÇ °æ?EÁ¦¿Ü.

		pMember = GetCharID ( cMEMBER.dwID );
		if ( !pMember )							continue;		//	¸â¹ö¿øÀÌ Á¸Á¦ ÇÒ °æ?E?

		//	Note : Å¬¶óÀÌ¾ðÆ®¿¡ ½Å±Ô ¸â?EÁ¤º¸ ?E?
		SENDTOCLIENT ( pMember->m_dwClientID, &NetMsgMember );
	}

	//	Note : ½Å±Ô·Î ?E¡ÇÏ°Ôµ?¸â?E¼³Á¤.
...
//	Note : ¸Þ½Ã?EÃ³¸®´Â GLGaeaServer::GetInstance().FrameMove () ÇÔ¼öºÎ
...
HRESULT GLGaeaServer::MsgProcess ( NET_MSG_GENERIC* nmg, DWORD dwClientID, DWORD dwGaeaID )
{
	...
	case NET_MSG_GCTRL_GET_ITEMSHOP_FROMDB:

	case NET_MSG_GCTRL_REQ_LINKSKILLEFF:	//link skill
	...
	case NET_MSG_INVEN_WRAP: // wrapper
	case NET_MSG_INVEN_NONDROP: //nondrop card Eduj
	case NET_MSG_INVEN_REMOVE_NONDROP: //nondrop card Eduj
	case NET_MSG_INVEN_REMOVE_WRAP:
	case NET_MSG_REBUILD_INPUT_MONEY:
	case NET_MSG_GCTRL_INVEN_SCHOOL_CHANGE:
	...
	case NET_MSG_GCTRL_REQ_DISABLESKILLEFF: //disable skill effect NaJDeV
	case NET_MSG_GCTRL_BUY_ITEMSHOP:
	//dmk14 | 12-9-16 | item compound
	case NET_MSG_GCTRL_INVEN_ITEM_COMPOUND:
	...
	/*case NET_MSG_GCTRL_BUY_ITEMSHOP_ITEM:
		RequestItemShopBuyItem ( dwClientID, dwGaeaID, (GLMSG::SNET_ITEMSHOP_ITEM_BUY*) nmg );
		break;*/
	...
	case NET_MSG_REQ_CROWLIST_FLD: //Request Crow List NaJDeV
	...
	case NET_MSG_GCTRL_SCHOOL_WARS_READY_FLD:
		ServerSchoolWarsReady ( (GLMSG::SNET_SCHOOL_WARS_READY_FLD *)nmg );
		break;

	case NET_MSG_GCTRL_SCHOOL_WARS_START_FLD:
		ServerSchoolWarsStart ( (GLMSG::SNET_SCHOOL_WARS_START_FLD *)nmg );
		break;

	case NET_MSG_GCTRL_SCHOOL_WARS_END_FLD:
		ServerSchoolWarsEnd ( (GLMSG::SNET_SCHOOL_WARS_END_FLD *)nmg );
		break;

	case NET_MSG_GCTRL_ROYAL_RUMBLE_READY_FLD:
		ServerRoyalRumbleReady ( (GLMSG::SNET_ROYAL_RUMBLE_READY_FLD *)nmg );
		break;

	case NET_MSG_GCTRL_ROYAL_RUMBLE_START_FLD:
		ServerRoyalRumbleStart ( (GLMSG::SNET_ROYAL_RUMBLE_START_FLD *)nmg );
		break;

	case NET_MSG_GCTRL_ROYAL_RUMBLE_END_FLD:
		ServerRoyalRumbleEnd ( (GLMSG::SNET_ROYAL_RUMBLE_END_FLD *)nmg );
		break;

	case NET_MSG_GCTRL_TOWERWARS_WINNER_FLD:
		ServerTowerWarsWinner ( (GLMSG::SNET_TOWERWARS_WINNER_FLD *)nmg );
		break;
	...
	case NET_MSG_GCTRL_CLUB_CERTIFIED_FLD:
		...
		break;
	//add sw buff
	case NET_MSG_GCTRL_SCHOOL_WARS_CERTIFIED_FLD:
		ServerSwCertify ( (GLMSG::SNET_SCHOOL_WARS_CERTIFIED_FLD *)nmg );
		break;

	case NET_MSG_GCTRL_GLOBAL_SUCCESS_GRIND:
		{
			
			GlobalUpgrade ( (GLMSG::SNET_GLOBAL_SUCCESS_GRIND*) nmg );
		}
	...
	case NET_MSG_GM_SLOT_EVENT_FLD:
		GMCtrlSlotEVENT ( dwClientID, dwGaeaID, (GLMSG::SNET_GM_SLOT_EVENT_FLD*) nmg );
		break;
	...
	case NET_MSG_GCTRL_SCHOOL_WARS_RANKING_REQ:
		ReqSchoolWarsRanking ( dwClientID, dwGaeaID, (GLMSG::SNET_SCHOOL_WARS_RANKING_REQ*) nmg );
		break;

	case NET_MSG_GCTRL_SCHOOL_WARS_PRANKING_REQ:
		ReqSchoolWarsPRanking ( dwClientID, dwGaeaID, (GLMSG::SNET_SCHOOL_WARS_PRANKING_REQ*) nmg );
		break;
	case NET_MSG_GCTRL_SCHOOLWAR_NEXTWAR_REQ:
		ReqSetNextWarTime ( dwClientID, dwGaeaID, (GLMSG::SNET_SCHOOLWAR_NEXTWAR_REQ*) nmg );
		break;
	case NET_MSG_GCTRL_ROYAL_RUMBLE_PRANKING_REQ:
		ReqRoyalRumblePRanking ( dwClientID, dwGaeaID, (GLMSG::SNET_ROYAL_RUMBLE_PRANKING_REQ*) nmg );
		break;
...
//----------------------------------------------------------------------------------------------------------------
	...
	case NET_MSG_PET_DUAL_SKILL:		//dmk14 | 11-21-16 | dual pet skill
		{
	...
	default:
		{
			...
			if ( pChar )
			{
				GetConsoleMsg()->Write ( LOG_CONSOLE,
			                    "[WARNING]GLGaeaServer::MsgProcess illegal message(%d) Name(%s)",
								nmg->nType, pChar->m_szName );
			}
			else
			{
				GetConsoleMsg()->Write ( LOG_CONSOLE,
			                    "[WARNING]GLGaeaServer::MsgProcess illegal message(%d)",
			                    nmg->nType);
			}
		}
		break;
...
BOOL GLGaeaServer::ServerTowerWarsWinner ( GLMSG::SNET_TOWERWARS_WINNER_FLD *pNetMsg )
{
	GLTowerWarsFieldMan::GetInstance().WinnerField ( pNetMsg->dwID, pNetMsg->wWINNERSCHOOL );
	return TRUE;
}
BOOL GLGaeaServer::ReqCrowList ( DWORD dwClientID, DWORD dwGaeaID, GLMSG::SNETPC_REQ_CROWLIST_FLD* pNetMsg ) //Request Crow List NaJDeV
{
	PGLCHAR pCHAR = GetChar ( pNetMsg->dwGaeaID );
