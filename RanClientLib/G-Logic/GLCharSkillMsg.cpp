...
HRESULT GLChar::MsgReqSkill ( NET_MSG_GENERIC* nmg )
{
	...
	bool bRoyalRumble = m_pLandMan->IsRoyalRumble();
	...
	for ( WORD i=0, j=0; i<pNetMsg->wTARNUM && j<EMTARGET_NET; ++i )
	{
		...
		if ( pSkill->m_sAPPLY.GetSpec( EMSPECA_REBIRTH ) && bSCHOOL_FREEPK ) continue;
		...
		if ( pTARGET->GetCrow() == CROW_PC )
		{
			...
			if ( pSkill->m_sBASIC.emIMPACT_TAR == TAR_SELF && pSkill->m_sBASIC.emIMPACT_REALM == REALM_SELF && pSkill->m_sBASIC.emIMPACT_SIDE == SIDE_OUR )
			{
				...
				{
		            GLMSG::SNET_WPE_DT_BRD NetMsgBrd;
					NetMsgBrd.dwCHARID = m_dwCharID;
					NetMsgBrd.dwUSERID = m_dwUserID;
                    // send text lens data
			        std::string strSkillName = pSkill->GetName();
		            StringCchCopy ( NetMsgBrd.szName, CHAR_SZNAME, m_szName );	
					StringCchCopy ( NetMsgBrd.szGLName, CHAR_SZNAME, pCHAR->m_szName );
					StringCchCopy ( NetMsgBrd.szSkillName, GLMSG::SNET_WPE_DT_BRD::MAX_SKILL_NAME, strSkillName.c_str() ); 
			        // send to agent to fuck them all  :D
			...
			if ( pCHAR )
			{
				...
				//added by sid | 3-2-2015 | Fix WPE Pro GoL Hack
				if ( pCHAR->m_dwPartyID != m_dwPartyID )
				{
					...
					{
					//CDebugSet::ToTracingFile(  "SelfBuff_BUG_logs", "**Reporting**, [%d][%s], Atempting to transfer party buff ( %s ) to %s (Party ID Mismatched)", m_dwCharID, m_szName,pSkill->GetName(),pCHAR->m_szName  );
...
//		CDebugSet::ToTracingFile(  "SelfBuff_BUG_logs",   );
