...
public:
	...
	DWORD				m_dwGaeaID;				//	생성 메모리 인덱스?E( 해?EGLLandMan내에서 유니크함 )
	DWORD				m_dwCeID;				//	셀 ID.

	GLLandManClient*		m_pLandManClient;	//	맵 인스턴스 포인터.
	SGLNODE<GLCharClient*>*	m_pGlobNode;		//	맵 리스트 ?E갋
	CLIENTQUADNODE*			m_pQuadNode;		//	쿼?E트리 ?E갋
	SGLNODE<GLCharClient*>*	m_pCellNode;		//	셀 리스트 ?E갋

	DWORD					m_dwSummonGUID;					//  소환?EID
...
protected:
	...
	float				m_fITEM_MOVE;					//  아이템 이동 속도 절?E?
	float				m_fITEMATTVELO;				//  아이템 공격속도 절?E?
