...
//	pCDM->m_bBattleEndCheck = true;
	...
	UpdateCDMRanking( dwID, TRUE );//Mike915
	CDMAwardItem( dwID );
...
bool GLClubDeathMatchFieldMan::FrameMove ( float fElaps )
{
	for ( CDM_VEC::size_type i=0; i<m_vecClubDM.size(); ++i )
	{
		...
		if( sClubDM.IsBattle() )
		{
			...
			{
				UpdateCDMRanking( sClubDM.m_dwID );// Mike915
...
void GLClubDeathMatchFieldMan::UpdateCDMRanking( DWORD dwID, bool Reveal )// Mike915
{
	GLClubDeathMatch *pCDM = Find ( dwID );
	if ( !pCDM )	return;
