...
HRESULT GLSummonClient::Create ( const PGLSUMMON pSummonData, D3DXVECTOR3 vPos, D3DXVECTOR3 vDir, NavigationMesh* pNavi, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( !pCharData )
	{
		...
		{
			CDebugSet::ToLogFile ( "GLSummonClient::CreateCrow()-DxSkinCharDataContainer::LoadData() [%s] 가져오?E실패.", m_pSummonCrowData->GetSkinObjFile() );
		}
		return E_FAIL;
	}
			...
			{
				{
					...
					// 주인이 걸으?E거리에 따?E뛰거나 걷는다
					// [주의] 위의 코드에서 이미 팻의 움직임 상태를 변경했으므로 
					//		   주인의 걷는 상태에 ?E?처리만 해주?E된다.
					if ( !m_pOwner->IsSTATE ( EM_ACT_RUN ) )
					{
						vDist = m_vPos - vOwnerPos;
						float fDist = D3DXVec3Length(&vDist);

						// 주인?E거리가 너무 멀엉痺?E일정거리를 유지하?E위해 달린다
					...
					// 1초에 한번씩 이동메시지를 ?E徘磯? (이부분은 수정해봐야 할딪豺다)
					fTimer += fElapsedTime;
					if ( fTimer > 1.0f )
					{
...
HRESULT GLSummonClient::FrameMove( float fTime, float fElapsedTime )
{
	...
	// UpdateSkillDelay ( fElapsedTime ); 기획팀 김병웜岷 요청으로 제거 [06.10.09]
	...
	if( IsSTATE( EM_SUMMONACT_WAIT  ) )	// 잠시?E갋
	{
		strState += " 잠시?E갋";
	}
	if( IsSTATE( EM_SUMMONACT_STOP  ) )	// 완?Eㅑ갋
	{
		strState += " 완?Eㅑ갋";
	}
	if( IsSTATE( EM_SUMMONACT_RUN   ) )		// 뛰기모?E
	{
		strState += " 뛰기모?E";
	}
	if( IsSTATE( EM_SUMMONACT_WALK	) )		// 걷기모?E
	{
		strState += " 걷기모?E";
	}
	if( IsSTATE( EM_SUMMONACT_RUN_CLOSELY )	)	// 근접거리깩?E뛰기모?E
	{
		strState += " 근접거리깩?E";
	}
	if( IsSTATE( EM_SUMMONACT_ATTACK	  )	)	// Attack모?E
	{
		strState += " Attack";
	}
	if( IsSTATE( EM_SUMMONACT_TRACING	  )	)	// 추적모?E
	{
		strState += " 추적모?E";
	}
