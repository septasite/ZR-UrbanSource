...
//		기획팀 요청으로 제거함
		...
		nRebornReq = (int)atoi( StrArray[ iCsvCur++ ] );
	...
	SBASIC& SBASIC::operator = ( const SBASIC& rvalue )
	{
		...
		strVPSFile = rvalue.strVPSFile;

		nRebornReq = rvalue.nRebornReq;
...
void SITEMCUSTOM::Assign ( const SITEMCUSTOM_112 &sOld )
{
	...
	cRESIST_POISON		= sOld.cRESIST_POISON;
	cRESIST_SPIRIT		= sOld.cRESIST_SPIRIT;
//	cISDROPITEM         = sOld.cISDROPITEM; // add modified item drop and undrop function eduj
...
void SITEMCUSTOM::Assign ( const SITEM_LOBY &sOldLoby )
{
	...
	//bWrap               = sOldLoby.bWrap;
	//bNonDrop            = sOldLoby.bNonDrop;
}
...
void SITEMCUSTOM::Assign ( const SITEMCLIENT &sClient )
{
	...
	bWrap = sClient.bWrap; // wrapper
	bNonDrop = sClient.bNonDrop; //nondrop card Eduj
...
inline float SITEMCUSTOM::GETOptVALUE ( EMRANDOM_OPT emOPT, short nVALUE ) const
{
	...
	case EMR_OPT_POWER:	// add direct value refixed by JohnArturooo
	case EMR_OPT_VITALITY:	// add direct value refixed by JohnArturooo
	case EMR_OPT_SPIRIT:	// add direct value refixed by JohnArturooo
	case EMR_OPT_DEXTERITY:	// add direct value refixed by JohnArturooo
	case EMR_OPT_STAMINA:	// add direct value refixed by JohnArturooo
	case EMR_OPT_MELEE:
...
void SITEM_LOBY::Assign ( const SITEMCUSTOM &sItemCustom )
{
	...
	bWrap			    = sItemCustom.bWrap;
	bNonDrop		    = sItemCustom.bNonDrop;
};
...
BOOL SETPUTONITEMS_BYBUF ( SITEMCUSTOM *pPutOnItems, CByteStream &ByteStream )
{
	...
	for ( DWORD i=0; i<dwNum; i++ )
	{
		...
		else if ( dwVersion==0x0112) // wrapper
		{
		...
		else if ( dwVersion==0x0113) //nondrop card Eduj
		{
			SITEMCUSTOM_112 sItemCustomOld;
			GASSERT(dwSize==sizeof(sItemCustomOld));
			ByteStream.ReadBuffer ( (LPBYTE)&sItemCustomOld, sizeof(sItemCustomOld) );
...
void SITEMCUSTOM::Assign ( const SITEMLINK &sItemLink )
{
	...
	bWrap               = sItemLink.bWrap;
	bNonDrop            = sItemLink.bNonDrop;
}
...
void SITEMCUSTOM::Convert ( SITEMLINK &sItemLink )
{
	...
	sItemLink.bWrap  	= bWrap;
	sItemLink.bNonDrop  = bNonDrop;
}
