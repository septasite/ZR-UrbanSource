...
DxSkinPiece* GLItemMan::LoadSkinPiece ( WORD wMID, WORD wSID, EMCHARINDEX emIndex )
{
	GASSERT(wMID<MAX_MID);
	GASSERT(wSID<MAX_SID);
	GASSERT(m_pd3dDevice&&"장치가 초기화되?E않았음!");
...
HRESULT GLItemMan::SyncUpdateData ()
{
	...
	char strCHAR_FLAGS[GLCI_NUM_NEWEST][256] =//opsimar4
	{
		"FighterM",	//격투부
		"KnightM",	//검도부
		"ArcherW",	//양궁부
...
HRESULT GLItemMan::SyncStringTable()
{
	...
	strBuffer += "// Note : 키(ID)와 내?E?반드시 탭으로 구분되엉雹 합니다.\r\n//\r\n";
	...
	//char * szEncode = new char[dwLength]; // 인코딩배열에 스트링 길이만큼 메모리 할?E
	//StringCchCopy( szEncode, dwLength, strBuffer.GetString() ); // 인코딩배열에 스트링 복?E
	//compbyte::encode ( (BYTE*)szEncode, dwLength ); // 인코?E

	CFile file;
	file.Open( _T(szFullPathFileName), CFile::modeCreate|CFile::modeWrite ); // 파일 열?E
	file.Write( strBuffer.GetString(), strBuffer.GetLength()-2 ); // 파일 쓰?E
	file.Close();
...
//#ifdef DAUMTESTPARAM	// 한국 테섭에서만 2?E랜덤 옵션 파일 사?E( 준?E)
	SRANDOM_GEN* GLItemMan::LoadRandomGenData ( const char* szName )
	{
		...
		if ( sGEN.dwSET_NUM >= SRANDOM_GEN::MAX_SET ){
			sGEN.dwSET_NUM = SRANDOM_GEN::MAX_SET;
		}
