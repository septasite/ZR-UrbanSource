...
HRESULT GLAnyPet::Create ( PSDROPPET pDropPet, NavigationMesh* pNavi, LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	if ( m_sActiveSkillID[s] != NATIVEID_NULL () )
	{
		...
		if ( pTarChar )
		{
			...
			FACTEFF::NewSkillFactEffect ( sTargetID, pTarChar->GetCharSkin (), m_sActiveSkillID[s], pTarChar->GetMatrix (), sTargetID.vPos );
		}
	}
	}	
...
void GLAnyPet::MsgSkillChange ( NET_MSG_GENERIC* nmg )
{
	...
	if ( pTarChar )
	{
		FACTEFF::DeleteSkillFactEffect ( STARGETID(CROW_PC, pTarChar->m_dwGaeaID, pTarChar->GetPosition () ), pTarChar->GetCharSkin (), m_sActiveSkillID[pNetMsg->nIndex] );
	}
	...
	// 이펙트 처리
	if ( m_sActiveSkillID[pNetMsg->nIndex] != SNATIVEID(false) )
	{
		PGLSKILL pSkill = GLSkillMan::GetInstance().GetData ( m_sActiveSkillID[pNetMsg->nIndex].wMainID, m_sActiveSkillID[pNetMsg->nIndex].wSubID );
		if ( !pSkill )								return;
		...
		{
			STARGETID sTargetID( CROW_PC, pTarChar->m_dwGaeaID, pTarChar->GetPosition () );
			FACTEFF::NewSkillFactEffect ( sTargetID, pTarChar->GetCharSkin (), m_sActiveSkillID[pNetMsg->nIndex], pTarChar->GetMatrix (), sTargetID.vPos );
		}
	}
}
