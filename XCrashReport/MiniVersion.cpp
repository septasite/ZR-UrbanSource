...
CMiniVersion::CMiniVersion(LPCTSTR lpszPath)
{
	...
	ZeroMemory(m_szCompanyName, sizeof(TCHAR) * (MAX_PATH*2));
	ZeroMemory(m_szProductName, sizeof(TCHAR) * (MAX_PATH*2));
	ZeroMemory(m_szFileDescription, sizeof(TCHAR) * (MAX_PATH*2));
...
BOOL CMiniVersion::Init()
{
	...
	ZeroMemory(m_pData, sizeof(BYTE) * (dwSize+1));
