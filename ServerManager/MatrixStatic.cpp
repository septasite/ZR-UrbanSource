...
void CMatrixStatic::SetArrange(int nArrange)
{
	m_nArrange = nArrange;
}
...
int CMatrixStatic::GetText(LPTSTR string, int size)
{
	...
	if (size < length || length == 0)
		return 0;
	_tcscpy((LPTSTR)string, m_csStrText.GetString());
	return length;
}
...
void CMatrixStatic::DoScroll(int speed, BOOL dir)
{
	if (m_btimer == false)
	{
		m_btimer = true;
		m_iDirection  = dir;
		m_iTimer = (UINT) SetTimer(1, speed, NULL); 
		ASSERT(m_iTimer);
	}
}
...
COLORREF SEGM_COLORS[3] = {
	...
	if (m_nArrange == CMatrixStatic::LEFT)
	{	
	...
	else if (m_nArrange == CMatrixStatic::RIGHT)
	{
		x = (m_icharwidth + m_ixspacing) * (m_iMaxXChars - strlen);
		for (int ix = 0; ix < strlen; ix++)
		{
			GetCharBmpOffset(&rc, (char)m_csStrText[ix]);
			dc.BitBlt(x, y, (m_icharwidth + m_ixspacing), (m_icharheight+m_iyspacing), &dcMemory, rc.left, rc.top, SRCCOPY);
