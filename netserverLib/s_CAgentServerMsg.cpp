...
{	
	...
	case CHINA_NET_MSG_REGISTER:			ChinaMsgRegister ( pMsg );			break;
	...
	case NET_MSG_GCTRL_REQ_SERVER_SCHOOL_WARS_REMAIN_AG:
	case NET_MSG_GCTRL_REQ_SERVER_SCHOOL_WAR_REMAIN_AG:
	case NET_MSG_GCTRL_REQ_SERVER_ROYAL_RUMBLE_REMAIN_AG: //add royal rumble - JADev
...
void CAgentServer::OtherMsgProcess(MSG_LIST* pMsg)
{
	...
	//add sw buff
	case NET_MSG_GCTRL_SCHOOL_WARS_CERTIFIED_AGT:
	...
	case NET_MSG_GCTRL_REQ_TO_AGENT_TOWERWARS_WINNER:
...
//	case NET_MSG_GCTRL_INVEN_GRINDING_BRD:
...
#if defined(TH_PARAM) || defined(KR_PARAM)
...
void CAgentServer::MsgCreateCharacter( MSG_LIST* pMsg )
{
	...
	if (nIndex < 0 || nIndex >= GLCI_NUM_NEWEST )	
	{
		nChaNum = -1; // 캐릭터 에러
