...
{	
	...
	// 베트남 탐직방지 시스템의 추가로 캐릭터 저장하는 부분의 수정
		...
		_snprintf( szTemp, 1024, "UPDATE ChaInfo SET ChaBright=%d, ChaLevel=%u, ChaMoney=%I64d, "
					...
					"ChaSpMID = %u, ChaSpSID = %u, ChaCP = '%u', ChaReborn = '%u', ChaPkWin = '%u', ChaPkLoss = '%u', "
					"ChaVotePoint=%I64d, ChaPremiumPoint=%I64d, ChaContributionPoint=%I64d "
					"WHERE ChaNum=%u", 
		...
		pCharData2->m_sCP.dwNow,
		pCharData2->m_nChaReborn, 	//add reborn
		pCharData2->m_dwPkWin, 		//add pk
		pCharData2->m_dwPkLoss, 	//add pk
		//pCharData2->m_sRB.wNow,
