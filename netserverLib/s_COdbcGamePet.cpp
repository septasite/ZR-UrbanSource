	...
	int nPetInvenSlotIndex)
{
	...
	TCHAR szTemp[128] = {0};
	_snprintf( szTemp, 128, "{call sp_UpdatePetInven(%d,%d,%d,%d,%d,%d,%d,%d,%d,?)}", 
		nChaNum,
		nPetNum,
		nPetInvenType,
		nPetInvenMID,
	...
	DWORD dwPetNum)
{
	...
	TCHAR szTemp[256] = {0};
	_snprintf( szTemp, 256, "SELECT PetInvenMID, PetInvenSID, PetInvenAvailable, PetInvenIndex "
	...
	SQLINTEGER nMainID = 0, cbMainID = SQL_NTS;
	SQLINTEGER nSubID = 0, cbSubID = SQL_NTS;
	SQLINTEGER nSlotIndex = 0, cbSlotIndex = SQL_NTS;
	...
	while (true)
	{
		...
		if (sReturn == SQL_SUCCESS || sReturn == SQL_SUCCESS_WITH_INFO)
		{
            ...
            ::SQLGetData(pConn->hStmt, 4, SQL_C_SHORT, &nSlotIndex, 0, &cbSlotIndex);

			PETSKILL PetSkill;
			PetSkill.sNativeID.wMainID = (WORD) nMainID;
			PetSkill.sNativeID.wSubID  = (WORD) nSubID;
			unsigned nIndex = ( unsigned )nSlotIndex;
			...
			if (nAvailable == 1)
			{
				if( nIndex >= PETSLOT_SIZE || nIndex < 0 ) nIndex = 0;
				//dmk14 | 11-21-16 | dual pet skill
				if( pPet->m_bDualSkill && nIndex == 1 ) pPet->m_sActiveSkillID[nIndex] = PetSkill.sNativeID;
				else if ( nIndex == 1 ) pPet->m_sActiveSkillID[nIndex] = NATIVEID_NULL();

				pPet->m_sActiveSkillID[nIndex] = PetSkill.sNativeID;
			}
		}
		else
		{
