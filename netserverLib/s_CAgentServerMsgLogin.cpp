...
void CAgentServer::ChinaMsgLoginBack( NET_LOGIN_FEEDBACK_DATA2* nlfd2 )
{
	...
	if (nlfd2->nResult == EM_LOGIN_FB_SUB_OK)
	{
			...
			(strcmp( m_pClientManager->GetUserID( dwClient ),   nlfd2->szUserid ) == 0))
		{
			...
			nlfd.uChaRemain = nlfd2->uChaRemain;
			nlfd.dwGameTime = nlfd2->nChinaGameTime;
			...
			//CString strText;
			//strText.Format("MsgLoginBack %d %d %s %d",nlfd2->uUserPoints , nlfd2->uBattlePoints , nlfd2->szUserEmail, nlfd2->nChinaGameTime );
			//MessageBox(NULL,strText,"Description",NULL);
