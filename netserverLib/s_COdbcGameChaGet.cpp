								   ...
								   SCHARDATA2* pChaData2)
{
	...
	_snprintf( szTemp, 1024, "SELECT UserNum,SGNum,ChaName,ChaTribe,ChaClass,"
							...
							"ChaGuName,ChaSex,ChaHairColor, ChaReExp, ChaSpMID, ChaSpSID, ChaCP, ChaReborn, ChaPkWin, ChaPkLoss "
							"ChaVotePoint, ChaPremiumPoint,ChaContributionPoint "
...
//		strTemp.freeze( false );	// Note : std::strstream의 freeze. 안 하면 Leak 발생.
	...
	// Add to ChaReExp
	LONGLONG   llChaReExp = 0; SQLINTEGER cbChaReExp=SQL_NTS;
...
//			strTemp.freeze( false );	// Note : std::strstream의 freeze. 안 하면 Leak 발생.
		...
		{		
            ...
            ::SQLGetData(pConn->hStmt,51, SQL_C_LONG, &nChaCP,      0, &cbChaCP);
            //add reborn
			::SQLGetData(pConn->hStmt,52, SQL_C_LONG, &nChaReborn, 0, &cbChaReborn);

			//add pk
			::SQLGetData(pConn->hStmt,53, SQL_C_LONG, &nChaPkWin, 0, &cbChaPkWin);	
			::SQLGetData(pConn->hStmt,54, SQL_C_LONG, &nChaPkLoss, 0, &cbChaPkLoss);
			//add money 2
			...
			pChaData2->m_sCP.dwNow			= (DWORD) nChaCP;  
			//pChaData2->m_sRB.wNow			= (WORD) nChaReborn;  
			
			// add reborn
			pChaData2->m_nChaReborn			= (int) nChaReborn;
            ...
            pChaData2->m_wINVENLINE = (DWORD) nChaInvenLine; 
							  ...
							  SCHARINFO_LOBBY* sci)
{
	...
	_snprintf( szTemp, 512, "SELECT ChaName, ChaClass, ChaSchool, ChaDex, ChaIntel, ChaPower,"
							...
							"ChaSaveMap, ChaHP, ChaCP, ChaReborn FROM ChaInfo "
