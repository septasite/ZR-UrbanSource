	...
	NET_LOGIN_FEEDBACK_DATA2* pMsg )
{
	...
	SQLINTEGER nUserPoint	  = 0, cbUserPoint	   = SQL_NTS;
	SQLINTEGER nBattlePoint	  = 0, cbBattlePoint	   = SQL_NTS;
	SQLCHAR vUserEmail[USR_EMAIL+RSA_ADD+1]	  = {0}; SQLINTEGER cbUserEmail	   = SQL_NTS;
	...
	TCHAR szTemp[256] = {0};
	_snprintf( szTemp, 256, "SELECT UserNum, UserType, PremiumDate, ChaRemain, "
							"ChaTestRemain, ChatBlockDate, GameTime2, OfflineTime, LastLoginDate, UserAge FROM UserInfo WHERE UserID='%s'",
							szUserID );
#elif defined(_RELEASED)

	TCHAR szTemp[256] = {0};
	_snprintf( szTemp, 256, "SELECT UserNum, UserType, PremiumDate, ChaRemain, "
		"ChaTestRemain, ChatBlockDate, LastLoginDate , UserEmail , UserPoint , UserPoint2 , GameTime2 FROM UserInfo WHERE UserID='%s'",
		szUserID );
	...
	TCHAR szTemp[256] = {0};
	_snprintf( szTemp, 256, "SELECT UserNum, UserType, PremiumDate, ChaRemain, "
		"ChaTestRemain, ChatBlockDate , UserEmail , UserPoint , UserPoint2 , GameTime2 FROM UserInfo WHERE UserID='%s'",
		szUserID );
#endif
	...
	else
	{
		...
		while(true)
		{
			...
			{	
				...
				::SQLGetData(pConn->hStmt, 13, SQL_C_LONG,           &nBattlePoint, 0, &cbBattlePoint);
				...
				::StringCchCopy(pMsg->szUserEmail, USR_EMAIL+1, (const char*) vUserEmail);
			}
			else
			{
				break;
		...
		while(true)
		{
			...
			{	
				...
				::SQLGetData(pConn->hStmt, 10, SQL_C_LONG,           &nChinaGameTime, 0, &cbChinaGameTime);
#endif
