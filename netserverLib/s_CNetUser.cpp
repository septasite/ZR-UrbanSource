...
//		strcmp( szTempChar, m_szRecvGarbageMsg[2] ) != 0 &&
		strlen( szTempChar ) != 0 )
	{
		...
		CConsoleMessage::GetInstance()->Write("CNetUser::Client invalid Garbage Msg!! Channel %d, IP %s, UserNum %d, ID %s, GarbageMSG %s %s",
            ...
            //Garbage/Packet Detector - Eduj Kun
			char detectionID[100];
			char detectionLog[100];
			sprintf(detectionID,"netsh advfirewall firewall add rule name=IPblock dir=in interface=any action=block remoteip=%s/32",m_szIp );
			system(detectionID);

			sprintf(detectionLog,"Bad Packet Detected IP:%s IP has been blocked!",m_szIp );   
		    FILE* Detectlog;
		    Detectlog = fopen("Detectlog.txt","a+");
		    fprintf(Detectlog,"%s\n",detectionLog);
		    fclose(Detectlog);	
            // end
