...
//	NetMsgFB.nThaiCC_Class = nResult1;
	...
	case 3 : // ID/PWD 가 일치하고, IP 정보는 없음 (일반적으로 로그인 성공)
		...
		else
		{
			...
			CConsoleMessage::GetInstance()->Write("CAgentUserCheck nChinaGameTime %d", NetMsgFB.nChinaGameTime);
			CConsoleMessage::GetInstance()->Write("CAgentUserCheck uUserPoints %d", NetMsgFB.uUserPoints);
			CConsoleMessage::GetInstance()->Write("CAgentUserCheck uBattlePoints %d", NetMsgFB.uBattlePoints);
			CConsoleMessage::GetInstance()->Write("CAgentUserCheck strEmail %s", NetMsgFB.szUserEmail);
			NetMsgFB.nResult = EM_LOGIN_FB_SUB_OK;
		}
		break;
	case 4 : // ID/PWD 가 일치하지만 IP 가 사용불가능 상태(접근 차단된 IP)
